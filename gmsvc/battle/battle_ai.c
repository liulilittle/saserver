...
static int BATTLE_ai_normal(int, int, BATTLE_ENTRY *, struct B_AI_RESULT *, int battleindex);
#ifdef _SMART_ENEMY_LUA
static int BATTLE_ai_smart(int turn, int charaindex,
                           BATTLE_ENTRY *pEntry,
                           struct B_AI_RESULT *result, int battleindex)
{
    ...
    NewBattleAiCommand(turn, charaindex, result, battleindex);
    //  printf("\n index=%d command=%d target=%d",charaindex,   result->command ,   result->target );
    return TRUE;
}
#endif
static int (*functbl[])(int, int, BATTLE_ENTRY *, struct B_AI_RESULT *, int battleindex) =
{
    NULL,
    BATTLE_ai_normal,
#ifdef _SMART_ENEMY_LUA
...
int BATTLE_ai_all(int battleindex, int side, int turn)
{
    ...
    for (i = 0; i < BATTLE_ENTRY_MAX; i ++)
    {
        ...
        else
        {
            ...
            {
                rc = functbl[mode](turn, charaindex, pEntry, &result, battleindex);
            ...
            if (rc)
            {
                ...
                CHAR_setWorkInt(charaindex, CHAR_WORKBATTLECOM2, result.target + (side ^ 1)*SIDE_OFFSET);
                CHAR_setWorkInt(charaindex, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
