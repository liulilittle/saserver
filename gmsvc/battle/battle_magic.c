...
int BATTLE_MagicDodge(int charindex, int nDefKind, int nFieldAttr)
{
    ...
    {
        if (CHAR_getInt(charindex, CHAR_LUCK) < 1)
        {
            CHAR_setInt(charindex, CHAR_LUCK, 1);
        }

        if (CHAR_getInt(charindex, CHAR_LUCK) > 5)
        {
            CHAR_setInt(charindex, CHAR_LUCK, 5);
        }
...
//  int attattr[5], defattr[5];
    ...
    for (i = 0; i < listidx; i++)
    {
        ...
        {
#ifdef _FIX_MAGICDAMAGE
            float Rmagic = (float)def_magic_resist[FieldAttr];
#ifdef _TRUE_DAMAGE_MAGIC
            int trueDamageFlg = CHAR_getWorkInt(BATTLE_No2Index(battleindex, attackNo), CHAR_WORK_DAMGEFLG);
...
#endif
            float Kmagic = ((float)att_magic_lv[FieldAttr] * 1.4 - Rmagic);
...
int analysis_profession_parameter(int attIdx, int skill, int toNo, int charaindex)
{
    ...
    // 取得技能参数
    if ((pszOption = PROFESSION_SKILL_getChar(skill, PROFESSION_SKILL_OPTION)) == "\0")
    {
        return -1;
    }
...
)
{
    ...
    //BATTLESTR_ADD( szCommand );
    {
        int _len = strlen(szCommand);
        (pszBattleTop + _len < (pszBattleLast) - 1) ?
        (memcpy(pszBattleTop, szCommand, _len), pszBattleTop += _len, pszBattleTop[0] = 0)
        : (0);
    }
    return TRUE;
}
...
#ifdef _TRUE_DAMAGE_MAGIC
void BATTLE_MultiAttMagicNew(int battleindex, int attackNo, int toNo,
                             int attIdx, int FieldAttr, int Power, int MagicLv, int acurate)
{
    //  printf("\nbattlemagicnew: %d %d %d %d %d",toNo,attIdx,Power,MagicLv,acurate);
    int  list[SIDE_OFFSET * 2 + 1];
    int  listidx, i, j, k, z;
    int  basex, basey, magicattidx, def_is_player[10], def_be_hit[10];
    int  att_magic_lv[4];
    ...
    for (i = 0; i < listidx; i++)
    {
        ...
        else    // 没闪过,计算攻击力
        {
            if (FieldAttr == 0)
            {
                attvalue = (int)(Power + Power * (RAND(0, MagicLv * 2) - MagicLv) / 100.0f);
            }
            else if (FieldAttr == 1)
            {
                int hp = CHAR_getWorkInt(charaidx, CHAR_WORKMAXHP);
                attvalue = (int)((Power + (RAND(0, MagicLv * 2) - MagicLv)) * hp / 100.0);
            }
            else if (FieldAttr == 2)
            {
                int hp = CHAR_getInt(charaidx, CHAR_HP);
                attvalue = (int)((Power + (RAND(0, MagicLv * 2) - MagicLv)) * hp / 100.0);
            }
            else if (FieldAttr == 3)
            {
                int hp = CHAR_getWorkInt(charaidx, CHAR_WORKMAXHP) - CHAR_getInt(charaidx, CHAR_HP);
                attvalue = (int)((Power + (RAND(0, MagicLv * 2) - MagicLv)) * hp / 100.0);
            }
        }
...
#ifdef _TRUE_DAMAGE_MAGIC
int MAGIC_AttMagicNew_Battle(int charaindex, int toNo, int marray, int mp)
{
    int  attno, attidx, battleindex;
    int  attr = -1, i, power;
    char aszattr[][32] = { "真", "比", "当", "撕" };
    char buf1[256];
    char *magicarg;
    int magiclv = 0;
    int acurate = 0;
    ...
    BATTLE_MultiAttMagicNew(battleindex, attno, toNo, attidx, attr, power, magiclv, acurate);
    return TRUE;
}
#endif
