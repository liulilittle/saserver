...
void ITEM_useImprecate(int charaindex, int toNo, int haveitemindex)
{
    ...
    for (i = 0; i < 3; i++)
    {
        if (strstr(arg, ImList[i].fun) != 0)
        {
            ...
            if (NPC_Util_GetStrFromStrWithDelim(arg, ImList[i].fun, buf, sizeof(buf)) == NULL)
            {
                continue;
            }
...
)
{
    ...
    Success = FreeuseStatusItem(charaindex, Success);
#endif
    battleindex = CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEINDEX);
    attackNo = BATTLE_Index2No(battleindex, charaindex);
...
void ITEM_useAddexp_Effect(int charaindex, int toindex, int haveitemindex)
{
    ...
    else if (CHAR_getWorkInt(toindex, CHAR_WORKITEM_ADDEXP) > power)
    {
        CHAR_talkToCli(charaindex, -1, "已经吃了更高倍数的智慧果", CHAR_COLORYELLOW);
        return;
    }
    ...
    sprintf(szBuffer, "A|%d|%d|%d|%d|%d|%d", getBattleexp() * 100, getBattleExpTime(), CHAR_getInt(toindex, CHAR_ADDEXPPOWER) + 100, CHAR_getInt(toindex, CHAR_ADDEXPTIME)
            , (CHAR_getInt(toindex, CHAR_ENEMY_MIN_BASE) + 1) * 100, CHAR_getInt(toindex, CHAR_ENEMY_MIN_TIME));
    lssproto_S_send(getfdFromCharaIndex(charaindex), szBuffer);
}
#endif
