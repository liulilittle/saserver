...
void BattleCommandDispach(int fd,   char *command)
{
    ...
    if (strncmp(command, "U", 1) == 0)
    {
        ...
        CHAR_setWorkInt(charaindex, CHAR_WORKBATTLEWAITCNT, 0);
        // shan end
    }
    else if (strncmp(command, "E", 1) == 0)
    {
        ...
        for (i = 0; i < 32; i++)
        {
            ...
            {
                CHAR_talkToCli(charaindex, -1,
                               "该地图无法逃跑哦 ~囧~ 留下来死战到底吧！", CHAR_COLORYELLOW);
                CHAR_setWorkInt(charaindex, CHAR_WORKBATTLECOM1, BATTLE_COM_GUARD);
                break;
            }
        }

        if (FreeBattleEscape(charaindex) == 0)
        {
            CHAR_setWorkInt(charaindex, CHAR_WORKBATTLECOM1, BATTLE_COM_GUARD);
            i = 0;
        }
        ...
        for (i = 0; i < 10; i++)
        {
            int eindex = BattleArray[battleindex].Side[1].Entry[i].charaindex;

            if (CHAR_CHECKINDEX(eindex))
            {
                if (CHAR_getWorkInt(charaindex, CHAR_WORK_CRAZYFLG) == 1)
                {
    ...
    else if (strncmp(command, "H|", 2) == 0)
    {
        ...
        int defindex = BATTLE_No2Index(battleindex, iNum);
    ...
    else if (strncmp(command, "S|", 2) == 0)
    {
        ...
        if (CHAR_CHECKINDEX(petindex) == 1)
        {
                    ...
                    || CHAR_getWorkInt(petindex, CHAR_WORKBATTLECOM1) == BATTLE_COM_S_EARTHROUND0)
            {
                ...
                iNum = -1;
                CHAR_setWorkInt(petindex, CHAR_WORKBATTLECOM1, BATTLE_COM_NONE);
            }
        }
...
)
{
    ...
    for (j = 0; j < 2; j ++)
    {
        ...
        for (i = 0; i < BATTLE_ENTRY_MAX; i ++)
        {
            ...
            if (CHAR_getInt(charaindex, CHAR_BASEIMAGENUMBER) >= 101424
                    && CHAR_getInt(charaindex, CHAR_BASEIMAGENUMBER) <= 101427
                    && CHAR_getInt(charaindex, CHAR_PETDATA) == 1
               )
            {
                if (CHAR_getWorkInt(charaindex, CHAR_WORKPETDATA) == 0)
                {
                    flg |= BC_FLG_WOLF;    // 狼人变身
                    CHAR_setWorkInt(charaindex, CHAR_WORKPETDATA, 1);
                }
                else if (CHAR_getWorkInt(charaindex, CHAR_WORKPETDATA) == 1)
                {
                    CHAR_setWorkInt(charaindex, CHAR_WORKPETDATA, 2);
                }
            }

#endif // _VARY_WOLF_JOINBATTLE
            ...
            if (CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPET)
            {
                if (CHAR_getInt(charaindex, CHAR_BASEIMAGENUMBER) == 101428)
                {
                    ...
                    if (strstr(CHAR_getChar(charaindex, CHAR_NAME), "格林巴肯") != NULL)
                    {
                        sprintf(tempshuxing, "%s[地]", CHAR_getChar(charaindex, CHAR_NAME));
                    }
                    else if (strstr(CHAR_getChar(charaindex, CHAR_NAME), "雷恩巴肯") != NULL)
                    {
                        sprintf(tempshuxing, "%s[水]", CHAR_getChar(charaindex, CHAR_NAME));
                    }
                    else if (strstr(CHAR_getChar(charaindex, CHAR_NAME), "法亚巴肯") != NULL)
                    {
                        sprintf(tempshuxing, "%s[火]", CHAR_getChar(charaindex, CHAR_NAME));
                    }
                    else if (strstr(CHAR_getChar(charaindex, CHAR_NAME), "奇欧巴肯") != NULL)
                    {
                        sprintf(tempshuxing, "%s[风]", CHAR_getChar(charaindex, CHAR_NAME));
                    }

                    makeEscapeString(tempshuxing,
                                     szEscapeName,
                                     sizeof(szEscapeName));
                }
                else
...
// Robin 0728 ride Pet
            ...
            if (petindex != -1)
            {
                ...
                if (CHAR_getInt(charaindex, CHAR_RIDENAMEFLG) == 1)
                    makeEscapeString(CHAR_getChar(petindex, CHAR_NAME),
                                     szEscapePetName,
...
#ifdef _BATTLE_HPSHOW

                if (petmaxhp > 0)
                {
                    pethp = (pethp / petmaxhp) * 100;
                    petmaxhp = 100;
                }

#endif // _BATTLE_HPSHOW
            }
            else
            {
...
#ifdef _BATTLE_HPSHOW
            int playermaxhp = CHAR_getWorkInt(charaindex, CHAR_WORKMAXHP);
            int playerhp = max(CHAR_getInt(charaindex, CHAR_HP), 0);
...
#endif // _BATTLE_HPSHOW
            int imageno = CHAR_getInt(charaindex, CHAR_BASEIMAGENUMBER);
#ifdef _VARY_WOLF_JOINBATTLE

            if (CHAR_getInt(charaindex, CHAR_BASEIMAGENUMBER) >= 101424
                    && CHAR_getInt(charaindex, CHAR_BASEIMAGENUMBER) <= 101427
                    && CHAR_getInt(charaindex, CHAR_PETDATA) == 1
                    && CHAR_getWorkInt(charaindex, CHAR_WORKPETDATA) == 2
               )
            {
                imageno = 109041;       // 人狼变身
            }
            ...
            sprintf(szBuffer, "%X|%s|%s|%X|%X|%X|%X|%X|%X|%s|%X|%X|%X|"
                    ...
                    petmaxhp
#ifdef _BATTLE_EFFECTS
                    , CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEEFFECTS) > 0 ? CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEEFFECTS) : CHAR_getInt(charaindex, CHAR_BATTLEEFFECTS2)
                    , CHAR_getInt(charaindex, CHAR_BATTLEEFFECTS)
#endif //#ifdef _BATTLE_EFFECTS
                   );
            STRCPY_TAIL(pszTop, pszLast, szBuffer);
...
void BATTLE_CharSendAll(int battleindex)
{
    ...
    for (j = 0; j < 2; j ++)
    {
        for (i = 0; i < BATTLE_ENTRY_MAX; i ++)
        {
            ...
            else if (BattleArray[battleindex].Side[1 - j].flg & BSIDE_FLG_SURPRISE)
            {
...
void BattleEncountOut(int charaindex)
{
    ...
    else if (CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEMODE) != BATTLE_MODE_NONE)
    {
        return;
    }
