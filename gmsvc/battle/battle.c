...
static int CharTableIdx[20][2] =
    {
        {3, 2}, {3, 1}, {3, 3}, {3, 0}, {3, 4}, {2, 2}, {2, 1}, {2, 3}, {2, 0}, {2, 4}, {0, 2}, {0, 1}, {0, 3}, {0, 0}, {0, 4}, {1, 2}, {1, 1}, {1, 3}, {1, 0}, {1, 4}
...
BOOL BATTLE_IsThrowWepon(int itemindex)
{
    ...
    if (itemtype == ITEM_BOOMERANG     //回旋镖
        || itemtype == ITEM_BOW        //弓箭
        || itemtype == ITEM_BREAKTHROW //投掷石
        || itemtype == ITEM_BOUNDTHROW //投掷斧头
    )
    {
        return TRUE;
    }
...
BOOL BATTLE_IsCharge(int charaindex)
{
    ...
    if (com == BATTLE_COM_S_CHARGE || com == BATTLE_COM_S_EARTHROUND1 || com == BATTLE_COM_S_EARTHROUND0
...
void BATTLE_AllCharaWatchWaitSet(int battleindex)
{
    ...
    for (i = 0; i < BATTLE_ENTRY_MAX; i++)
    {
        ...
        if (CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEMODE) != BATTLE_CHARMODE_WATCHINIT)
        {
            CHAR_setWorkInt(charaindex, CHAR_WORKBATTLEMODE,
                            BATTLE_CHARMODE_C_WAIT);
        }
...
int BATTLE_CharaBackUp(int battleindex)
{
    ...
    for (j = 0; j < 2; j++)
    {
        ...
        for (i = 0; i < BATTLE_ENTRY_MAX / 2; i++)
        {
            ...
            if (CHAR_CHECKINDEX(charaindex) == TRUE && CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
            {
    ...
    int side)
{
    ...
    if (BattleJoin(charaindex, battleindex) == 0)
    {
        return BATTLE_ERR_TYPE;
    }
    ...
    CHAR_setWorkInt(charaindex, CHAR_WORKPETDATA, 0);
#endif
    ...
    CHAR_setWorkInt(charaindex, CHAR_WORKTEMPTATION, 0); //水漾诱惑
#endif
    return 0;
}
...
INLINE int _BATTLE_Exit(char *file, int line, int charaindex, int battleindex)
{
    ...
    if (CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
    {
        CHAR_setWorkInt(charaindex, CHAR_WORKBATTLEFINISHTIME, time(NULL));
        ...
        for (i = 0; i < 5; i++)
        {
            int mypetindex = CHAR_getCharPet(charaindex, i);

            if (CHAR_CHECKINDEX(mypetindex))
            {
                CHAR_setWorkInt(mypetindex, CHAR_WORKBATTLEDIETYPE, 0);
            }
        }
        ...
        {
            if (BattleArray[battleindex].topbattleindex > -1)
            {
                int temppaitype = BattleArray[BattleArray[battleindex].topbattleindex].paitype;

                if (temppaitype > 0)
                {
                    int zhaotype = 0;

                    for (i = 0; i < temppaitype; i++)
                    {
                        if (zhaotype == 0)
                        {
                            if (CHAR_CHECKINDEX(BattleArray[BattleArray[battleindex].topbattleindex].paiindex[i]) && charaindex == BattleArray[BattleArray[battleindex].topbattleindex].paiindex[i])
                            {
                                BattleArray[BattleArray[battleindex].topbattleindex].paitype--;
                                zhaotype = 1;
                            }
                        }
                        ...
                        if (zhaotype == 1)
                        {
                            BattleArray[BattleArray[battleindex].topbattleindex].paiindex[i] = BattleArray[BattleArray[battleindex].topbattleindex].paiindex[i + 1];
                            int tempfd = getfdFromCharaIndex(BattleArray[BattleArray[battleindex].topbattleindex].paiindex[i]);

                            if (tempfd != -1)
                            {
                                lssproto_BattlePaiDui_send(tempfd, i + 1);
                            }
                        }

                        if (BattleArray[BattleArray[battleindex].topbattleindex].paiindex[i] == -1)
                        {
                            break;
                        }
                    }
        ...
        lssproto_KS_send(getfdFromCharaIndex(charaindex), petarray, TRUE);
    }
    ...
    if (CHAR_getInt(charaindex, CHAR_BASEIMAGENUMBER) == 101749 || CHAR_getWorkInt(charaindex, CHAR_WORKFOXROUND) != -1) //变成小狐狸
    {
    ...
    if (CHAR_getInt(charaindex, CHAR_BECOMEPIG) > -1 && CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
    {
    ...
    for (j = 0; j < 2; j++)
    {
        ...
        for (i = 0; i < BATTLE_ENTRY_MAX; i++)
        {
            ...
            else if (CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYERNPC || CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYERPETNPC)
            {
                CHAR_setWorkInt(charaindex, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_NONE);
                CHAR_setWorkInt(charaindex, CHAR_WORKBATTLEINDEX, -1);
                {
                    if (CHAR_getInt(charaindex, CHAR_HP) < CHAR_getWorkInt(charaindex, CHAR_WORKMAXHP) * 0.8)
                    {
                    ...
                    for (i = 0; i < CHAR_MAXPETHAVE; i++)
                    {
                        ...
                        if (CHAR_getInt(petindex, CHAR_HP) < CHAR_getWorkInt(petindex, CHAR_WORKMAXHP) * 0.8)
                        {
            ...
            else if (CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
            {
                ...
                for (k = 0; k < CHAR_MAXPETHAVE; k++)
                {
                    ...
                    if (CHAR_getFlg(petindex, CHAR_ISDIE) == TRUE || CHAR_getInt(petindex, CHAR_HP) <= 0)
                    {
                        CHAR_setFlg(petindex, CHAR_ISDIE, 0);
                ...
                if (BattleArray[battleindex].type == BATTLE_TYPE_P_vs_P)
                {
                    ...
                    if (CHAR_getFlg(charaindex, CHAR_ISEFFECT))
                    {
                        flg |= CHAR_FS_EFFECT;
                    }

                    if (CHAR_getFlg(charaindex, CHAR_ISPLAYER))
                    {
                        flg |= CHAR_FS_PLAYER;
                    }

                    if (CHAR_getFlg(charaindex, CHAR_ISTEAMFULL))
                    {
                        flg |= CHAR_FS_TEAMFULL;
                    }

                    if (CHAR_getFlg(charaindex, CHAR_ISSHOWPET))
                    {
                        flg |= CHAR_FS_SHOWPET;
                    }

                    if (CHAR_getFlg(charaindex, CHAR_ISAUTOPARTY))
                    {
                        flg |= CHAR_FS_AUTOPARTY;
                    }

                    if (CHAR_getFlg(charaindex, CHAR_ISALLWORLD))
                    {
                        flg |= CHAR_FS_ALLWORLD;
                    }

                    if (CHAR_getFlg(charaindex, CHAR_ISDRAWEFFECT))
                    {
        ...
        if (CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
        {
            if (BattleArray[battleindex].type != BATTLE_TYPE_WATCH)
            {
                ...
                )
                {
                    ...
                    CHAR_talkToCli(charaindex, -1, "当前地图设置有战后自动治疗功能 满血满状态原地恢复哦！o(∩_∩)o ", CHAR_COLORYELLOW);
            ...
            if ((BattleArray[battleindex].type == BATTLE_TYPE_P_vs_P || (CHAR_getInt(charaindex, CHAR_FLOOR) >= 40000 && CHAR_getInt(charaindex, CHAR_FLOOR) <= 40034)) && CHAR_getWorkInt(charaindex, CHAR_WORK_OFFLINE) != 0)
            {
    ...
    int side)
{
        ...
        CHAR_getInt(pindex, CHAR_HP) > 0)
    {
        ...
        CHAR_setWorkInt(pindex, CHAR_WORKBATTLECOM1, 0); //修复宠物第一回合不动
    }
    else
    {
#ifdef _PLAYER_NPC
...
int BATTLE_RescueEntry(int charaindex, int toindex)
{
    ...
    if (CHAR_getWorkInt(toindex, CHAR_WORKBATTLEMODE) == BATTLE_CHARMODE_FINAL || CHAR_getWorkInt(toindex, CHAR_WORKBATTLEINDEX) == -1)
    {
        iRet = 1;
    }
    ...
    if (iRet == 0)
    {
        ...
        flg = (BattleArray[battleindex].Side[CHAR_getWorkInt(toindex, CHAR_WORKBATTLESIDE)].flg & BSIDE_FLG_HELP_OK) ? TRUE : FALSE;
    ...
    else
    {
        ...
        if (iRet == 4)
        {
            if (BattleArray[battleindex].paitype > -1)
            {
                if (BattleArray[battleindex].paitype >= 99)
                {
                    BattleArray[battleindex].paitype = 99;
                }
                else
                {
                    BattleArray[battleindex].paiindex[BattleArray[battleindex].paitype] = charaindex;
                    BattleArray[battleindex].paitype++;
                    BATTLE_PaiDuiWatchEntry(charaindex, toindex);
                }
            }
        }
...
BOOL BATTLE_RescueTry(int charaindex)
{
         ...
         object = NEXT_OBJECT(object))
    {
        ...
        if (BattleArray[CHAR_getWorkInt(toindex, CHAR_WORKBATTLEINDEX)].type != BATTLE_TYPE_P_vs_E)
        {
            continue;
        }

        if (!(BattleArray[CHAR_getWorkInt(toindex, CHAR_WORKBATTLEINDEX)].Side[CHAR_getWorkInt(toindex, CHAR_WORKBATTLESIDE)].flg & BSIDE_FLG_HELP_OK))
        {
            continue;
        }
    ...
    else if (cnt > 1)
    {
        ...
        for (i = 0;
             CONNECT_getBattlecharaindex(fd, i) != -1 && i < CONNECT_WINDOWBUFSIZE; i++)
        {
    ...
    int toindex)
{
    ...
    if (CHAR_getWorkInt(toindex, CHAR_WORKBATTLEMODE) == BATTLE_CHARMODE_FINAL || CHAR_getWorkInt(toindex, CHAR_WORKBATTLEMODE) == BATTLE_CHARMODE_NONE || CHAR_getWorkInt(toindex, CHAR_WORKBATTLEINDEX) == -1)
    {
        iRet = 1;
    }
    ...
    if (iRet == 0)
    {
        ...
        /*if (BattleArray[battleindex].type == BATTLE_TYPE_P_vs_P)
        {
            ...
            for (j = 0; j < 2; j++)
            {
                for (i = 0; i < 5; i++)
                {
                    battlecharaindex = BattleArray[battleindex].Side[j].Entry[i].charaindex;

                    if (CHAR_CHECKINDEX(battlecharaindex))
                    {
                        if (CHAR_getInt(battlecharaindex, CHAR_RIDENAMEFLG) == 1)
                        {
                            int ridehaveid = CHAR_getInt(battlecharaindex, CHAR_RIDEPET);
                            int ridepetindex = CHAR_getCharPet(battlecharaindex, ridehaveid);

                            if (CHAR_CHECKINDEX(ridepetindex))
                            {
                                char ridebufftmp[128];
                                sprintf(ridebufftmp, " < %s > 使用特殊皮肤改变造型，提示真实骑宠为 < %s >", CHAR_getChar(battlecharaindex, CHAR_NAME), CHAR_getChar(ridepetindex, CHAR_NAME));
                                CHAR_talkToCli(charaindex, -1, ridebufftmp, CHAR_COLORYELLOW);
                            }
                        }

                        int attpetid = CHAR_getInt(battlecharaindex, CHAR_DEFAULTPET);
                        int attpetindex = CHAR_getCharPet(battlecharaindex, attpetid);

                        if (CHAR_CHECKINDEX(attpetindex))
                        {
                            if (CHAR_getInt(attpetindex, CHAR_BASEIMAGENUMBER) == 101428)
                            {
                                char tempshuxing[16];
                                memset(tempshuxing, 0, sizeof(tempshuxing));

                                if (strstr(CHAR_getChar(attpetindex, CHAR_NAME), "格林巴肯") != NULL)
                                {
                                    sprintf(tempshuxing, "[地]");
                                }
                                else if (strstr(CHAR_getChar(attpetindex, CHAR_NAME), "雷恩巴肯") != NULL)
                                {
                                    sprintf(tempshuxing, "[水]");
                                }
                                else if (strstr(CHAR_getChar(attpetindex, CHAR_NAME), "法亚巴肯") != NULL)
                                {
                                    sprintf(tempshuxing, "[火]");
                                }
                                else if (strstr(CHAR_getChar(attpetindex, CHAR_NAME), "奇欧巴肯") != NULL)
                                {
                                    sprintf(tempshuxing, "[风]");
                                }

                                char ridebufftmp[128];
                                sprintf(ridebufftmp, " < %s > 的人狼使用人狼变身改变造型，提示名字为 < %s%s >", CHAR_getChar(battlecharaindex, CHAR_NAME), CHAR_getChar(attpetindex, CHAR_NAME), tempshuxing);
                                CHAR_talkToCli(charaindex, -1, ridebufftmp, CHAR_COLORYELLOW);
                            }
                        }
                    }
    ...
    else
    {
        if (iRet != 2)
        {
            if (CHAR_getInt(toindex, CHAR_FLOOR) != 2005)
            {
                snprintf(szBuffer, sizeof(szBuffer), "观看的战斗已经结束，观战失败，快速战斗请勿观战，可能引起客户端卡死。");
                CHAR_talkToCli(charaindex, -1, szBuffer, RAND(0, CHAR_COLORNUM - 2));
            }
        }
        else if (iRet == 2)
        {
...
#ifdef _BATTLE_PAIDUI
int BATTLE_PaiDuiWatchEntry(
    int charaindex,
    int toindex)
{
    if (BATTLE_WatchEntry(charaindex, toindex) == 0)
    {
        int battleindex = CHAR_getWorkInt(toindex, CHAR_WORKBATTLEINDEX);
        int fd;

        if ((fd = getfdFromCharaIndex(charaindex)) != -1)
        {
            lssproto_BattlePaiDui_send(fd, BattleArray[battleindex].paitype);
        }
    }
}
#endif
...
BOOL BATTLE_WatchTry(int charaindex)
{
    ...
    if (FreeLookBattleFlag(charaindex) == 1)
    {
        return FALSE;
    }
...
int *Doujyou_GetEnemy(int meindex, int charaindex)
{
    ...
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "enemypetno", buf, sizeof(buf)) == NULL)
    {
        // 中卅井匀凶日窒手踏井卅中
        DoujyouEnemyTbl[1] = -1;
    }
...
int BATTLE_CreateVsEnemyNew(int charaindex, int npcindex, int *table)
{
    ...
    BattleArray[battleindex].x = CHAR_getInt(charaindex, CHAR_X);
    BattleArray[battleindex].y = CHAR_getInt(charaindex, CHAR_Y);
...
int BATTLE_CreateVsEnemy(int charaindex, int mode, int npcindex)
{
    ...
    if (CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
    {
        if (CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEMODE) == BATTLE_CHARMODE_NONE)
        {
            if (CharVsEnemyFunction(charaindex) == TRUE)
            {
                return 0;
            }
        }
...
int BATTLE_CreateVsPlayer(int charaindex0, int charaindex1)
{
    ...
    /*#ifdef _BATTLE_OVER_STAR_TIME
        if( CHAR_getWorkInt( charaindex0, CHAR_WORKDBATTLETIME ) + 60 > time(NULL)){
            char token[256];
            sprintf(token, "你当前需冷却%d后才能进行PK", CHAR_getWorkInt( charaindex0, CHAR_WORKDBATTLETIME ) + 60 - time(NULL));
            CHAR_talkToCli( charaindex0, -1, token, CHAR_COLORYELLOW );
            return BATTLE_ERR_ALREADYBATTLE;
        }
        if( CHAR_getWorkInt( charaindex1, CHAR_WORKDBATTLETIME ) + 60 > time(NULL)){
            char token[256];
            sprintf(token, "对方需冷却%d后才能进行PK", CHAR_getWorkInt( charaindex0, CHAR_WORKDBATTLETIME ) + 60 - time(NULL));
            CHAR_talkToCli( charaindex0, -1, token, CHAR_COLORYELLOW );
            return BATTLE_ERR_ALREADYBATTLE;
        }
    #endif*/
    ...
    for (j = 0; j < 2; j++)
    {
        ...
        else if (CHAR_getWorkInt(charaindex[j], CHAR_WORKPARTYMODE) == CHAR_PARTY_CLIENT)
        {
    ...
    for (j = 0; j < 2; j++)
    {
        if (CHAR_getWorkInt(charaindex[j], CHAR_WORKPARTYMODE) == CHAR_PARTY_LEADER)
        {
            for (jj = 0; jj < 5; jj++)
            {
                if (CHAR_CHECKINDEX(CHAR_getWorkInt(charaindex[j], CHAR_WORKPARTYINDEX1 + jj)))
                {
                    partynum[j]++;
                    CHAR_setWorkInt(CHAR_getWorkInt(charaindex[j], CHAR_WORKPARTYINDEX1 + jj), CHAR_WORKFMBATTLETYPE, 0);
                }
            }
        }
        else
        {
            ...
            {
                int partyindex = CHAR_getWorkInt(charaindex[j], CHAR_WORKPARTYINDEX1);

                for (jj = 0; jj < 5; jj++)
                {
                    if (CHAR_CHECKINDEX(CHAR_getWorkInt(partyindex, CHAR_WORKPARTYINDEX1 + jj)))
                    {
                        partynum[j]++;
                        CHAR_setWorkInt(CHAR_getWorkInt(partyindex, CHAR_WORKPARTYINDEX1 + jj), CHAR_WORKFMBATTLETYPE, 0);
                    }
                }
            }
            else
            {
                partynum[j] = 1;
                CHAR_setWorkInt(charaindex[j], CHAR_WORKFMBATTLETYPE, 0);
            }
        }
    }

    if (partynum[0] > partynum[1] + 1 || partynum[0] + 1 < partynum[1])
    {
        for (j = 0; j < 2; j++)
        {
            if (CHAR_getWorkInt(charaindex[j], CHAR_WORKPARTYMODE) == CHAR_PARTY_LEADER)
            {
                for (jj = 0; jj < 5; jj++)
                {
                    if (CHAR_CHECKINDEX(CHAR_getWorkInt(charaindex[j], CHAR_WORKPARTYINDEX1 + jj)))
                    {
                        CHAR_setWorkInt(CHAR_getWorkInt(charaindex[j], CHAR_WORKPARTYINDEX1 + jj), CHAR_WORKFMBATTLETYPE, 1);
                    }
                }
            }
            else
            {
                if (CHAR_getWorkInt(charaindex[j], CHAR_WORKPARTYMODE) == CHAR_PARTY_CLIENT)
                {
                    ...
                    for (jj = 0; jj < 5; jj++)
                    {
                        if (CHAR_CHECKINDEX(CHAR_getWorkInt(partyindex, CHAR_WORKPARTYINDEX1 + jj)))
                        {
                            CHAR_setWorkInt(CHAR_getWorkInt(partyindex, CHAR_WORKPARTYINDEX1 + jj), CHAR_WORKFMBATTLETYPE, 1);
                        }
                    }
                }
                else
                {
                    CHAR_setWorkInt(charaindex[j], CHAR_WORKFMBATTLETYPE, 1);
                }
            }
        }
    }
...
BATTLE_CreateVsPlayer_End:;
    ...
    else
    {
        ...
        if (CHAR_getInt(charaindex[0], CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
        {
            if (CHAR_getInt(charaindex[0], CHAR_FLOOR) == 1042 || CHAR_getInt(charaindex[0], CHAR_FLOOR) == 2032 || CHAR_getInt(charaindex[0], CHAR_FLOOR) == 3032 || CHAR_getInt(charaindex[0], CHAR_FLOOR) == 4032)
            {
                int fmid = CHAR_getInt(charaindex[0], CHAR_FLOOR) / 1000;
                damagebase = sasql_queryFmPointData(fmid, 0);

                if (damagebase > 0 && (NowTime.tv_sec - damagebase) / 86400 >= 7)
                {
                    damagebase = (NowTime.tv_sec - damagebase) / 86400 - 2;
...
#ifdef _FMPOINT_DAMAGE2
                    damagebase = damagebase / 2;
#endif // _FMPOINT_DAMAGE2
                }
                else
                {
                    damagebase = 0;
        ...
        {
            char damagebuff[256];
            sprintf(damagebuff, "此次族战开启BUFF，伤害加成为 < %d％ >", damagebase);
            CHAR_talkToCliAndParty(charaindex[0], -1, damagebuff, CHAR_COLORYELLOW);
            CHAR_talkToCliAndParty(charaindex[1], -1, damagebuff, CHAR_COLORYELLOW);
        }

        int battleplayernum = 2;
        ...
        for (j = 0; j < 2; j++)
        {
            for (i = 1; i < getPartyNum(charaindex[j]); i++)
            {
                ...
                {
                    battleplayernum = battleplayernum + 1;
                }
            }
        }
        ...
        for (j = 0; j < 2; j++)
        {
            ...
            /*if (CHAR_getInt(charaindex[j], CHAR_RIDENAMEFLG) == 1)
            {
                int ridehaveid = CHAR_getInt(charaindex[j], CHAR_RIDEPET);
                int ridepetindex = CHAR_getCharPet(charaindex[j], ridehaveid);

                if (CHAR_CHECKINDEX(ridepetindex))
                {
                    char ridebufftmp[128];
                    sprintf(ridebufftmp, " < %s > 使用特殊皮肤改变造型，提示真实骑宠为 < %s >", CHAR_getChar(charaindex[j], CHAR_NAME), CHAR_getChar(ridepetindex, CHAR_NAME));
                    CHAR_talkToCliAndParty(charaindex[0], -1, ridebufftmp, CHAR_COLORYELLOW);
                    CHAR_talkToCliAndParty(charaindex[1], -1, ridebufftmp, CHAR_COLORYELLOW);
                }
            }

            int attpetid = CHAR_getInt(charaindex[j], CHAR_DEFAULTPET);
            int attpetindex = CHAR_getCharPet(charaindex[j], attpetid);
            ...
            if (CHAR_CHECKINDEX(attpetindex))
            {
                if (CHAR_getInt(attpetindex, CHAR_BASEIMAGENUMBER) == 101428)
                {
                    ...
                    char ridebufftmp[128];
                    sprintf(ridebufftmp, " < %s > 的人狼使用人狼变身改变造型，提示名字为 < %s%s >", CHAR_getChar(charaindex[j], CHAR_NAME), CHAR_getChar(attpetindex, CHAR_NAME), tempshuxing);
                    CHAR_talkToCliAndParty(charaindex[0], -1, ridebufftmp, CHAR_COLORYELLOW);
                    CHAR_talkToCliAndParty(charaindex[1], -1, ridebufftmp, CHAR_COLORYELLOW);
                }
            }*/
            ...
            for (i = 1; i < getPartyNum(charaindex[j]); i++)
            {
                ...
                CHAR_sendBattleEffect(pindex, ON);
                /*if (CHAR_getInt(pindex, CHAR_RIDENAMEFLG) == 1)
                {
                    int ridehaveid = CHAR_getInt(pindex, CHAR_RIDEPET);
                    int ridepetindex = CHAR_getCharPet(pindex, ridehaveid);

                    if (CHAR_CHECKINDEX(ridepetindex))
                    {
                        char ridebufftmp[128];
                        sprintf(ridebufftmp, " < %s > 使用特殊皮肤改变造型，提示真实骑宠为 < %s >", CHAR_getChar(pindex, CHAR_NAME), CHAR_getChar(ridepetindex, CHAR_NAME));
                        CHAR_talkToCliAndParty(charaindex[0], -1, ridebufftmp, CHAR_COLORYELLOW);
                        CHAR_talkToCliAndParty(charaindex[1], -1, ridebufftmp, CHAR_COLORYELLOW);
                    }
                }

                int attpetid = CHAR_getInt(pindex, CHAR_DEFAULTPET);
                int attpetindex = CHAR_getCharPet(pindex, attpetid);
                ...
                if (CHAR_CHECKINDEX(attpetindex))
                {
                    if (CHAR_getInt(attpetindex, CHAR_BASEIMAGENUMBER) == 101428)
                    {
                        ...
                        char ridebufftmp[128];
                        sprintf(ridebufftmp, " < %s > 的人狼使用人狼变身改变造型，提示名字为 < %s%s >", CHAR_getChar(pindex, CHAR_NAME), CHAR_getChar(attpetindex, CHAR_NAME), tempshuxing);
                        CHAR_talkToCliAndParty(charaindex[0], -1, ridebufftmp, CHAR_COLORYELLOW);
                        CHAR_talkToCliAndParty(charaindex[1], -1, ridebufftmp, CHAR_COLORYELLOW);
                    }
                }*/
...
//*********************************************************
{
    ...
    {
        BattleArray[battleindex].topbattleindex = BattleArray[topbattleindex].topbattleindex;
    }
...
void BATTLE_PreCommandSeq(int battleindex)
{
    ...
    for (j = 0; j < 2; j++)
    {
        ...
        for (i = 0; i < BATTLE_ENTRY_MAX; i++)
        {
            ...
            if (CHAR_getWorkInt(charaindex, CHAR_WORKTEMPTATION) == 1)
            {
                CHAR_setWorkInt(charaindex, CHAR_WORKTEMPTATION, 0);
            }
...
static BOOL BATTLE_CommandWait(int battleindex, int side)
{
    ...
    for (i = 0; i < BATTLE_ENTRY_MAX; i++)
    {
        ...
        if (CHAR_getFlg(charaindex, CHAR_ISDIE) == TRUE)
        {
            ...
            if (CHAR_getWorkInt(charaindex, CHAR_WORK_OFFLINE) == 1 && (BattleArray[battleindex].type == BATTLE_TYPE_P_vs_P || (CHAR_getInt(charaindex, CHAR_FLOOR) >= 40000 && CHAR_getInt(charaindex, CHAR_FLOOR) <= 40034)))
            {
            ...
            else if (CHAR_getWorkInt(charaindex, CHAR_WORK_OFFLINE) == 1)
            {
...
#ifdef _OFFLINE_BATTLE_RETEUN
                && BattleArray[battleindex].type != BATTLE_TYPE_P_vs_P && (CHAR_getInt(charaindex, CHAR_FLOOR) < 40000 || CHAR_getInt(charaindex, CHAR_FLOOR) > 40034)
#endif
#ifdef _OFFLINE_RETEUN
            || CHAR_getWorkInt(charaindex, CHAR_WORK_OFFLINE) == 2 || CHAR_getWorkInt(charaindex, CHAR_WORK_OFFLINE) == 3
#endif
#ifdef _NEW_OFFLINEBATTLERETURN
            || CHAR_getWorkInt(charaindex, CHAR_OFFLINEBATTLETIME) == -1
#endif
        )
        {
            OffLineCommand(battleindex, charaindex, side);
            ...
            if (CHAR_getWorkInt(charaindex, CHAR_OFFLINEBATTLETIME) == -1)
            {
                CHAR_setWorkInt(charaindex, CHAR_OFFLINEBATTLETIME, 0);
            }
...
#endif
        FreeBattleCommand(battleindex, charaindex, side);
        ...
        case BATTLE_CHARMODE_C_WAIT:
        {
            ...
            if (TimeOut == TRUE)
            {
                ...
                */
                ...
                if (CHAR_getWorkInt(charaindex, CHAR_WORK_OFFLINE) == 1 && (BattleArray[battleindex].type == BATTLE_TYPE_P_vs_P || (CHAR_getInt(charaindex, CHAR_FLOOR) >= 40000 && CHAR_getInt(charaindex, CHAR_FLOOR) <= 40034)))
                {
                    if (CHAR_getWorkInt(charaindex, CHAR_OFFLINEBATTLENUM) > 3)
                    {
                        CHAR_DischargeParty(charaindex, 0); //解散团队
                        CHAR_setWorkInt(charaindex, CHAR_WORKFMPKFLAG, -1);
                        CHAR_setWorkInt(charaindex, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
                    ...
                    else
                    {
                        CHAR_setWorkInt(charaindex, CHAR_WORKBATTLECOM1, BATTLE_COM_NONE);
                        CHAR_setWorkInt(charaindex, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
                        CHAR_setWorkInt(charaindex, CHAR_OFFLINEBATTLENUM, CHAR_getWorkInt(charaindex, CHAR_OFFLINEBATTLENUM) + 1);
                    }
                }
                else
                {
                    ...
                    CHAR_setWorkInt(charaindex, CHAR_WORKBATTLECOM1, BATTLE_COM_NONE);
                    CHAR_setWorkInt(charaindex, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
                    CHAR_setWorkInt(charaindex, CHAR_WORKBATTLEWAITCNT, CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEWAITCNT) + 1);
#ifdef _OFFLINE_SYSTEM
#ifdef _OFFLINE_BATTLE_RETEUN
                }
...
#endif
{
    ...
    {
#ifdef _ALLBLUES_LUA_1_9
        // FreeCharExpSave(charaindex, getexp);  //龙写的经验丹LUA，暂时不用，注释掉
#endif
        getexp = 0;
    }
    ...
    itemaddexp = FreeModeExp(charaindex, itemgetexp, modexp);
#else
        ...
        for (i = 0; i < CHAR_EQUIPPLACENUM; i++)
        {
            ...
            if (ITEM_CHECKINDEX(id))
            {
                ...
                if ((P = strstr(arg, "EXPUP")))
                {
                    ...
                    {
                        if (CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPET && (CHAR_getInt(midx, CHAR_RIDEPET) != charaindex))
                        {
                            addexp += getexp * atoi(P += 2) * 0.01;
                        }
                    }
    ...
    if (CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPET)
    {
        ...
        if (CHAR_getInt(ownerindex, CHAR_FLOOR) == getNoFameMap())
        {
            CHAR_setInt(charaindex, CHAR_NOFAME, 1);
        }
    }
    else if (CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
    {
    ...
    if (CHAR_getWorkInt(midx, CHAR_WORK_OFFLINE) == 1)
    {
        CHAR_setInt(charaindex, CHAR_OFFLINE_EXP, CHAR_getInt(charaindex, CHAR_OFFLINE_EXP) + addexp);
    }
...
)
{
    ...
    if (CHAR_getInt(charaindex, CHAR_WHICHTYPE) != CHAR_TYPEPLAYER)
    {
        ...
        if (CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYERNPC || CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYERPETNPC)
        {
    ...
    if (CHAR_getFlg(charaindex, CHAR_ISDIE) == FALSE)
    {
        BattleFinishFunction(charaindex, BattleArray[battleindex].CreateTime, BattleArray[battleindex].turn, 1, "", "", "", "");
#ifdef _ITEM_ADDEQUIPEXP
        BATTLE_GetExp(charaindex, charaindex);
#else
        BATTLE_GetExp(charaindex);
...
static int BATTLE_Finish(int battleindex)
{
    ...
    if (BattleArray[battleindex].winside == -1 && BattleArray[battleindex].type == BATTLE_TYPE_P_vs_E && BattleArray[battleindex].WinFunc != NULL)
    {
    ...
    if (BattleArray[battleindex].type == BATTLE_TYPE_P_vs_P)
    {
        ...
        for (i = 0; i < BATTLE_ENTRY_MAX; i++)
        {
            ...
            if (CHAR_CHECKINDEX(winindex) == TRUE)
            {
                ...
                if (BattleArray[battleindex].winside == -1)
                {
                    BattleFinishFunction(winindex, BattleArray[battleindex].CreateTime, BattleArray[battleindex].turn, 1, BattleArray[battleindex].leadercdkey, BattleArray[battleindex].leadername, BattleArray[battleindex].rivalcdkey, BattleArray[battleindex].rivalname);
                }
                else
                {
                    BattleFinishFunction(winindex, BattleArray[battleindex].CreateTime, BattleArray[battleindex].turn, 1, BattleArray[battleindex].rivalcdkey, BattleArray[battleindex].rivalname, BattleArray[battleindex].leadercdkey, BattleArray[battleindex].leadername);
                }
            ...
            if (CHAR_CHECKINDEX(lostindex) == TRUE)
            {
                ...
                if (BattleArray[battleindex].winside == -1)
                {
                    BattleFinishFunction(lostindex, BattleArray[battleindex].CreateTime, BattleArray[battleindex].turn, 0, BattleArray[battleindex].leadercdkey, BattleArray[battleindex].leadername, BattleArray[battleindex].rivalcdkey, BattleArray[battleindex].rivalname);
                }
                else
                {
                    BattleFinishFunction(lostindex, BattleArray[battleindex].CreateTime, BattleArray[battleindex].turn, 0, BattleArray[battleindex].rivalcdkey, BattleArray[battleindex].rivalname, BattleArray[battleindex].leadercdkey, BattleArray[battleindex].leadername);
                }
...
static BOOL BATTLE_TimeOutCheck(int battleindex)
{
    ...
    for (j = 0; j < 2; j++)
    {
        ...
        for (i = 0; i < BATTLE_ENTRY_MAX; i++)
        {
            ...
            if (CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEMODE) == BATTLE_CHARMODE_C_WAIT) //等待指令当中
            {
                BATTLE_Exit(charaindex, battleindex); //
                CHAR_DischargePartyNoMsg(charaindex);
                CHAR_talkToCli(charaindex, -1,
...
//              BattleCommand( charaindex, battleindex );
    ...
    for (j = 0; j < 2; j++)
    {
        for (i = 0; i < BATTLE_ENTRY_MAX / 2; i++)
        {
            ...
            if (CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
            {
                int petNo = CHAR_getInt(charaindex, CHAR_DEFAULTPET);

                if (petNo >= 0)
                {
                    int petindex = CHAR_getCharPet(charaindex, petNo);

                    if (CHAR_getWorkInt(petindex, CHAR_WORKBATTLEDIETYPE) == 1 && CHAR_getFlg(petindex, CHAR_ISDIE) == FALSE)
                    {
                        CHAR_setWorkInt(petindex, CHAR_WORKBATTLEDIETYPE, 0);
                    }
                }
            }
        }
...
int BATTLE_Loop(void)
{
    ...
    for (i = 0; i < BATTLE_battlenum; i++) //最多战斗场数
    {
        ...
        if (getBattleTime() > 0 && time_diff_us(NowTime, old) < getBattleTime() * 1000.0)
        {
            continue;
        }
...
//************************************************************
{
    ...
    case BATTLE_COM_S_ATTACK_MAGIC:
        ...
        dex = work - RAND(0, work * getBattleRandDex() / 100 + getBattleRandDex2());
#else
        dex = work - RAND(0, 15);
#endif
        break;
    ...
    case BATTLE_COM_ITEM: // 失奶  丞毛银丹桦宁
        work = CHAR_getWorkInt(charaindex, CHAR_WORKQUICK) + 20;
#ifdef _BATTLE_RAND_DEX
        dex = work - RAND(0, work * getBattleRandDex() / 100 + getBattleRandDex2()) + work * 0.15;
#else
...
#endif
        per = FreeBattleComboCheck(charaindex, per);

        //     互ㄟ井  仃卅井匀凶日
        if (CHAR_getInt(charaindex, CHAR_HP) <= 0 || BATTLE_CanMoveCheck(charaindex) == FALSE)
        {
            move = 0;
        }
        else
    ...
    int enemyindex)
{
    ...
    )
    {
        ...
        else if (BattleArray[battleindex].type == BATTLE_TYPE_P_vs_E)
        {
            ...
            if ((CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER || CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPET) && (CHAR_getInt(enemyindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER || CHAR_getInt(enemyindex, CHAR_WHICHTYPE) == CHAR_TYPEPET))
            {
                int badindex;
                ...
                if (CHAR_getWorkInt(charaindex, CHAR_WORKCONFUSION) <= 0)
                {
                    if (enemyindex != badindex)
                    {
                        ...
                        char message[256];
                        sprintf(message, "%s 被 %s 击飞至远方， %s 因飞人被送进监狱！", CHAR_getUseName(enemyindex), CHAR_getUseName(badindex), CHAR_getUseName(badindex));
                        int i;
                        int playernum = CHAR_getPlayerMaxNum();
            ...
            if (CHAR_getElderPosition(
                    CHAR_getInt(enemyindex, CHAR_LASTTALKELDER), &floor, &x, &y) != TRUE)
            {
            }
            else
            {
                if (BattleFly(enemyindex) == 1)
                {
        ...
        if (CHAR_getInt(enemyindex, CHAR_WHICHTYPE) == CHAR_TYPEPET)
        {
            ...
            */
            ...
            BATTLE_Exit(enemyindex, battleindex);
            //#ifdef    _FIX_BATTLE_DIE
            //      CHAR_setWorkInt(enemyindex,CHAR_WORKBATTLEDIETYPE,0);
            //#endif
        }
        else
        {
...
//*************************************************************
{
    ...
    // 衬午及爵  分匀凶日
    // 伉旦弁及  中田玄伙卅日
    if (CHAR_getInt(enemyindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER && BattleArray[battleindex].type == BATTLE_TYPE_P_vs_E && BattleArray[battleindex].norisk == 0)
    {
        int levelflg = 1;
        ...
        // 矢永玄卅日
        if (CHAR_getInt(enemyindex, CHAR_WHICHTYPE) == CHAR_TYPEPET && BattleArray[battleindex].type == BATTLE_TYPE_P_vs_E && BattleArray[battleindex].norisk == 0)
        {
            int levelflg = 1;
            int playerindex = CHAR_getWorkInt(enemyindex, CHAR_WORKPLAYERINDEX);
    ...
    *pEntryEnemy;
    ...
    for (j = 0; j < 2; j++)
    {
        ...
        for (i = 0; i < BATTLE_ENTRY_MAX; i++)
        {
            ...
            if (CHAR_getInt(enemyindex, CHAR_HP) <= 0 && CHAR_getFlg(enemyindex, CHAR_ISDIE) == FALSE)
            {
            }
            else
            {
...
int BATTLE_AddExpItem(int battleindex, int *pBidList)
{
    ...
    *pEntryEnemy,
    ...
    if (BattleArray[battleindex].Side[side].type != BATTLE_S_TYPE_PLAYER || BattleArray[battleindex].Side[1 - side].type == BATTLE_S_TYPE_PLAYER)
    {
        proflg = 0;
    }
...
#define EXPGET_DIV 15
                ...
                if (getBattleAllExpBase() > 0)
                {
                    int charaindex2[BATTLE_ENTRY_MAX + 1], ii, jj, iii;

                    for (ii = 0; ii < BATTLE_ENTRY_MAX; ii++)
                    {
                        charaindex2[ii] = -1;
                    }
                    ...
                    for (ii = 0; ii < BATTLE_ENTRY_MAX; ii++)
                    {
                        if (BATTLE_No2Index(battleindex, ii) > -1)
                        {
                            int tempi = -1;

                            for (iii = 0; charaindex[iii] != -1; iii++)
                            {
                                if (BATTLE_No2Index(battleindex, ii) == charaindex[iii])
                                {
                                    tempi = charaindex[iii];
                                    break;
                                }
                            }
                            ...
                            if (tempi == -1)
                            {
                                charaindex2[jj] = BATTLE_No2Index(battleindex, ii);
                                // printf("\ncharaindex2[%d]=%s",jj,CHAR_getChar(charaindex2[jj],CHAR_NAME));
                                jj++;
                            }
                        }
                    }
                    ...
                    for (ii = 0; ii < jj; ii++)
                    {
                        int nowexp, b_level;
                        int ridepet;
                        b_level = CHAR_getInt(charaindex2[ii], CHAR_LV) - enemylevel;

                        if (b_level <= EXPGET_MAXLEVEL) //如 人物大怪物5以下 或怪物大於人物
                        {
                            nowexp = exp;
                            nowexp = nowexp * getBattleAllExpBase() / 100;
                        }
                        else //如果人物大於怪物5以上
                        {
                        ...
                        CHAR_setWorkInt(charaindex2[ii], CHAR_WORKGETEXP,
                                        CHAR_getWorkInt(charaindex2[ii], CHAR_WORKGETEXP) + nowexp);
                        CHAR_setInt(charaindex2[ii], CHAR_KILLPETCOUNT,
                                    CHAR_getInt(charaindex2[ii], CHAR_KILLPETCOUNT) + 1);
#ifdef _ITEM_EXP_LV

                        if (CHAR_getInt(charaindex2[ii], CHAR_WHICHTYPE) == CHAR_TYPEPLAYER && CHAR_getInt(charaindex2[ii], CHAR_LV) >= 140
#ifdef _479SA
                            && CHAR_getInt(charaindex2[ii], CHAR_TRANSMIGRATION) >= 5
#endif // _479SA
                        )
                        {
                            int itemtype = 1;
#ifndef _NO_ITEMEXPPET
                            int ridepet = CHAR_getInt(charaindex2[ii], CHAR_RIDEPET);
                            int ridepetindex = CHAR_getCharPet(charaindex2[ii], ridepet);

                            if (CHAR_CHECKINDEX(ridepetindex))
                            {
                                if (CHAR_getInt(ridepetindex, CHAR_LV) < 140)
                                {
                                    itemtype = 0;
                                }
                            }

                            int zhanpet = CHAR_getInt(charaindex2[ii], CHAR_DEFAULTPET);
                            int zhanpetindex = CHAR_getCharPet(charaindex2[ii], zhanpet);

                            if (CHAR_CHECKINDEX(zhanpetindex))
                            {
                                if (CHAR_getInt(zhanpetindex, CHAR_LV) < 140)
                                {
                                    itemtype = 0;
                                }
                            }

#endif // !_NO_ITEMEXPPET
                            int toitemindex = -1;
                            int iii;

                            for (iii = 9; iii <= 23; iii++)
                            {
                                if (itemtype == 0)
                                {
                                    break;
                                ...
                                toitemindex = CHAR_getItemIndex(charaindex2[ii], iii);
                                ...
                                if (ITEM_CHECKINDEX(toitemindex))
                                {
                                    if (strlen(ITEM_getChar(toitemindex, ITEM_USEFUNC)) > 0 && strcmp(ITEM_getChar(toitemindex, ITEM_USEFUNC), "ITEM_useExpLv") == 0)
                                    {
                                        ...
                                        if (getStringFromIndexWithDelim(itemarg, "|", 1, buftest, sizeof(buftest)) == TRUE)
                                        {
                                            ...
                                            CHAR_setWorkInt(charaindex2[ii], CHAR_WORKITEMEXP, CHAR_getWorkInt(charaindex2[ii], CHAR_WORKITEMEXP) + nowexp);
                                            break;
                                        }
                                    }
                                }
...
#endif
                        ridepet = BATTLE_getRidePet(charaindex2[ii]);
                        ...
                        if (BattleArray[battleindex].norisk == 0 && CHAR_getInt(charaindex2[ii], CHAR_WHICHTYPE) == CHAR_TYPEPET)
                        {
                            if (CHAR_getInt(enemyindex, CHAR_LV) > CHAR_getInt(charaindex2[ii], CHAR_LV))
                            {
                                CHAR_PetAddVariableAi(charaindex2[ii], AI_FIX_PETGOLDWIN);
                            }
                            else
                            {
                                CHAR_PetAddVariableAi(charaindex2[ii], AI_FIX_PETWIN);
                            }
                        }
                    }
                }
                ...
                for (k = 0; charaindex[k] != -1; k++)
                {
                    ...
                    if (getBattleComExpBase(allnum) > 0)
                    {
                        nowexp = nowexp + nowexp * getBattleComExpBase(allnum) / 100;
...
#endif
                    // printf("\nname=%s,CHAR_WORKITEMEXP=%d,nowexp=%d",CHAR_getChar(charaindex[k],CHAR_NAME),CHAR_getWorkInt(charaindex[k],CHAR_WORKGETEXP),nowexp);
                    CHAR_setWorkInt(charaindex[k], CHAR_WORKGETEXP,
...
#ifdef _479SA
                        && CHAR_getInt(charaindex[k], CHAR_TRANSMIGRATION) >= 5
#endif // _479SA
                    )
                    {
                        int itemtype = 1;
#ifndef _NO_ITEMEXPPET
                        int ridepet = CHAR_getInt(charaindex[k], CHAR_RIDEPET);
                        int ridepetindex = CHAR_getCharPet(charaindex[k], ridepet);
                        ...
                        int zhanpet = CHAR_getInt(charaindex[k], CHAR_DEFAULTPET);
                        int zhanpetindex = CHAR_getCharPet(charaindex[k], zhanpet);
                        ...
                        for (iii = 9; iii <= 23; iii++)
                        {
                            ...
                            toitemindex = CHAR_getItemIndex(charaindex[k], iii);
                    ...
                    if (BattleArray[battleindex].norisk == 0 && CHAR_getInt(charaindex[k], CHAR_WHICHTYPE) == CHAR_TYPEPET)
                    {
    ...
    int defNo)
{
    ...
    if (CHAR_CHECKINDEX(defindex) == FALSE || CHAR_getWorkInt(defindex, CHAR_WORKBATTLEMODE) == 0 || CHAR_getFlg(defindex, CHAR_ISDIE) == TRUE || CHAR_getInt(defindex, CHAR_HP) <= 0 || CHAR_getFlg(defindex, CHAR_ISATTACKED) == FALSE || CHAR_getWorkInt(defindex, CHAR_WORKBATTLEMODE) == BATTLE_CHARMODE_RESCUE)
    {
        return FALSE;
    }
    ...
    int defNo)
{
    ...
    if (CHAR_CHECKINDEX(defindex) == FALSE || CHAR_getWorkInt(defindex, CHAR_WORKBATTLEMODE) == 0 || CHAR_getWorkInt(defindex, CHAR_WORKBATTLEMODE) == BATTLE_CHARMODE_RESCUE || CHAR_getFlg(defindex, CHAR_ISATTACKED) == FALSE || CHAR_getFlg(defindex, CHAR_ISDIE) == FALSE)
    {
...
void Compute_Down(int charaindex, int rideindex, int *down1, int *down2, int flg)
{
    ...
    *down2 = 0;
    //人物
    downs =
        CHAR_getInt(charaindex, CHAR_VITAL) + CHAR_getInt(charaindex, CHAR_STR) + CHAR_getInt(charaindex, CHAR_DEX) + CHAR_getInt(charaindex, CHAR_TOUGH);
    ...
    else //骑乘宠物
    {
        ...
        downs =
            CHAR_getInt(rideindex, CHAR_VITAL) + CHAR_getInt(rideindex, CHAR_STR) + CHAR_getInt(rideindex, CHAR_DEX) + CHAR_getInt(rideindex, CHAR_TOUGH);
...
static int BATTLE_StatusSeq(int charaindex)
{
    ...
    if (GETLOWVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_RIDE)) > 0)
    {
        ...
        CHAR_setWorkInt(charaindex, CHAR_WORK_RIDE, MAKE2VALUE(hworkride, lworkride));
    }
...
#ifdef _PROFESSION_ADDSKILL
            && (StatusTbl[i] != CHAR_WORKICECRACK2) && (StatusTbl[i] != CHAR_WORKICECRACK3) && (StatusTbl[i] != CHAR_WORKICECRACK4) && (StatusTbl[i] != CHAR_WORKICECRACK5) && (StatusTbl[i] != CHAR_WORKICECRACK6) && (StatusTbl[i] != CHAR_WORKICECRACK7) && (StatusTbl[i] != CHAR_WORKICECRACK8) && (StatusTbl[i] != CHAR_WORKICECRACK9) && (StatusTbl[i] != CHAR_WORKICECRACK10)
#endif
                ) // 冰爆术
        {
#else
            ...
            if (StatusTbl[i] == CHAR_WORKDRUNK)
            {
                if (ridepet != -1)
                {
                    CHAR_setWorkInt(charaindex, CHAR_WORKQUICK,
                                    CHAR_getWorkInt(charaindex, CHAR_WORKQUICK) + CHAR_getWorkInt(ridepet, CHAR_WORKQUICK));
                }
                else
                {
                    CHAR_setWorkInt(charaindex, CHAR_WORKQUICK,
        ...
        case CHAR_WORKDEEPPOISON:
            ...
            else
            {
                ...
                down =
                    CHAR_getInt(charaindex, CHAR_VITAL) + CHAR_getInt(charaindex, CHAR_STR) + CHAR_getInt(charaindex, CHAR_DEX) + CHAR_getInt(charaindex, CHAR_TOUGH);
                petdown =
                    CHAR_getInt(ridepet, CHAR_VITAL) + CHAR_getInt(ridepet, CHAR_STR) + CHAR_getInt(ridepet, CHAR_DEX) + CHAR_getInt(ridepet, CHAR_TOUGH);
                down = (down / 100 - 20) / 4;
...
void BATTLE_TargetListSet(int charaindex, int attackNo, int *pList)
{
    ...
    {
        if (BATTLE_CheckSameSide(charaindex, defNo) == 1 && battleindex > -1 && BattleArray[battleindex].type != BATTLE_TYPE_P_vs_P)
        {
            CHAR_setWorkInt(charaindex, CHAR_WORKBATTLECOM1, BATTLE_COM_NONE);
...
#define PETSKILLSERCHTIME 50
    ...
    for (j = 0, i = 0, k = 0; k < PETSKILLSERCHTIME; i++, k++)
    {
        ...
        if (skill_type == PETSKILL_FIELD_BATTLE || skill_type == PETSKILL_FIELD_ALL)
        {
        }
        else
        {
...
static int BATTLE_PetLoyalCheck(int battleindex, int bid, int charaindex)
{
    ...
    if (CHAR_getWorkInt(charaindex, CHAR_WORKBATTLECOM1) == BATTLE_COM_GUARD || bid == toNo)
    {
        type = 1;
    }
    else
...
int TargetIndex[MAX_MAGIC_NUMBER][2] =
    {
        {301, -1}, {302, -1}, {303, 26}, {304, -1}, {305, 20}, {306, 20}, {307, -1}, {308, -1}, {309, -1}, {310, -1}, {311, 26}, {312, 20}, {313, -1}, {314, -1}, {315, -1}, {316, -1}, {317, 26}, {318, 20}, {319, -1}, {320, -1}, {321, 26}, {322, -1}, {323, 26}, {324, 20}, {325, 20}
#ifdef _TRUE_DAMAGE_MAGIC
        ,
        {700, 20},
        {701, 20},
...
static int BATTLE_Battling(int battleindex)
{
    ...
    for (j = 0; j < 2; j++)
    {
        ...
        for (i = 0; i < BATTLE_ENTRY_MAX / 2; i++)
        {
            ...
            if (pBattle->iEntryBack[k] >= 0 && pBattle->Side[j].Entry[i].charaindex == -1)
            {
    ...
    for (i = 0; i < entrynum; i++)
    {
        ...
        if (CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEMODE) != BATTLE_CHARMODE_C_OK)
        {
        ...
        if (CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPET && CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEDIETYPE) == 1)
        {
            continue;
        }
            ...
            || CHAR_getInt(charaindex, CHAR_BASEIMAGENUMBER) == 101749)
        {
            ...
            if (!(COM == BATTLE_COM_ATTACK || COM == BATTLE_COM_GUARD || COM == BATTLE_COM_NONE))
            {
                CHAR_setWorkInt(charaindex, CHAR_WORKBATTLECOM1, BATTLE_COM_NONE);
            ...
            && (CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER))
        {
            ...
            // CHAR_setWorkInt( charaindex, CHAR_WORKBATTLECOM1, BATTLE_COM_ATTACK );
            if (!(COM == BATTLE_COM_ATTACK || COM == BATTLE_COM_GUARD || COM == BATTLE_COM_NONE || COM == BATTLE_COM_ITEM || COM == BATTLE_COM_ESCAPE || COM == BATTLE_COM_CAPTURE || COM == BATTLE_COM_WAIT || COM == BATTLE_COM_PETIN || COM == BATTLE_COM_PETOUT))
            {
                CHAR_setWorkInt(charaindex, CHAR_WORKBATTLECOM1, BATTLE_COM_GUARD);
        ...
        if (attack_max <= 0)
        {
            ...
            else
            {
                ...
                if (luckwork > 5 * 5)
                {
                    ...
                    luckwork = 5 * 5;
                    CHAR_setInt(charaindex, CHAR_LUCK, 5);
                }
        ...
        if (CHAR_getWorkInt(charaindex, CHAR_DOOMTIME) == 0 && (CHAR_getWorkInt(charaindex, CHAR_WORK_com1) == BATTLE_COM_S_FIRE_SPEAR || CHAR_getWorkInt(charaindex, CHAR_WORK_com1) == BATTLE_COM_S_DOOM))
        {
        ...
        case BATTLE_COM_ATTACK:
            if (gWeponType == ITEM_BOOMERANG)
            {
                ...
                if (CHAR_getInt(charaindex, CHAR_BASEIMAGENUMBER) == 101749 || CHAR_getWorkInt(charaindex, CHAR_WORKFOXROUND) != -1) //若是变成小狐狸不可以使用回力标
                {
                    CHAR_setWorkInt(charaindex, CHAR_WORKBATTLECOM1, BATTLE_COM_NONE);
                }
        ...
        case BATTLE_COM_S_STATUSCHANGE:
            ...
            gBattleDuckModyfy = CHAR_GETWORKINT_HIGH(charaindex, CHAR_WORKBATTLECOM6);
            attack_max = CHAR_GETWORKINT_LOW(charaindex, CHAR_WORKBATTLECOM6);
#endif
            gDamageDiv = attack_max;
            break;
        ...
        case BATTLE_COM_S_RENZOKU:
            ...
            gDamageDiv = attack_max;
            int N = CHAR_GETWORKINT_HIGH(charaindex, CHAR_WORKBATTLECOM3);
            int pow = CHAR_getWorkInt(charaindex, CHAR_WORKFIXSTR);
            pow += pow * N * 0.01;
            CHAR_setWorkInt(charaindex, CHAR_WORKATTACKPOWER,
                            CHAR_getWorkInt(charaindex, CHAR_WORKMODATTACK) + pow);
            break;
#ifdef _VARY_WOLF
        ...
        if (CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEFLG) & CHAR_BATTLEFLG_AIBAD && COM == BATTLE_COM_COMBO)
        {
            print("由於忠诚度不足使用必杀技失败(%s)\n", CHAR_getUseName(charaindex));
            CHAR_setWorkInt(charaindex, CHAR_WORKBATTLECOM1, BATTLE_COM_ATTACK);
        }
...
#ifdef _PREVENT_TEAMATTACK
        {
            ...
            if (BATTLE_CheckSameSide(charaindex, defNo) == 1 && BattleArray[battleindex].type != BATTLE_TYPE_P_vs_P)
            {
                BATTLE_NoAction(battleindex, attackNo);
                break;
            }
        ...
        case BATTLE_COM_S_FIREKILL:
            if (COM == BATTLE_COM_S_FIREKILL)
            {
                ...
                //目标死亡,或不存在,或地球一周
                if (BATTLE_TargetCheck(battleindex, defNo) == FALSE || CHAR_getWorkInt(BATTLE_No2Index(battleindex, defNo), CHAR_WORKBATTLECOM1) == BATTLE_COM_S_EARTHROUND0)
                {
                    //随意找一只打
                    if (defNo < 10)
                    {
                        for (i = 0; i < 10; i++)
                            if (BATTLE_TargetCheck(battleindex, i) && CHAR_getWorkInt(BATTLE_No2Index(battleindex, i), CHAR_WORKBATTLECOM1) != BATTLE_COM_S_EARTHROUND0)
                            {
                                defNo = i;
                                break;
                            }
        ...
        case BATTLE_COM_S_HECTOR:
            if (COM == BATTLE_COM_S_HECTOR)
            {
                ...
                if (BATTLE_GetDamageReact(def_index) > 0)
                {
                    fPer = 0;
                }
...
//#endif
        ...
        case BATTLE_COM_S_ANTINTER:
            if (COM == BATTLE_COM_S_ANTINTER)
            {
                ...
                if (CHAR_getInt(BATTLE_No2Index(battleindex, defNo), CHAR_WHICHTYPE) == CHAR_TYPEPET && CHAR_getInt(BATTLE_No2Index(battleindex, defNo), CHAR_HP) <= 0)
                {
        ...
        case BATTLE_COM_S_RETRACE:
            ...
            else if ((defNo = BATTLE_TargetAdjust(battleindex, charaindex, myside)) < 0 || defNo == attackNo)
            {
                BATTLE_NoAction(battleindex, attackNo);
                break;
            }
            ...
            else
            {
                ...
                if (CHAR_getWorkInt(charaindex, CHAR_WORKITEMMETAMO) > 0 ||
                    CHAR_getWorkInt(charaindex, CHAR_WORKNPCMETAMO) > 0 || CHAR_getWorkInt(charaindex, CHAR_WORKFOXROUND) != -1) //若是变成小狐狸
                {
                ...
                if (gWeponType == ITEM_BOW || gWeponType == ITEM_BOUNDTHROW ||
                    gWeponType == ITEM_BREAKTHROW || gWeponType == ITEM_BOOMERANG)
                {
                    char temponotouzhi[1024];
                    int noutouzhitype = 0;
                    memset(temponotouzhi, 0, sizeof(temponotouzhi));
                    strcpy(temponotouzhi, getNoTouzhiBase());

                    if (strlen(temponotouzhi) > 0)
                    {
                        int tempii = 1;
                        char temptoken[16];
                        memset(temptoken, 0, sizeof(temptoken));

                        while (getStringFromIndexWithDelim(temponotouzhi, ",", tempii, temptoken, sizeof(temptoken)) != FALSE)
                        {
                            if (strlen(temptoken) > 0)
                            {
                                if (CHAR_getInt(charaindex, CHAR_BASEIMAGENUMBER) == atoi(temptoken))
                                {
                                    noutouzhitype = 1;
                                    break;
                                }
                        ...
                        if (noutouzhitype == 1)
                        {
                            CHAR_talkToCli(charaindex, -1, "由于您的造型过于特殊，没有投掷动作，无法使用远距离武器。", CHAR_COLORYELLOW);
                            continue;
                        }
                    }
                }
            ...
            for (attack_count = 0, k = 0;;)
            {
                ...
                if (attack_flg)
                {
                    ...
                    if (Battle_Attack_ReturnData_x.Battle_Attack_ReturnData == BATTLE_RET_DODGE && COM == BATTLE_COM_S_RETRACE)
                    {
                ...
                else if ((defNo = BATTLE_TargetAdjust(battleindex, charaindex, myside)) < 0)
                {
                    break;
                }
            ...
            if (COM == BATTLE_COM_S_INVERSION)
            {
                if ((Battle_Attack_ReturnData_x.Battle_Attack_ReturnData != BATTLE_RET_MISS)                                                                                     //没有miss
                    && (Battle_Attack_ReturnData_x.Battle_Attack_ReturnData != BATTLE_RET_DODGE) && (Battle_Attack_ReturnData_x.Battle_Attack_ReturnData != BATTLE_RET_ALLGUARD) //没有闪避过
            ...
            if ((COM == BATTLE_COM_S_BECOMEFOX) && (Battle_Attack_ReturnData_x.Battle_Attack_ReturnData != BATTLE_RET_MISS) //没有miss
...
#endif
                && (BATTLE_TargetCheck(battleindex, defNo))                                                                     //还活着
                && (rand() % 100 < 31) && (CHAR_getInt(BATTLE_No2Index(battleindex, defNo), CHAR_WHICHTYPE) != CHAR_TYPEPLAYER) //只有宠物(CHAR_TYPEPET)才可以被媚惑
            ...
            if ((COM == BATTLE_COM_S_BECOMEPIG) && (Battle_Attack_ReturnData_x.Battle_Attack_ReturnData != BATTLE_RET_MISS) //没有miss
                && (Battle_Attack_ReturnData_x.Battle_Attack_ReturnData != BATTLE_RET_DODGE)                                //没有闪避过
                && (Battle_Attack_ReturnData_x.Battle_Attack_ReturnData != BATTLE_RET_ALLGUARD)
#ifdef _EQUIT_ARRANGE
                && (Battle_Attack_ReturnData_x.Battle_Attack_ReturnData != BATTLE_RET_ARRANGE)
...
#ifdef _PREVENT_TEAMATTACK
                && ((BATTLE_CheckSameSide(charaindex, defNo) != 1 || BattleArray[battleindex].type == BATTLE_TYPE_P_vs_P)) // 防止同队互打
#endif
            ...
            if (COM == BATTLE_COM_S_STATUSCHANGE)
            {
                if (defNo <= 19)
                {
                    ...
                    if (BattleArray[battleindex].Side[temp2].Entry[temp].flg & BENT_FLG_ULTIMATE && !BATTLE_TargetCheck(battleindex, defNo)) //是否被打飞
                    {
                        for (temp3 = (temp2 * SIDE_OFFSET); temp3 < ((temp2 * SIDE_OFFSET) + SIDE_OFFSET); temp3++)
                        {
                            if (BATTLE_TargetCheck(battleindex, temp3))
                            {
                                ...
                                {
                                    tempdamage = (CHAR_GETWORKINT_LOW(charaindex, CHAR_WORKBATTLECOM5) / 100.0) * CHAR_getInt(tempdefidx, CHAR_HP);

                                    if (tempdefpet != -1)
                                    {
                                        temppetdamage = (CHAR_GETWORKINT_LOW(charaindex, CHAR_WORKBATTLECOM5) / 100.0) * CHAR_getInt(tempdefpet, CHAR_HP);
                                    }
                                }
                                else if (CHAR_GETWORKINT_HIGH(charaindex, CHAR_WORKBATTLECOM5) > 0)
                                {
                                    tempdamage = (CHAR_GETWORKINT_HIGH(charaindex, CHAR_WORKBATTLECOM5) / 100.0) * CHAR_getWorkInt(tempdefidx, CHAR_WORKMAXHP);

                                    if (tempdefpet != -1)
                                    {
                                        temppetdamage = (CHAR_GETWORKINT_HIGH(charaindex, CHAR_WORKBATTLECOM5) / 100.0) * CHAR_getWorkInt(tempdefpet, CHAR_WORKMAXHP);
                                    }
                                }

                                // BATTLE_DamageSub( charaindex, tempdefidx, &tempdamage, &temppetdamage, &tempprefrect );
                                ...
                                if (tempdamage > 0 || temppetdamage > 0) //位置,(0扣血,1扣蓝色,3 闪开),未知,人物伤害,宠物上
                                {
                                    sprintf(szWork, "BD|r%X|%X|0|d%X|p%X|", temp3, 0, tempdamage, temppetdamage);
                                    BATTLESTR_ADD(szWork);
                                }
                            }
                        }
        ...
        case BATTLE_COM_S_VARY:
        {
            ...
            {
                if (CHAR_getWorkInt(charaindex, CHAR_WORKBATTLECOM3) > 0)
                {
                    bk_amn = CHAR_getWorkInt(charaindex, CHAR_WORKBATTLECOM3);
                }
                else
                {
        ...
        case BATTLE_COM_S_RIDE:
        {
            ...
            if (GETLOWVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_RIDE)) > 0)
            {
                ...
                sprintf(szBuffer, "上马术于 %d 回合后才可使用。", GETLOWVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_RIDE)));
        ...
        case BATTLE_COM_PETIN:
            ...
            else
            {
                ...
                if (CHAR_CHECKINDEX(petindex))
                {
                    if (CHAR_getInt(petindex, CHAR_BASEIMAGENUMBER) == 101749 || CHAR_getWorkInt(petindex, CHAR_WORKFOXROUND) != -1) //若是变成小狐狸不可以使用回力标
                    {
                        BATTLE_NoAction(battleindex, attackNo);
                        break;
                    }
...
// Terry add 2001/11/05
        ...
        case BATTLE_COM_JYUJYUTU2:
            CHAR_setWorkInt(charaindex, CHAR_WORKBATTLECOM2, BATTLE_ToRandNo(battleindex, 1));
            MAGIC_DirectUse(
                charaindex,
                CHAR_GETWORKINT_LOW(charaindex, CHAR_WORKBATTLECOM3),
                CHAR_getWorkInt(charaindex, CHAR_WORKBATTLECOM2),
        ...
        case BATTLE_COM_ITEM:
            ...
            */
            ...
            if (itemtype == CHAR_ARM || itemtype == CHAR_HEAD || itemtype == CHAR_BODY || itemtype == CHAR_DECORATION1 || itemtype == CHAR_DECORATION2)
            {
        ...
        case BATTLE_COM_WAIT:
        {
            ...
            if (BattleCommand(attackindex, battleindex) == TRUE)
            {
                BATTLE_NoAction(battleindex, attackNo);
            }
        }
        ...
        case BATTLE_COM_S_BATTLE_MODEL:
            ...
            break;
#endif
#ifdef _PETSKILL_TEMPTATION // cyg add 宠技:水漾诱惑

        case BATTLE_COM_S_TEMPTATION:
            if (COM == BATTLE_COM_S_TEMPTATION)
            {
                int skill;
                ...
                defNo = CHAR_getWorkInt(charaindex, CHAR_WORKBATTLECOM2);
                skill = CHAR_GETWORKINT_LOW(charaindex, CHAR_WORKBATTLECOM3);
                ContFlg = BATTLE_S_Temptation(battleindex, attackNo, defNo, skill);

                if (ContFlg == TRUE) //允许使用魔法
                {
                    break;
                }
            }
        ...
        if (CHAR_CHECKINDEX(charaindex))
        {
            ...
            if (CHAR_getInt(charaindex, CHAR_BASEIMAGENUMBER) == 101428
#ifdef _EXPANSION_VARY_WOLF
                || CHAR_getInt(charaindex, CHAR_BASEIMAGENUMBER) == 102011 || CHAR_getInt(charaindex, CHAR_BASEIMAGENUMBER) == 102012
#endif
            )
            {
                if (CHAR_getWorkInt(charaindex, CHAR_WORKTURN) == 0)
...
#ifdef _PROFESSION_ADDSKILL
            {
                ...
                if (GETHIGHVALUE(CHAR_getWorkInt(charaindex, CHAR_WORKFIXEARTHAT_BOUNDARY)) == 0 && GETHIGHVALUE(CHAR_getWorkInt(charaindex, CHAR_WORKFIXWATERAT_BOUNDARY)) == 0 && GETHIGHVALUE(CHAR_getWorkInt(charaindex, CHAR_WORKFIXFIREAT_BOUNDARY)) == 0 && GETHIGHVALUE(CHAR_getWorkInt(charaindex, CHAR_WORKFIXWINDAT_BOUNDARY)) == 0 && boundaryclear)
                {
...
#ifdef _PETSKILL_LER
    {
        ...
        for (i = 0; i < 20; i++)
        {
            ...
            {
                FreeBattleDie(battleindex, dieindex, i);
            }
        }
    }
#endif
                ...
                if ((strcmp(skill_name, "PROFESSION_ICE_PRACTICE")) == 0)
                {
                    ...
                    if ((strcmp(skill_name, "PROFESSION_THUNDER_PRACTICE")) == 0)
                    {
                        ...
                        if ((strcmp(skill_name, "PROFESSION_AVOID")) == 0)
                        {
                            ...
                            if (profession_class != PROFESSION_SKILL_getInt(Pskillid, PROFESSION_SKILL_PROFESSION_CLASS))
                            {
                                return;
                            }
...
int BATTLE_getBattleX(int battleindex)
{
    if (battleindex >= BATTLE_battlenum || battleindex < 0)
    {
        return -1;
    ...
    return BattleArray[battleindex].x;
}

int BATTLE_getBattleY(int battleindex)
{
    if (battleindex >= BATTLE_battlenum || battleindex < 0)
    {
        return -1;
    ...
    return BattleArray[battleindex].y;
}

#ifdef _BATTLE_PAIDUI
int BATTLE_getBattlePaiType(int battleindex)
{
    if (battleindex >= BATTLE_battlenum || battleindex < 0)
    {
        return -1;
    ...
    return BattleArray[battleindex].paitype;
}
int BATTLE_setBattlePaiType(int battleindex, int num)
{
    if (battleindex >= BATTLE_battlenum || battleindex < 0)
    {
        return -1;
    ...
    BattleArray[battleindex].paitype = num;
    return BattleArray[battleindex].paitype;
}
int BATTLE_getBattlePaiIndex(int battleindex, int num)
{
    if (battleindex >= BATTLE_battlenum || battleindex < 0)
    {
        return -1;
    ...
    return BattleArray[battleindex].paiindex[num];
}
int BATTLE_setBattlePaiIndex(int battleindex, int num, int index)
{
    if (battleindex >= BATTLE_battlenum || battleindex < 0)
    {
        return -1;
    ...
    BattleArray[battleindex].paiindex[num] = index;
    return BattleArray[battleindex].paiindex[num];
}
#endif

int BATTLE_getTopBattleIndex(int battleindex)
{
    if (battleindex >= BATTLE_battlenum || battleindex < 0)
    {
        return -1;
    ...
    if (BattleArray[battleindex].type == BATTLE_TYPE_WATCH)
    {
        return BattleArray[battleindex].topbattleindex;
    }
    else
    {
        return -1;
...
int BATTLE_getLeaderIndex(int battleindex)
{
    if (battleindex >= BATTLE_battlenum || battleindex < 0)
    {
        return -1;
    ...
    return BattleArray[battleindex].leaderindex;
}

void BATTLE_setBattleEntryCharaindex(int battleindex, int side, int no, int newindex)
{
    BATTLE *pBattle;
    BATTLE_ENTRY *pEntry;
    pBattle = &BattleArray[battleindex];
...
BOOL BATTLE_getHelpType(int charaindex)
{
    if (BattleArray[CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEINDEX)].Side[CHAR_getWorkInt(
                                                                                charaindex, CHAR_WORKBATTLESIDE)]
            .flg &
        BSIDE_FLG_HELP_OK)
    {
        return TRUE;
...
int BATTLE_getFieldAtt(int battleindex)
{
    if (battleindex >= BATTLE_battlenum || battleindex < 0)
    {
        return -1;
    ...
    return BattleArray[battleindex].field_att;
}
int BATTLE_getAttCount(int battleindex)
{
    if (battleindex >= BATTLE_battlenum || battleindex < 0)
    {
        return -1;
    ...
    return BattleArray[battleindex].att_count;
}
int BATTLE_getAttPow(int battleindex)
{
    if (battleindex >= BATTLE_battlenum || battleindex < 0)
    {
        return -1;
    ...
    return BattleArray[battleindex].att_pow;
}

#endif

int BATTLE_getTurn(int battleindex)
{
    if (battleindex >= BATTLE_battlenum || battleindex < 0)
    {
        return -1;
    ...
    return BattleArray[battleindex].turn;
}