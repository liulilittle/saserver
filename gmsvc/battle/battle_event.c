...
int StatusTbl[] = {-1, CHAR_WORKPOISON, CHAR_WORKPARALYSIS,
                   ...
                   CHAR_WORKFEAR //恐惧
                   ,
                   CHAR_WORKICECRACK2, CHAR_WORKICECRACK3, CHAR_WORKICECRACK4, CHAR_WORKICECRACK5, CHAR_WORKICECRACK6, CHAR_WORKICECRACK7, CHAR_WORKICECRACK8, CHAR_WORKICECRACK9, CHAR_WORKICECRACK10
#endif
};
...
#ifdef _PLAYER_NPC
        && CHAR_getInt(charaindex, CHAR_WHICHTYPE) != CHAR_TYPEPLAYERNPC && CHAR_getInt(charaindex, CHAR_WHICHTYPE) != CHAR_TYPELUANPC
#endif
    )
    {
        return -1;
...
float BATTLE_adjustRidePet3A(int charaindex, int petindex, int workindex, int action)
{
    ...
    int luaret = FreePlayerRideCtrl(charaindex, petindex, workindex, action, throwweapon);
    ...
    case CHAR_WORKATTACKPOWER: // 攻击力
        if (throwweapon)       // 投掷
            ...
            ret = CHAR_getWorkInt(charaindex, CHAR_WORKATTACKPOWER) * 0.8 + CHAR_getWorkInt(petindex, CHAR_WORKATTACKPOWER) * 0.8;

        break;

    case CHAR_WORKDEFENCEPOWER: // 防御力
        ret = CHAR_getWorkInt(charaindex, CHAR_WORKDEFENCEPOWER) * 0.7 + CHAR_getWorkInt(petindex, CHAR_WORKDEFENCEPOWER) * 0.7;
        break;
    ...
    case CHAR_WORKQUICK: // 敏捷力
        if (action == ATTACKSIDE)
        {
            if (throwweapon)
            {
                ret = CHAR_getWorkInt(charaindex, CHAR_WORKQUICK) * 0.8 + CHAR_getWorkInt(petindex, CHAR_WORKQUICK) * 0.2;
            }
            else
            {
#ifdef _BATTLE_NEWPOWER // andy 1/24 reEdit
                ret = CHAR_getWorkInt(charaindex, CHAR_WORKQUICK) * 0.2 + CHAR_getWorkInt(petindex, CHAR_WORKQUICK) * 0.8;
#else
                ret = CHAR_getWorkInt(charaindex, CHAR_WORKQUICK) * 0.5 + CHAR_getWorkInt(petindex, CHAR_WORKQUICK) * 0.5;
#endif
            }
        }
        else if (action == DEFFENCESIDE)
        {
            ret = CHAR_getWorkInt(charaindex, CHAR_WORKQUICK) * 0.1 + CHAR_getWorkInt(petindex, CHAR_WORKQUICK) * 0.9;
        }
...
void BATTLE_talkToCli(
    ...
    int color)
{
    if (getBattleDebugMsg() != 0 && CHAR_getFlg(charaindex, CHAR_ISSHOWBATTLEMSG))
    {
...
int BATTLE_ItemCrush(int charaindex)
{
    ...
    for (j = 0, i = 0; i < CHAR_EQUIPPLACENUM; i++)
    {
        ...
        if (ITEM_CHECKINDEX(itemindex) == TRUE && ITEM_getInt(itemindex, ITEM_MERGEFLG) == 1)
        {
            EquipTbl[j] = i;
            j++;
        }
    ...
    CHAR_send_P_StatusString(charaindex,
                             CHAR_P_STRING_ATK | CHAR_P_STRING_DEF | CHAR_P_STRING_QUICK | CHAR_P_STRING_CHARM);
    return TRUE;
}
#endif
    ...
    int defindex)
{
    ...
    // 百分百命中
    if (CHAR_GETWORKINT_LOW(attackindex, CHAR_WORKBATTLECOM5) == 777)
    {
        // print("\n inside if statement! (777)");
        return FALSE;
    }
    ...
    if (BATTLE_CanMoveCheck(defindex) == FALSE)
    {
        ...
        {
            if (CHAR_getWorkInt(defindex, CHAR_WORKDRAGNET) == 0 && CHAR_getWorkInt(defindex, CHAR_WORKDIZZY) == 0)
            {
            }
            else
            {
    ...
    if (CHAR_getInt(defindex, CHAR_WHICHTYPE) == CHAR_TYPEENEMY || CHAR_getInt(defindex, CHAR_WHICHTYPE) == CHAR_TYPEPET)
    {
        per = (float)(At_Dex) / (float)(Df_Dex)*0.8 * 100;
    }
    else
    {
        per = (float)(At_Dex) / (float)(Df_Dex)*0.6 * 100;
    }

    per = 100 - per;
#else

    if (CHAR_getInt(attackindex, CHAR_WHICHTYPE) == CHAR_TYPEENEMY && CHAR_getInt(defindex, CHAR_WHICHTYPE) == CHAR_TYPEPET)
    {
        At_Dex *= 0.8;
    }
    else if (CHAR_getInt(attackindex, CHAR_WHICHTYPE) != CHAR_TYPEENEMY && CHAR_getInt(defindex, CHAR_WHICHTYPE) == CHAR_TYPEPET)
    {
        Df_Dex *= 0.8;
    }
    else if (CHAR_getInt(attackindex, CHAR_WHICHTYPE) != CHAR_TYPEPLAYER && CHAR_getInt(defindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
    {
        At_Dex *= 0.6;
    }
    else if (CHAR_getInt(attackindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER && CHAR_getInt(defindex, CHAR_WHICHTYPE) != CHAR_TYPEPLAYER)
    ...
    if (CHAR_getWorkInt(defindex, CHAR_WORKBATTLECOM1) == BATTLE_COM_JYUJYUTU || CHAR_getWorkInt(defindex, CHAR_WORKBATTLECOM1) == BATTLE_COM_JYUJYUTU2)
    {
        gKawashiPara = 0.027;
    }
    else
    ...
    if (CHAR_getWorkInt(defindex, CHAR_WORKDRUNK) > 0)
    {
        per -= RAND(20, 30);
    }
...
#endif
    // per = FreeBattleDuck(attackindex, defindex, per);

    char buff[128];
    sprintf(buff, "命中率：%f  攻敏:%d 防敏:%d", per, At_Dex,Df_Dex);
    strcat(g_strtemp, buff);
...
{
    int iRet = 0;
    My_Fire = My_Fire * Vs_None * AJ_UP + My_Fire * Vs_Fire * AJ_SAME + My_Fire * Vs_Water * AJ_DOWN + My_Fire * Vs_Earth * AJ_SAME + My_Fire * Vs_Wind * AJ_UP;
    My_Water = My_Water * Vs_None * AJ_UP + My_Water * Vs_Fire * AJ_UP + My_Water * Vs_Water * AJ_SAME + My_Water * Vs_Earth * AJ_DOWN + My_Water * Vs_Wind * AJ_SAME;
    My_Earth = My_Earth * Vs_None * AJ_UP + My_Earth * Vs_Fire * AJ_SAME + My_Earth * Vs_Water * AJ_UP + My_Earth * Vs_Earth * AJ_SAME + My_Earth * Vs_Wind * AJ_DOWN;
    My_Wind = My_Wind * Vs_None * AJ_UP + My_Wind * Vs_Fire * AJ_DOWN + My_Wind * Vs_Water * AJ_SAME + My_Wind * Vs_Earth * AJ_UP + My_Wind * Vs_Wind * AJ_SAME;
    My_None = My_None * Vs_None * AJ_SAME + My_None * Vs_Fire * AJ_DOWN + My_None * Vs_Water * AJ_DOWN + My_None * Vs_Earth * AJ_DOWN + My_None * Vs_Wind * AJ_DOWN;
...
#ifdef _BATTLE_NEWPOWER
#else
        T_pow[0] = (CHAR_getWorkInt(charaindex, CHAR_WORKFIXEARTHAT) + CHAR_getWorkInt(petindex, CHAR_WORKFIXEARTHAT)) / 2;
        T_pow[1] = (CHAR_getWorkInt(charaindex, CHAR_WORKFIXWATERAT) + CHAR_getWorkInt(petindex, CHAR_WORKFIXWATERAT)) / 2;
        T_pow[2] = (CHAR_getWorkInt(charaindex, CHAR_WORKFIXFIREAT) + CHAR_getWorkInt(petindex, CHAR_WORKFIXFIREAT)) / 2;
        T_pow[3] = (CHAR_getWorkInt(charaindex, CHAR_WORKFIXWINDAT) + CHAR_getWorkInt(petindex, CHAR_WORKFIXWINDAT)) / 2;
        /*
                *pAt_Fire = ( CHAR_getWorkInt( charaindex, CHAR_WORKFIXFIREAT )
...
int BATTLE_DamageCalc(int attackindex, int defindex)
{
...
#endif
        sprintf(buff, "无骑 防(原防*0.7):%f ", defense);
        strcat(g_strtemp, buff);
    }
    else
    {
...
#endif
        sprintf(buff, "有骑 终防(原防*0.7):%f 人原防:%d", defense, CHAR_getWorkInt(defindex, CHAR_WORKDEFENCEPOWER));
        strcat(g_strtemp, buff);
    }
    ...
    //无装备防御
    if (CHAR_getWorkInt(attackindex, CHAR_WORKBATTLECOM1) == BATTLE_COM_S_REGRET || CHAR_getWorkInt(attackindex, CHAR_WORKBATTLECOM1) == BATTLE_COM_S_REGRET2)
    {
    ...
    if (defense <= attack && attack < (defense * 8.0 / 7.0))
    {
        ...
        sprintf(buff, " 随机0-attack/16伤:%d  防:%f 攻:%f ", damage, defense, attack);
        strcat(g_strtemp, buff);
    }
    else if (defense > attack)
    {
        damage = (int)(RAND(0, 1));
        sprintf(buff, " 随机0-1伤:%d 防:%f 攻:%f ", damage, defense, attack);
        strcat(g_strtemp, buff);
    }
    else if (attack >= (defense * 8 / 7))
    {
        ...
        sprintf(buff, " 随机0-第三个条件 伤:%d 防:%f 攻:%f ", damage, defense, attack);
        strcat(g_strtemp, buff);
    }

    //四属性
    damage = BATTLE_AttrAdjust(attackindex, defindex, damage);
    sprintf(buff, " 属性克制伤:%d", damage);
    strcat(g_strtemp, buff);
#ifdef _PROFESSION_ADDSKILL
    {
        //四属结界
...
#endif
#ifdef _DAMMAGE_CALC
    damage = damage * getDamageCalc() / 100;
    sprintf(buff, " (服务器伤害最终*0.9)后:%d", damage);
    strcat(g_strtemp, buff);
    if (CHAR_getInt(attackindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
    {
        if (CHAR_getInt(attackindex, CHAR_FLOOR) == 1042 || CHAR_getInt(attackindex, CHAR_FLOOR) == 2032 || CHAR_getInt(attackindex, CHAR_FLOOR) == 3032 || CHAR_getInt(attackindex, CHAR_FLOOR) == 4032)
        {
            int tkfmindex = CHAR_getWorkInt(attackindex, CHAR_WORKFMINDEXI);
            char fmindex[4];
            int i;
            ...
            if (i >= MANORNUM)
            {
                int fmid = CHAR_getInt(attackindex, CHAR_FLOOR) / 1000;
                int damagebase = sasql_queryFmPointData(fmid, 0);
                ...
                if (damagebase > 0 && (NowTime.tv_sec - damagebase) / 86400 >= 7)
                {
                    ...
                    damage = damage * (damagebase + 100) / 100;
                }
            }
        }
    }
    ...
    {
        int ownerindex = CHAR_getWorkInt(attackindex, CHAR_WORKPLAYERINDEX);

        if (CHAR_getInt(ownerindex, CHAR_FLOOR) == 1042 || CHAR_getInt(ownerindex, CHAR_FLOOR) == 2032 || CHAR_getInt(ownerindex, CHAR_FLOOR) == 3032 || CHAR_getInt(ownerindex, CHAR_FLOOR) == 4032)
        {
            int tkfmindex = CHAR_getWorkInt(ownerindex, CHAR_WORKFMINDEXI);
            char fmindex[4];
            int i;
            ...
            if (i >= MANORNUM)
            {
                int fmid = CHAR_getInt(ownerindex, CHAR_FLOOR) / 1000;
                int damagebase = sasql_queryFmPointData(fmid, 0);
...
#ifdef _DAMMAGE_CALC
    damage = FreeBattleDamage(attackindex, defindex, damage);
#endif
    sprintf(buff, " 最终伤(独立处理):%d", damage);
    strcat(g_strtemp, buff);
    return damage;
}
...
static int BATTLE_CriticalCheckPlayer(int attackindex, int defindex)
{
    ...
    if (CHAR_getInt(attackindex, CHAR_WHICHTYPE) == CHAR_TYPEPET && CHAR_getInt(defindex, CHAR_WHICHTYPE) == CHAR_TYPEENEMY)
    {
        Df_Dex *= 0.8;
    }
    else if (CHAR_getInt(attackindex, CHAR_WHICHTYPE) == CHAR_TYPEENEMY && CHAR_getInt(defindex, CHAR_WHICHTYPE) == CHAR_TYPEPET)
    ...
    // per = FreeBattleCritical(attackindex, defindex, per);
#ifdef _PETSKILL_LER
...
#endif
    char buff[128];
    sprintf(buff, "会心概率：%d  攻敏:%d 防敏:%d", (int)per, At_Dex, Df_Dex);
    strcat(g_strtemp, buff);
    return (int)per;
}
...
static int BATTLE_CounterCalc(int attackindex, int defindex)
{
    ...
    if (CHAR_getInt(attackindex, CHAR_WHICHTYPE) == CHAR_TYPEENEMY && CHAR_getInt(defindex, CHAR_WHICHTYPE) == CHAR_TYPEPET) //攻击者为怪，被攻击者为宠物
    {
        divpara = 10;
        root = 0;
    }
    else if (CHAR_getInt(attackindex, CHAR_WHICHTYPE) == CHAR_TYPEPET && CHAR_getInt(defindex, CHAR_WHICHTYPE) == CHAR_TYPEENEMY) //攻击者为宠物,被攻击则为怪物
    {
        Df_Dex *= 0.8;
    }
    else if (CHAR_getInt(attackindex, CHAR_WHICHTYPE) != CHAR_TYPEPLAYER && CHAR_getInt(defindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER) //攻击者为非人物,被攻击者为人物
    {
        divpara = 10;
        root = 0;
    }
    else if (CHAR_getInt(attackindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER && CHAR_getInt(defindex, CHAR_WHICHTYPE) != CHAR_TYPEPLAYER) //攻击者为人物,被攻击者为非人物
    {
        Df_Dex *= 0.6;
    }
...
static int BATTLE_CriDamageCalc(int attackindex, int defindex)
{
    ...
    damage += CHAR_getWorkInt(defindex, CHAR_WORKDEFENCEPOWER) * (float)CHAR_getInt(attackindex, CHAR_LV) / (float)CHAR_getInt(defindex, CHAR_LV) * 0.5;

    sprintf(buff, " 会心伤害:%d", CHAR_getWorkInt(defindex, CHAR_WORKDEFENCEPOWER) * (float)CHAR_getInt(attackindex, CHAR_LV) / (float)CHAR_getInt(defindex, CHAR_LV) * 0.5);
    strcat(g_strtemp, buff);
...
static int BATTLE_GuardianCheck(int attackindex, int defindex)
{
    ...
    if (CHAR_getWorkInt(GuardianIndex, CHAR_WORKSLEEP) > 0 || CHAR_getWorkInt(GuardianIndex, CHAR_WORKCONFUSION) > 0 || CHAR_getWorkInt(GuardianIndex, CHAR_WORKPARALYSIS) > 0 || CHAR_getWorkInt(GuardianIndex, CHAR_WORKSTONE) > 0 || CHAR_getWorkInt(GuardianIndex, CHAR_WORKBARRIER) > 0 || GuardianIndex == attackindex
#ifdef _PROFESSION_SKILL                                          // WON ADD 人物职业技能
...
static int BATTLE_AttackSeq(int attackindex, int defindex, int *pDamage, int *pGuardian, int opt)
{
    ...
    // 被攻击者为宠物
    if (CHAR_getInt(defindex, CHAR_WHICHTYPE) == CHAR_TYPEPET && BattleArray[battleindex].norisk == 0 //不是百人道场
        && BattleArray[battleindex].type == BATTLE_TYPE_P_vs_E)                                       //人和怪的战斗
    {
        if (CHAR_getWorkInt(defindex, CHAR_WORKPLAYERINDEX) == attackindex) //如果人物攻击自己宠物
        {
            CHAR_PetAddVariableAi(defindex, AI_FIX_SEKKAN); // 修正忠诚度 宠物忠诚减少
        }
    }

    // 闪避
    if (opt != BATTLE_COM_COMBO) //攻击不为必杀技时
    {
        ...
        if (CHAR_getWorkInt(defindex, CHAR_WORKDUCKPOWER) > 0) //装备套装闪避功能
        {
    ...
    if (opt == BATTLE_COM_S_DAMAGETOHP2) //暗月狂狼 (会心一击上升30%)  && CHAR_getInt( attackindex, CHAR_HP) <= CHAR_getWorkInt( attackindex, CHAR_MAXMP)*0.5
    {
        ...
        CHAR_setWorkInt(attackindex, CHAR_WORKQUICK, CHAR_getWorkInt(attackindex, CHAR_WORKFIXDEX));                                                             //敏上升20%
        // print("\n2会心一击:%d",perCri);
...
#endif
                if (CHAR_getWorkInt(defindex, CHAR_WORKBATTLECOM1) == BATTLE_COM_GUARD && CHAR_getWorkInt(defindex, CHAR_WORKCONFUSION) <= 0)
            {
        ...
        CHAR_getInt(defindex, CHAR_WHICHTYPE) == CHAR_TYPEPET)
    {
        ...
        if (myside == CHAR_getWorkInt(attackindex, CHAR_WORKBATTLESIDE) && BattleArray[battleindex].type != BATTLE_TYPE_P_vs_P)
        {
    ...
    if ((*pDamage) == 0)
    {
        ...
        else if (CHAR_getWorkInt(defindex, CHAR_WORKBATTLECOM1) == BATTLE_COM_GUARD && CHAR_getWorkInt(defindex, CHAR_WORKCONFUSION) <= 0)
        {
            iRet = BATTLE_RET_ALLGUARD;
        }
    }
    char buff11[128];
    sprintf(buff11, " 修正值(技能效果):%f 未修伤:%d 修正后(相乘)%d", gBattleDamageModyfy, *pDamage, (int)((*pDamage) * gBattleDamageModyfy));
    strcat(g_strtemp, buff11);
    ...
    if (CHAR_GETWORKINT_LOW(attackindex, CHAR_WORKBATTLECOM5) == 777)
    {
        CHAR_SETWORKINT_LOW(attackindex, CHAR_WORKBATTLECOM5, 0);
    }
...
int BATTLE_DamageSub(int attackindex, int defindex, int *pDamage, int *pPetDamage, int *pRefrect)
{
    ...
    int tmdamage = *pDamage;
    *pDamage = BattleDamage(attackindex, defindex, tmdamage); //伤害调用LUA
    damage = *pDamage;
    ...
    if (CHAR_getInt(defindex, CHAR_HP) - damage <= 0 && CHAR_getWorkInt(attackindex, CHAR_WORKBATTLECOM1) == BATTLE_COM_S_SHOWMERCY)
    {
    ...
    else if (*pRefrect == BATTLE_MD_REFLEC)
    {
            ...
            if (*pRefrect == BATTLE_MD_ACUPUNCTURE) //针刺外皮
        {
...
#endif
            {
                ...
                playerdamage = playerdamage / 4 * 3;
                petdamage = petdamage / 4 * 3;
                defindex = attackindex;
                defpet = attackpet;
            }
        }
    ...
    if (defpet != -1)
    {
        ...
        if (pethp <= 0)
        {
            ...
            CHAR_setWorkInt(defindex, CHAR_WORK_RIDE, MAKE2VALUE(CHAR_getInt(defindex, CHAR_RIDEPET), GETHIGHVALUE(CHAR_getWorkInt(defindex, CHAR_WORK_RIDE))));
#endif
    ...
    if (FreeBattlePer(defindex, IsUltimate) == 0)
    {
        IsUltimate = 0;
    }
...
int BATTLE_Attack(int battleindex, int attackNo, int defNo)
{
    ...
    sprintf(szBuffer, "攻方[%s] 守方[%s]", CHAR_getChar(attackindex, CHAR_NAME), CHAR_getChar(defindex, CHAR_NAME));
    strcat(g_strtemp, szBuffer);
    ...
    if (CHAR_getInt(defindex, CHAR_HP) <= 0)
    {
        ...
        if (FreeBattlePer(defindex, ultimate) == 0)
        {
            ultimate = 0;
        }
    ...
    if (damage > 0 && gBattleStausChange >= 0)
    {
                ...
                &perStatus) == TRUE)
        {
            ...
            if (gBattleStausChange == BATTLE_ST_PARALYSIS || gBattleStausChange == BATTLE_ST_SLEEP || gBattleStausChange == BATTLE_ST_STONE || gBattleStausChange == BATTLE_ST_BARRIER)
            {
    ...
    if (damage > 0 && (CHAR_getInt(attackindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER))
    {
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            if (turn > 0)
            {
                ...
                // 状态
                for (j = 1; j < min(BATTLE_ST_END, 32); j++)
                {
                ...
                if (Guardian >= 0)
                {
                    iRet = BATTLE_PROFESSION_RANG_ATTACK_DAMAGE(attackindex, attackNo, Guardian, skill_type, status, turn, perStatus, 0, img1, img2);
                }
                else
                {
                    iRet = BATTLE_PROFESSION_RANG_ATTACK_DAMAGE(attackindex, attackNo, defNo, skill_type, status, turn, perStatus, 0, img1, img2);
                }
    ...
    if (CHAR_getInt(attackindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
    {
        CHAR_talkToCli(attackindex, -1, g_strtemp, 0);
    }

    if (CHAR_getInt(attackindex, CHAR_WHICHTYPE) == CHAR_TYPEPET)
    {
        CHAR_talkToCli(CHAR_getWorkInt(attackindex, CHAR_WORKPLAYERINDEX), -1, g_strtemp, 0);
    }
    if (CHAR_getInt(defindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
    {
        CHAR_talkToCli(defindex, -1, g_strtemp, 0);
    }
    if (CHAR_getInt(defindex, CHAR_WHICHTYPE) == CHAR_TYPEPET)
    {
        CHAR_talkToCli(CHAR_getWorkInt(defindex, CHAR_WORKPLAYERINDEX), -1, g_strtemp, 0);
    }
    return iRet;
}
...
static BOOL BATTLE_CounterCheckPlayer(int attackindex, int defindex, int *pPar)
{
    ...
    if (BATTLE_IsThrowWepon(At_SoubiIndex) == TRUE) //如果攻击者使用的武器为弓箭、回旋镖、弓箭、投掷石、投掷斧头
    {
        *pPar = 0.0;
        return FALSE;
    }

    if (ITEM_CHECKINDEX(At_SoubiIndex) == FALSE) //对象没使用武器道具
    {
...
#endif
    per = FreeBattleCounter(attackindex, defindex, per);
    *pPar = per;
    per *= 100;
...
static BOOL BATTLE_CounterCheck(int attackindex, int defindex, int *pPar)
{
    ...
    if (CHAR_getInt(attackindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER) //人物
    {
...
BOOL BATTLE_Counter(int battleindex, int attackNo, int defNo)
{
    ...
    //攻击者使用技能如果非攻击类技能或不守战法技能,则无反击效果
    if (CHAR_getWorkInt(attackindex, CHAR_WORKBATTLECOM1) == BATTLE_COM_ATTACK || CHAR_getWorkInt(attackindex, CHAR_WORKBATTLECOM1) == BATTLE_COM_S_NOGUARD)
    {
        ;
        ;
    }
    ...
    //这条代码没使用
    gDamageDiv = 1.0;
    //获取攻击者攻击的效果，效果如下，击中，暴击,击中没出血（MISS）,闪躲,格挡
    ...
    float *pPer)
{
    ...
    *pPer = 0;
    ...
    WorkGet = CapturePer(attackindex, defindex, WorkGet);
    *pPer = WorkGet;
...
BOOL BATTLE_Escape(int battleindex, int attackNo, int flag)
{
    ...
    if (flg == 1 || flag == 1)
    {
        ...
        //       (attackNo >= 10)? CHAR_COLORGRAY : CHAR_COLORPURPLE ) ;
        // }
        if (BattleArray[battleindex].type == BATTLE_TYPE_P_vs_P && CHAR_getInt(attackindex, CHAR_HP) > 0)
        {
...
int BATTLE_SurpriseCheck(int battleindex)
{
    ...
    if (BattleArray[battleindex].Side[0].type == BATTLE_S_TYPE_PLAYER && BattleArray[battleindex].Side[1].type == BATTLE_S_TYPE_PLAYER)
    {
        return 0;
    }
...
int BATTLE_PetIn(int battleindex, int attackNo)
{
    ...
    if (CHAR_getWorkInt(petindex, CHAR_WORKFOXROUND) != -1 || CHAR_getInt(petindex, CHAR_BASEIMAGENUMBER) == 101749) //若是变身为狸
    {
        CHAR_setInt(petindex, CHAR_BASEIMAGENUMBER, CHAR_getInt(petindex, CHAR_BASEBASEIMAGENUMBER));
        CHAR_setWorkInt(petindex, CHAR_WORKATTACKPOWER, CHAR_getWorkInt(petindex, CHAR_WORKFIXSTR));
        CHAR_setWorkInt(petindex, CHAR_WORKQUICK, CHAR_getWorkInt(petindex, CHAR_WORKFIXDEX));
...
int BATTLE_S_Refresh(int battleindex, int attackNo, int defNo, int marray)
{
    ...
    {
        for (i = 0; i < min(BATTLE_ST_END, 32); i++)
        {
    ...
    int *pPer)
{
    ...
    else
    {
        float templP = 0.0;
        float fVitalP = 0.0;
        fVitalP = CHAR_getInt(defindex, CHAR_VITAL) + CHAR_getInt(defindex, CHAR_STR) + CHAR_getInt(defindex, CHAR_TOUGH) + CHAR_getInt(defindex, CHAR_DEX);
        ...
        if (BattleArray[battleindex].type != BATTLE_TYPE_P_vs_P)
        {
            level = CHAR_getInt(attackindex, CHAR_LV) - CHAR_getInt(defindex, CHAR_LV);
            level *= Bai;
        }
        else
        {
...
#ifdef _SUIT_ADDENDUM
        per = PerOffset + level + 3 // CHAR_getWorkInt( attackindex, CHAR_WORKFIXLUCK )
        ...
        // printf("\nPerOffset=%d,level=%d,luck=%d,Df_Reg=%d,fVitalP=%f,RESIST=%d,per=%d",PerOffset,level,CHAR_getWorkInt( attackindex, CHAR_WORKFIXLUCK ),Df_Reg,fVitalP,CHAR_getWorkInt( defindex, CHAR_WORKRESIST ),per);
        if (per > 50)
        {
            per = 50;
        }
    ...
    per = FreeBattleStatus(attackindex, defindex, status, per);
    ...
    int array)
{
    ...
    if (RAND(1, 100) < per)
    {
        ...
        if (Deftype == CHAR_TYPEPET)
        {
            ...
            CHAR_setWorkInt(defindex, CHAR_WORK_PETABDUCTTYPE, 1);
#endif
        }
...
#ifdef _PETSTANDY_ABDUCT
        CHAR_setWorkInt(attackindex, CHAR_WORK_PETABDUCTTYPE, 1);
#endif
    }
    else
    {
...
int BATTLE_S_FallGround(int battleindex, int attackNo, int defNo, int skill_type)
{
    ...
    if (skill_type == BATTLE_COM_S_FALLRIDE && damage > 0 && react == 0
        ...
        && (BATTLE_CheckSameSide(attackindex, defNo) == 0 || BattleArray[battleindex].type == BATTLE_TYPE_P_vs_P)
#endif
            ) //落马
    {
        ...
        )
        {
            if (CHAR_getInt(defindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
            {
                ...
                if (CHAR_getInt(defindex, CHAR_RIDEPET) > 0)
                {
                    ...
                    CHAR_setWorkInt(defindex, CHAR_WORK_RIDE, MAKE2VALUE(CHAR_getInt(defindex, CHAR_RIDEPET), GETLOWVALUE(fallflg)));
#endif
                    CHAR_setInt(defindex, CHAR_RIDEPET, -1);
                    BATTLE_changeRideImage(defindex);
                    CHAR_setWorkInt(defindex, CHAR_WORKPETFALL, 1);
...
void BATTLE_S_ToothCrushe(int battleindex, int attackindex, int defindex, int damage, int skill)
{
    ...
    if ((crushindex = BATTLE_ItemCrushCheck(defindex, 1)) >= 0)
    {
        ...
        CHAR_complianceParameter(defindex);
        CHAR_send_P_StatusString(defindex, CHAR_P_STRING_ATK | CHAR_P_STRING_DEF | CHAR_P_STRING_QUICK | CHAR_P_STRING_CHARM);
    }
...
void BATTLE_S_Modifyattack(int battleindex, int attackindex, int defindex, int *damage, int skill)
{
    ...
    struct tagModKind
    {
        ...
        {
            {"EA", CHAR_EARTHAT}, {"WA", CHAR_WATERAT}, {"FI", CHAR_FIREAT}, {"WI", CHAR_WINDAT}, {"ALL", 100}};
...
int BATTLE_S_AttackDamage(int battleindex, int attackNo, int defNo, int skill_type, int skill)
{
...
#ifdef _PREVENT_TEAMATTACK
        if (BATTLE_CheckSameSide(attackindex, defNo) == 1 && BattleArray[battleindex].type != BATTLE_TYPE_P_vs_P)
        {
        }
        else
#endif
    ...
    //强制将死掉的宠打飞
    if (CHAR_getWorkInt(attackindex, CHAR_WORKBATTLECOM1) == BATTLE_COM_S_ANTINTER && CHAR_getInt(defindex, CHAR_WHICHTYPE) == CHAR_TYPEPET)
    {
        ultimate = 2;
        flg &= (!BCF_ULTIMATE_1);
        flg |= BCF_DEATH;
    ...
    case BATTLE_COM_S_REGRET:
    {
        ...
        if ((PROFESSION_BATTLE_StatusAttackCheck(attackindex, defindex, 12, Success) == 0) || (CHAR_getInt(defindex, CHAR_HP) <= 0))
        {
        }
        else
        {
    ...
    int marray)
{
    ...
    if (IsBATTLING(charaindex) == TRUE)
    {
        for (i = 0; ToList[i] != -1; i++)
        {
            ...
            if (BATTLE_StatusAttackCheck(charaindex, toindex, BATTLE_ST_NOCAST, Success, 30, 1.0, &perStatus) == TRUE && CHAR_getInt(toindex, CHAR_WHICHTYPE) != CHAR_TYPEPET)
            {
                //无法展开咒术页
...
int CHAR_BattleStayLoop(int charaindex)
{
    ...
    if (CHAR_getWorkInt(charaindex, CHAR_WORK_OFFLINE) > 0)
    {
        if (CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEMODE) == BATTLE_CHARMODE_NONE)
        {
            ...
            if (CHAR_getWorkInt(charaindex, CHAR_WORKPARTYMODE) == CHAR_PARTY_NONE && CHAR_getWorkInt(charaindex, CHAR_WORK_OFFLINE) > 1)
            {
                return 0;
            }
...
int battle_profession_attack_fun(int battleindex, int attackNo, int defNo, int charaindex)
{
    ...
    // 非战斗使用技能
    if (PROFESSION_SKILL_getInt(skill, PROFESSION_SKILL_USE_FLAG) != 1)
    {
        return iRet;
    }
    ...
    case BATTLE_COM_S_CHAOS: // 混乱攻击
    {
        ...
        for (i = f_num; i < f_num + 10; i++)
        {
            ...
            {
                if (CHAR_getInt(BATTLE_No2Index(battleindex, i), CHAR_WHICHTYPE) != CHAR_TYPEPET || CHAR_getWorkInt(BATTLE_No2Index(battleindex, i - 5), CHAR_WORKULTIMATE))
                {
                    temp[j] = i;
                    j++;
                }
        ...
        while (atk_num - 1 > 0)
        {
            ...
            {
                if (CHAR_getInt(BATTLE_No2Index(battleindex, chose_temp[k]), CHAR_WHICHTYPE) != CHAR_TYPEPET || CHAR_getWorkInt(BATTLE_No2Index(battleindex, i - 5), CHAR_WORKULTIMATE))
                {
...
int battle_profession_assist_fun(int battleindex, int attackNo, int defNo, int charaindex)
{
    ...
    case BATTLE_COM_S_CALL_NATURE: // 号召自然
    {
        ...
        for (i = 0; ToList[i] != -1; i++)
        {
            ...
            if (BattleArray[battleindex].norisk == 0 && CHAR_getInt(toindex, CHAR_WHICHTYPE) == CHAR_TYPEPET)
            {
...
int battle_profession_status_chang_fun(int battleindex, int attackNo, int defNo, int charaindex)
{
    ...
    case BATTLE_COM_S_THUNDER_ENCLOSE: // 雷附体
    {
        ...
        for (i = 0; ToList[i] != -1; i++)
        {
            ...
            // 状态命中检定
            if ((PROFESSION_BATTLE_StatusAttackCheck(charaindex, toindex, status, Success) == 0) || (CHAR_getInt(toindex, CHAR_HP) <= 0))
            {
            ...
            else
            {
                ...
                if (status == BATTLE_ST_PARALYSIS || status == BATTLE_ST_SLEEP || status == BATTLE_ST_STONE || status == BATTLE_ST_BARRIER || status == BATTLE_ST_DIZZY || status == BATTLE_ST_ENTWINE || status == BATTLE_ST_DRAGNET || status == BATTLE_ST_ICECRACK || status == BATTLE_ST_ICEARROW)
                {
    ...
    case BATTLE_COM_S_SHIELD_ATTACK: // 盾击
    {
        ...
        for (i = 0; ToList[i] != -1; i++)
        {
            ...
            if (hit == 1)
            {
                if ((PROFESSION_BATTLE_StatusAttackCheck(charaindex, defindex, status, Success) == 0) || (CHAR_getInt(defindex, CHAR_HP) <= 0))
                {
                    // snprintf( szBuffer, sizeof(szBuffer), "(%s)无法将(%s)做成(%s)(%d%%)",
    ...
    case BATTLE_COM_S_TOXIN_WEAPON: // 毒素武器
    {
...

        {
            ...
            if (CHAR_getWorkInt(charaindex, CHAR_WORKFOXROUND) > 0 || CHAR_getWorkInt(charaindex, CHAR_BECOMEPIG) > 0)
            {
                bChange = TRUE;
            }
            ...
            for (i = 0; ToList[i] != -1; i++)
            {
                ...
                if (damage > 0)
                {
                    ...
                    if ((PROFESSION_BATTLE_StatusAttackCheck(charaindex, defindex, status, perStatus) == 0) || (CHAR_getInt(defindex, CHAR_HP) <= 0))
                    {
                        // snprintf( szBuffer, sizeof(szBuffer), "(%s)无法将(%s)做成(%s)(%d%%)",
                        //   CHAR_getUseName( charaindex ), CHAR_getUseName( defindex ), aszStatusFull[status], perStatus );
                        // BATTLE_BroadCast( battleindex, szBuffer,
...
int notSkill[] =
    {
        BATTLE_COM_S_FIREKILL,          //火线猎杀
        BATTLE_COM_S_GYRATE,            //回旋攻击
        ...
        BATTLE_COM_S_BATTLE_MODEL,      //群蜂乱舞
        BATTLE_COM_S_HECTOR,            //威吓
        BATTLE_COM_S_BECOMEPIG,         // 乌力化
    };
BOOL BATTLE_S_Temptation(int battleindex, int attackNo, int defNo, int skill)
{
    BOOL iRet = FALSE, findflg = FALSE;
    char *magicarg;
    char szWork[256];
    int attackindex, defindex, defside, flg = 0, img2, img1, deftarget, com, i;
    int strdef;
    ...
    if (CHAR_getWorkInt(defindex, CHAR_WORKFIXAI) <= atoi(magicarg))
    {
        ...
        com = CHAR_getWorkInt(defindex, CHAR_WORKBATTLECOM1);

        for (i = 0; i < arraysizeof(notSkill); i++)
        {
            if (com == notSkill[i])
            {
                findflg = TRUE;
        ...
        {
            //不能打自己的或有问题的改为背水2
            CHAR_setWorkInt(defindex, CHAR_WORKBATTLECOM1, BATTLE_COM_S_POWERBALANCE);
            deftarget = BATTLE_DefaultAttacker(battleindex, defside);

            if (deftarget == defNo)
            {
                deftarget = deftarget - 5;
            }

            strdef = CHAR_getWorkInt(defindex, CHAR_WORKFIXSTR);
            strdef = (int)(strdef * 0.45);
            CHAR_setWorkInt(defindex, CHAR_WORKATTACKPOWER, (CHAR_getWorkInt(defindex, CHAR_WORKFIXSTR) + strdef));
            strdef = CHAR_getWorkInt(defindex, CHAR_WORKFIXTOUGH);
            strdef = (int)(strdef * -0.5);
            CHAR_setWorkInt(defindex, CHAR_WORKDEFENCEPOWER, (CHAR_getWorkInt(defindex, CHAR_WORKFIXTOUGH) + strdef));
        }
        else
        {
            deftarget = CHAR_getWorkInt(defindex, CHAR_WORKBATTLECOM2);

            if (deftarget == TARGET_SIDE_0) //右下全体
            {
                deftarget = TARGET_SIDE_1;
            }
            else if (deftarget == TARGET_SIDE_1) //左上全体
            {
                deftarget = TARGET_SIDE_0;
            }
            else if (deftarget == TARGET_SIDE_1_B_ROW) //左上后一列
            {
                deftarget = TARGET_SIDE_0_B_ROW;
            }
            else if (deftarget == TARGET_SIDE_1_F_ROW) //左上前一列
            {
                deftarget = TARGET_SIDE_0_F_ROW;
            }
            else if (deftarget == TARGET_SIDE_0_B_ROW) //右下后一列
            {
                deftarget = TARGET_SIDE_1_B_ROW;
            }
            else if (deftarget == TARGET_SIDE_0_F_ROW) //右下前一列
            {
                deftarget = TARGET_SIDE_1_F_ROW;
            }
            else //单人
            {
                deftarget = BATTLE_DefaultAttacker(battleindex, defside);
        ...
        CHAR_setWorkInt(defindex, CHAR_WORKBATTLECOM2, deftarget);
        img1 = 101684; //泡泡101505;   //珠珠动画101637; //准备期动画
        img2 = 101684; //泡泡101502;   //珠珠动画101645; //击中后动画
...
void BATTLE_BatFly(int battleindex, int attackNo, int myside)
{
    ...
    for (i = 0; i < SIDE_OFFSET; i++)
    {
        ...
        if (CHAR_getInt(toindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
        {
            charhp = CHAR_getInt(toindex, CHAR_MP);

            if (charhp >= 30)
            {
                CHAR_setInt(toindex, CHAR_MP, charhp - 30);
                charhp = 30;
            }
            else if (charhp > 0)
            {
                CHAR_setInt(toindex, CHAR_MP, 0);
            }

            if (charhp > 0)
            {
                sprintf(szCommand, "BD|r%X|%X|0|d%X|", ToList[i], BD_KIND_MP, charhp);
                BATTLESTR_ADD(szCommand);
            }
        }
    }
    ...
    int iType)
{
    ...
    if (CHAR_getInt(iDefindex, CHAR_HP) <= 0)
    {
        ...
        if (FreeBattlePer(iDefindex, iUltimate) == 0)
        {
            iUltimate = 0;
        }
    ...
    else
    {
        ...
        if (iDamage > 0 && BATTLE_StatusAttackCheck(charaindex, iDefindex, iEffect, iEffectHit, 30, 1.0, &iTemp))
        {
            ...
            {
                CHAR_setWorkInt(iDefindex, CHAR_WORKDRUNK, CHAR_getWorkInt(iDefindex, CHAR_WORKDRUNK) >> 1);
            }
            ...
            if (iGuardian >= 0)
            {
                BATTLE_BadStatusString(iGuardian, iEffect); // iGuardian 大於等於 0 表示有目标使用了忠犬,该目标状态改变
            }
            else
            {
