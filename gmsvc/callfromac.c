...
extern char manorname[10][CHARNAMELEN];
#endif
...
void saacproto_S_UpdataStele_recv(int i, char *ocdkey, char *oname, char *ncdkey,
                                  char *nname, char *title, int level, int trns, int floor, int time)
{
    //print("\nSyu log 收到Single=> %s , %s , %s , %s ",ocdkey , oname , ncdkey , nname);
    NPC_Alldoman_S_WriteStele(ocdkey, oname, ncdkey, nname, title, level, trns, floor, time);
}
...
void saacproto_ACFMCharLogin_recv(int fd, char *result, int index, int floor,
                                  ...
                                  int charfame, char *charid, char *charname
...
//        print("Login charfame:%d\n", charfame);
    ACFMCharLogin(charid, charname, ret, index, floor, fmdp, joinflag, fmsetupflag,
                                ...
                                int charfdid)
{
    ...
    if (kindflag == 1)
    {
		...
		if(FamilyBankRecv(CONNECT_getCharaindex(clifd),data) == 0)
#endif
...
void saacproto_ACCharLogin_recv(int fd, int clifd, int flag)
{
    ...
    {
        lssproto_ClientLogin_send(clifd, "您的游戏账号已被锁定，无法登录游戏，如有疑问请联系客服。");
        //CONNECT_endOne_debug(clifd);
        return;
    }
    else if (flag == 3)
    {
        lssproto_ClientLogin_send(clifd, "您的IP已被锁定，无法登录游戏，如有疑问请联系客服。");
        //CONNECT_endOne_debug(clifd);
        return;
    }
    else if (flag == 4)
    {
        lssproto_ClientLogin_send(clifd, "您在当前线路登录数量已满，请尝试登录其他线路。");
        //CONNECT_endOne_debug(clifd);
        return;
    }
    else if (flag == 5)
    {
        lssproto_ClientLogin_send(clifd, "您的账号尚未注册，请登录www.shiqi.pk，注册后登录游戏。");
        //CONNECT_endOne_debug(clifd);
        return;
    }
    else if (flag == 6)
    {
        lssproto_ClientLogin_send(clifd, "您的密码输入错误，请核对后登录。（注意大小写）");
        //CONNECT_endOne_debug(clifd);
        return;
    }
    else
...
void saacproto_QueryPoint_recv(int fd, int point)
{
    ...
    char token[64];
    sprintf(token, "你目前的个人点卷为：%d", point);
...
void saacproto_NewVipShop_recv(int fd, int point, char* buf, int flag)
{
    ...
    if (point == -1)
    {
        CHAR_talkToCli(charaindex, -1, "你的点卷不足！",  CHAR_COLORYELLOW);
        return;
    }
                               ...
                               )
{
    ...
    for (i = 0; i < playernum; i++)
    {
        if (CHAR_getCharUse(i) != FALSE)
        {
            ...
            lssproto_TK_send(fd, -1, data, rand() % CHAR_COLORNUM
#ifdef _ITEM_PET_TALK_
                             , petitemstr
#endif
#ifdef  _PLAYER_UID
...
#ifdef  _PLAYER_UID
void saacproto_ACPlayerUid_recv(int index, char* cdkey, char* name, char* uid)
{
    if (CHAR_CHECKINDEX(index))
    {
        if (strcmp(CHAR_getChar(index, CHAR_CDKEY), cdkey) == 0 && strcmp(CHAR_getChar(index, CHAR_NAME), name) == 0)
        {
            if (strlen(CHAR_getChar(index, CHAR_PLAYERUID)) < 1)
            {
                CHAR_setChar(index, CHAR_PLAYERUID, uid);

                if (CHAR_getWorkInt(index, CHAR_WORKFD) > -1)
                {
                    lssproto_PlayerUid_send(CHAR_getWorkInt(index, CHAR_WORKFD), uid);
                }
            }
        }
    }
...
void cleanFmPk()
{
    memset(fmpks, 0, sizeof(FamilyPKSchedule)*MAX_SCHEDULE * MAX_SCHEDULEMAN);
    int i = 0;

    for (i; i < MAX_SCHEDULE * MAX_SCHEDULEMAN; i++)
    {
        fmpks[i].flag = -1;
        fmpks[i].host_index = -1;
        fmpks[i].guest_index = -1;
...
#endif
#ifdef _NEW_STONEAGE_TYPE
void saacproto_NewCardUpdate_recv(char* cdkey, char* name, int online, int level, int face, int trans
    ...
    for (i = 0; i < playernum; i++)
    {
        if (CHAR_CHECKINDEX(i))
        {
            ...
            for (j = 0; j < ADDRESSBOOK_MAX; j++)
            {
                ...
                if (ae && ae->use == TRUE &&
                        strcmp(ae->cdkey, cdkey) == 0 &&
                        strcmp(ae->charname, name) == 0)
                {
                    ae->online = online;
                    ae->level = level;
                    ae->graphicsno = face;
                    ae->transmigration = trans;
#ifdef _PLAYERCARD_FAMILYNAME
