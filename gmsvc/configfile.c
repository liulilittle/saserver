...
typedef struct tagConfig
{
...
#ifdef FUNC_DU_PKCF
    unsigned int    Funcfstart;//ÅäÖÃÎÄ¼þ±äÁ¿
    char    Funcftalk[125];
#endif
    ...
    int battleallexpbase;
#endif
#ifdef  _COMBO_EXP
    char battlecomexpbase[64];
#endif
#ifdef  _OTHER_PETUP_LV
    int otherpetuplv;
#endif
#ifdef  _NOTOUZHI_BASE
    char notouzhibase[1024];
#endif
#ifdef  _RED_MEMOY_
    int redmoney;
#endif
    ...
    char nopoolitem1[256];
    char nopoolitem2[256];
#endif
    int playerpetlv;
...
{
    char    name[CHARNAMELEN];       /*xxxx±åØ¤Ð×ÔÂ°À*/
...
ReadConf readconf[] =
{
...
#ifdef FUNC_DU_PKCF
    { "Funcfstart", NULL, 0, (void*)&config.Funcfstart, INT},
    { "Funcftalk", config.Funcftalk, sizeof(config.Funcftalk), NULL, 0},
#endif
    ...
    { "CHARMAXLEVEL", NULL, 0, (void*)&config.charmaxlevel, INT },
#endif
#ifdef _POINT
...
#endif
#ifdef  _NO_ATTACK
    { "NOATTTYPE", NULL, 0, (void*)&config.noatttype, INT},
    { "NOATTTIME", NULL, 0, (void*)&config.noatttime, INT},
#endif
#ifdef  _NO_ENEMYID_GROUP
    ...
    { "BATTLERANDDEX2", NULL, 0, (void*)&config.battleranddex2,   INT},
#endif
    { "MANRENNUM", NULL, 0, (void*)&config.manrennum, INT},
    { "MANRENTIME", NULL, 0, (void*)&config.manrentime,   INT},
#ifdef  _BATTLE_ALL_EXP
    { "BATTLEALLEXPBASE", NULL, 0, (void*)&config.battleallexpbase,   INT},
#endif
#ifdef  _COMBO_EXP
    { "BATTLECOMEXPBASE", config.battlecomexpbase, sizeof(config.battlecomexpbase), NULL, 0},
#endif
#ifdef  _OTHER_PETUP_LV
    { "OTHERPETUPLV", NULL, 0, (void*)&config.otherpetuplv,   INT},
#endif
#ifdef  _NOTOUZHI_BASE
    { "NOTOUZHIBASE", config.notouzhibase, sizeof(config.notouzhibase), NULL, 0},
#endif
#ifdef  _RED_MEMOY_
    { "REDMONEY", NULL, 0, (void*)&config.redmoney,   INT},
#endif
    { "SELLLOGOUT", NULL, 0, (void*)&config.selllogout,   INT},
    { "PETUPTYPE", NULL, 0, (void*)&config.petuptype, INT},
#ifdef _UNIQUE_NAME
    { "UNIQUENAME",     NULL, 0, (void*)&config.uniquename,                INT},
#endif
#ifdef _NO_TEAMWARP_SKYLAND
    { "noskland", config.noskland, sizeof(config.noskland), NULL, 0},
#endif
#ifdef _NEW_FUNC_DECRYPT
    { "ALLOWERRORNUM2", NULL, 0, (void*)&config.allowerrornum2,   INT},
#endif
#ifdef _IOSTALK_
    { "iosTalkFlg",     NULL, 0, (void*)&config.iosTalkFlg,       INT},
#endif
#ifdef _MO_ADD_BOW_ACURATE
    { "BOWPVE", NULL, 0, (void*)&config.bowpve, INT },
#endif
#ifdef _NOLIMITEPOOL_
    { "NOPOOLITEM1", config.nopoolitem1, sizeof(config.nopoolitem1), NULL, 0 },
    { "NOPOOLITEM2", config.nopoolitem2, sizeof(config.nopoolitem2), NULL, 0 },
#endif
    { "PLAYERPETLV", NULL, 0, (void*)&config.playerpetlv, INT },
#ifdef _MO_REDUCE_PVP_COMBO
    { "COMBOPVP", NULL, 0, (void*)&config.pvpcomborate, INT },
#endif
#ifdef _SERVER_NP_
    { "SERVERNP", NULL, 0, (void*)&config.servernp, INT },
#endif // _SERVER_NP_
    { "battleexptime", NULL, 0, (void*)&config.battleexptime, INT },
};
...
#ifdef FUNC_DU_PKCF
unsigned int Funcfstart(void)  //º¯ÊýÎÄ¼þÃû
{
    return config.Funcfstart;
}

char* Funcftalk(void)
{
    return config.Funcftalk;
}
...
{
    int expnum = getlevelupexp(charaindex, level);
...
int getMaxLevel(int charaindex)
{
    if (CHAR_CHECKINDEX(charaindex))
    {
        ...
        {
            if (config.charmaxlevel == 0)
            {
                return config.maxlevel;
            }
            else
            {
                return config.charmaxlevel;
            }
        }
        else
        {
...
int getLuckStarTime()
{
    if (config.luckstartime < 0)
    {
        config.luckstartime = 0;
    }
...
int getStreetVendor(int id)
{
    ...
    char streetvendorpoint[16];
    memset(streetvendorpoint, 0, sizeof(streetvendorpoint));
...
{
    return config.noatttype;
}
int getNoAttTime(void)
{
    if (config.noatttime <= 0)
    {
        return 5;
    }

    return config.noatttime;
}
#endif
#ifdef  _NO_ENEMYID_GROUP
...
char* getFmMagicMpRate(int num)
{
    if (num <= 0)
    {
        return config.fmmagicmprate[0];
    }
    else
    {
        return config.fmmagicmprate[1];
    }
}
#endif
#ifdef _WAN_FIX
int getVsFlg(void)
{
    if (config.vsflg > 3 || config.vsflg < 0)
    {
        return 0;
    }
...
{
    char nosellpetid[32];
    memset(nosellpetid, 0, sizeof(nosellpetid));

    if (getStringFromIndexWithDelim(config.nosellpetid, ",", index + 1, nosellpetid, sizeof(nosellpetid)) == FALSE)
    {
        return -1;
    }
...
{
    return config.battleranddex2;
}
#endif
int getManRenNum(void)
{
    if (config.manrennum <= 60)
    {
        return 200;
    }
...
{
    if (config.manrentime <= 5)
    {
        return 5;
    }

    return config.manrentime;
}
#ifdef  _BATTLE_ALL_EXP
int getBattleAllExpBase(void)
{
    if (config.battleallexpbase < 0 || config.battleallexpbase > 100)
    {
        return 0;
    }

    return config.battleallexpbase;
}
#endif
#ifdef  _COMBO_EXP
int getBattleComExpBase(int index)
{
    ...
    char comexpbase[6];
    getStringFromIndexWithDelim(config.battlecomexpbase, ",", index, comexpbase, sizeof(comexpbase));
    return atoi(comexpbase);
}
#endif
#ifdef  _OTHER_PETUP_LV
int getOtherPetUpLv(void)
{
    if (config.otherpetuplv < 0 || config.otherpetuplv > 140)
    {
        return 0;
    }

    return config.otherpetuplv;
}
#endif
#ifdef  _NOTOUZHI_BASE
char* getNoTouzhiBase(void)
{
    return config.notouzhibase;
}
#endif
#ifdef  _RED_MEMOY_
int getRedMoney(void)
{
    if (config.redmoney <= 0)
    {
        return 0;
    }
    else
...
int getnoskland(unsigned int index)
{
    ...
    if (getStringFromIndexWithDelim(config.noskland, ",", index + 1, noskland, sizeof(noskland)) == TRUE)
    {
        return atoi(noskland);
    }
    else
    {
        return -1;
...
int getNotLimitedPool(int id)
{
    ...
    if (getStringFromIndexWithDelim(config.nopoolitem1, ",", id + 1, nopoolitem, sizeof(nopoolitem)) == TRUE)
    {
        return atoi(nopoolitem);
    }
    else
    {
        return -1;
...
int getNotLimitedPool2(int id)
{
    ...
    if (getStringFromIndexWithDelim(config.nopoolitem2, ",", id + 1, nopoolitem, sizeof(nopoolitem)) == TRUE)
    {
        return atoi(nopoolitem);
    }
    else
...
int getPlayerPetLv()
{
    if (config.playerpetlv <= 0)
    {
        return 5;
    }

    return config.playerpetlv;
}
#ifdef _MO_REDUCE_PVP_COMBO
int getCombPvP(void)
{
    if (config.pvpcomborate != 0)
    {
        return config.pvpcomborate;
    }
...
{
    if (config.servernp != 0)
    {
        return config.servernp;
    }
