...
char   *makeStringFromEscaped(char* src)
{
    ...
    while (1)
    {
        int utf8len = CopyOneUTF8Char(src[i], src[searchindex]);
...
char*  makeEscapeString(char* src, char* dest, int sizeofdest)
{
    ...
    for (i = 0 ; i < srclen ; i ++)
    {
...
#ifdef _UTF8
        int cpylen = 0;
        cpylen = CopyOneUTF8Char(&src[i], &dest[destindex]);
        ...
        {
            if (dest[destindex] == escapeChar[j].escapechar)
            {
                dirty = TRUE;
