...
/****************************** MACROS ******************************/
// The least significant unsigned char of the word is rotated to the end.
#define KE_ROTUINT(x) (((x) << 8) | ((x) >> 24))
...
void ccm_prepare_first_ctr_blk(unsigned char counter[], const unsigned char nonce[], int nonce_len, int payload_len_store_size);
void ccm_prepare_first_format_blk(unsigned char buf[], int assoc_len, int payload_len, int payload_len_store_size, int mac_len, const unsigned char nonce[], int nonce_len);
void ccm_format_assoc_data(unsigned char buf[], int *end_of_buf, const unsigned char assoc[], int assoc_len);
void ccm_format_payload_data(unsigned char buf[], int *end_of_buf, const unsigned char payload[], int payload_len);
...
// XORs the in and out buffers, storing the result in out. Length is in unsigned chars.
...
void increment_iv(unsigned char iv[], int counter_size)
{
    ...
    // Use counter_size unsigned chars at the end of the IV as the big-endian integer to increment.
...
// Input may be an arbitrary length (in unsigned chars).
void aes_encrypt_ctr(const unsigned char in[], size_t in_len, unsigned char out[], const unsigned int key[], int keysize, const unsigned char iv[])
{
    ...
    aes_encrypt(iv_buf, out_buf, key, keysize);
    xor_buf(out_buf, &out[idx], in_len - idx);   // Use the Most Significant unsigned chars.
}
...
int aes_encrypt_ccm(const unsigned char payload[], unsigned int payload_len, const unsigned char assoc[], unsigned short assoc_len,
                    const unsigned char nonce[], unsigned short nonce_len, unsigned char out[], unsigned int *out_len,
                    unsigned int mac_len, const unsigned char key_str[], int keysize)
{
    unsigned char temp_iv[AES_BLOCK_SIZE], counter[AES_BLOCK_SIZE], mac[16], *buf;
    ...
    increment_iv(temp_iv, AES_BLOCK_SIZE - 1 - mac_len);   // Last argument is the unsigned char size of the counting portion of the counter block. /*BUG?*/
...
int aes_decrypt_ccm(const unsigned char ciphertext[], unsigned int ciphertext_len, const unsigned char assoc[], unsigned short assoc_len,
                    const unsigned char nonce[], unsigned short nonce_len, unsigned char plaintext[], unsigned int *plaintext_len,
                    unsigned int mac_len, int *mac_auth, const unsigned char key_str[], int keysize)
{
    unsigned char temp_iv[AES_BLOCK_SIZE], counter[AES_BLOCK_SIZE], mac[16], mac_buf[16], *buf;
    ...
    *plaintext_len = ciphertext_len - mac_len;
    ...
    memcpy(temp_iv, counter, AES_BLOCK_SIZE);
    increment_iv(temp_iv, AES_BLOCK_SIZE - 1 - mac_len);   // (AES_BLOCK_SIZE - 1 - mac_len) is the unsigned char size of the counting portion of the counter block.
...
// Creates the first counter block. First unsigned char is flags, then the nonce, then the incremented part.
void ccm_prepare_first_ctr_blk(unsigned char counter[], const unsigned char nonce[], int nonce_len, int payload_len_store_size)
{
...
void ccm_prepare_first_format_blk(unsigned char buf[], int assoc_len, int payload_len, int payload_len_store_size, int mac_len, const unsigned char nonce[], int nonce_len)
{
    // Set the flags for the first unsigned char of the first block.
...
void aes_key_setup(const unsigned char key[], unsigned int w[], int keysize)
{
    ...
    for (idx = Nk; idx < Nb * (Nr + 1); ++idx)
    {
        ...
        if ((idx % Nk) == 0)
        {
            temp = SubWord(KE_ROTUINT(temp)) ^ Rcon[(idx - 1) / Nk];
        }
...
// Performs the AddRoundKey step. Each round has its own pre-generated 16-unsigned char key in the
...
// Performs the SubBytes step. All unsigned chars in the state are substituted with a
...
void aes_encrypt(const unsigned char in[], unsigned char out[], const unsigned int key[], int keysize)
{
    ...
    // Copy input array (should be 16 unsigned chars long) to a matrix (sequential unsigned chars are ordered
...
int aesEncrypt(char *src, int len, char *enc, char *key)
{
    ...
    for (i = 0; i < block; i++)
    {
        aes_encrypt((unsigned char *)(src + (i << 4)), (unsigned char *)(enc + (i << 4)), key_schedule, 256);
    }
...
int aesDecrypt(char *src, int len, char *dec, char *key)
{
    ...
    for (i = 0; i < block; i++)
    {
        aes_decrypt((unsigned char *)(src + (i << 4)), (unsigned char *)(dec + (i << 4)), key_schedule, 256);
    }
