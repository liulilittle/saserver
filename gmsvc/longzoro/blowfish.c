...
/**************************** VARIABLES *****************************/
static const unsigned int p_perm[18] =
{
...
static const unsigned int s_perm[4][256] = { {
...
void blowfish_key_setup(const unsigned char user_key[], BLOWFISH_KEY *keystruct, size_t len)
{
    ...
    // Combine the key with the P box. Assume key is standard 448 bits (56 unsigned chars) or less.
...
int blowfishEncrypt(char *src, char *enc, int len, char *key, int lkey)
{
    ...
    block = len >> 3;
    blowfish_key_setup((unsigned char *)key, &fkey, lkey);

    for (i = 0; i < block; i++)
    {
        blowfish_encrypt((unsigned char *)(src + (i << 3)), (unsigned char *)(enc + (i << 3)), &fkey);
    }
...
int blowfishDecrypt(char *src, char *dec, int len, char *key, int lkey)
{
    ...
    for (i = 0; i < block; i++)
    {
        blowfish_decrypt((unsigned char *)(src + (i << 3)), (unsigned char *)(dec + (i << 3)), &fkey);
    }
