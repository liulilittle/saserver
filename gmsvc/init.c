...
BOOL init(int argc, char** argv, char** env)
{
    ...
    print("读取deskey文件：deskey.txt\n");
    GOTORETURNFALSEIFFALSE(readDesKey("deskey.txt"));
    print("读取desrand文件：desrand.txt\n");
    GOTORETURNFALSEIFFALSE(readDesRand("desrand.txt"));
    nice(getrunlevel());
    //ttom start
    {
...
//      if( !ATTMAGIC_initMagic( getMagicfile() ) )
    ...
    */
    ...
    /*  夫弘奶件邰菲毛请允  */
    {
        saacproto_ACServerLogin_send(acfd, getServernumber(), getGameservername(), getAccountserverpasswd());
    }
    print("完成\n");
