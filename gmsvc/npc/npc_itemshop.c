...
void NPC_GetItemList(char *argstr, char *argtoken)
{
    ...
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "CostFame", buff3, sizeof(buff3)) == NULL)
    {
        iCostFame = -1;
    }

    if (NPC_Util_GetStrFromStrWithDelim(argstr, "ChangeItemCost", buff5, sizeof(buff5)) == NULL)
    {
        iChangeItemCost = -1;
    }
    ...
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "ItemList", buff, sizeof(buff)) != NULL)
    {
        while (getStringFromIndexWithDelim(buff, ",", i, buff2, sizeof(buff2)) != FALSE)
        {
            if (iCostFame > -1)
            {
                if (getStringFromIndexWithDelim(buff3, ",", i, buff4, sizeof(buff4)) != FALSE)
                {
            ...
            {
                if (getStringFromIndexWithDelim(buff5, ",", i, buff6, sizeof(buff6)) != FALSE)
                {
...
BOOL NPC_SetNewItem(int meindex, int talker, char *data)
{
    ...
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "ItemList", buf, sizeof(buf)) != NULL)
    {
        while (getStringFromIndexWithDelim(buf, ",", j, buff2, sizeof(buff2)) != FALSE)
        {
            if (iCostFame > -1)
            {
                if (getStringFromIndexWithDelim(buff3, ",", j, buff4, sizeof(buff4)) != FALSE)
                {
                    iCostFame = atoi(buff4);
                }
            }
            ...
            if (iChangeItemCost > -1)
            {
                if (getStringFromIndexWithDelim(buff5, ",", j, buff6, sizeof(buff6)) != FALSE)
                {
                    iChangeItemCost = atoi(buff6);
                }
            }
...
BOOL NPC_AddItemBuy(int meindex, int talker, int itemID, int kosuu, double rate, int iCostFame, int iChangeItemCost)
{
    ...
    {
        CHAR_newMessageToCli(talker, -1, "您的石币不足，无法购买此物品。", CHAR_COLORYELLOW);
        return FALSE;
    }
    ...
    if (iCostFame > 0)
    {
        ...
        {
            CHAR_newMessageToCli(talker, -1, "您的声望不足，无法购买此物品。", CHAR_COLORYELLOW);
            return FALSE;
        }
    }
    ...
    */
    ...
    if (iTotalCostFame > 0)
    {
        ...
        sprintf(token, "扣除[%d]声望", iTotalCostFame / 100);
        CHAR_newMessageToCli(talker, -1, token, CHAR_COLORWHITE);
    ...
    {
        sprintf(token, "扣除[%d]石币", maxgold);
        CHAR_newMessageToCli(talker, -1, token, CHAR_COLORWHITE);
    }

    sprintf(token, "获得%d个[%s]", kosuu, ITEM_getNameFromNumber(itemID));
    CHAR_newMessageToCli(talker, -1, token, CHAR_COLORWHITE);
	for (i = 9; i <= 22; i++)
	{
		int fromindex = CHAR_getItemIndex(talker, i);
		if (ITEM_CHECKINDEX(fromindex))
		{
			if (ITEM_getInt(fromindex, ITEM_CANBEPILE) == 1)
			{
				int frompilenum = ITEM_getInt(fromindex, ITEM_USEPILENUMS);
				if (frompilenum < CHAR_getMyMaxPilenum(talker))
				{
					int b;
					for (b = i + 1; b <= 23; b++)
					{
						int toindex = CHAR_getItemIndex(talker, b);
						if (ITEM_CHECKINDEX(toindex))
						{
							if (ITEM_getInt(toindex, ITEM_USEPILENUMS) < CHAR_getMyMaxPilenum(talker))
							{
								if (ITEM_getInt(fromindex, ITEM_ID) == ITEM_getInt(toindex, ITEM_ID) && strcmp(ITEM_getChar(fromindex, ITEM_NAME), ITEM_getChar(toindex, ITEM_NAME)) == 0)
								{
									int itemlocked = 0;
									if (ITEM_getInt(fromindex, ITEM_LOCKED) == 1 || ITEM_getInt(toindex, ITEM_LOCKED) == 1)
									{
										itemlocked = 1;
									}
									CHAR_PileItemFromItemBoxToItemBox(talker, b, i);
									if (itemlocked == 1)
									{
										ITEM_setInt(fromindex, ITEM_LOCKED, 1);
									}
									frompilenum = ITEM_getInt(fromindex, ITEM_USEPILENUMS);
									if (frompilenum >= CHAR_getMyMaxPilenum(talker))
									{
										break;
									}
	...
	for (i = 9; i <= 23;i++)
	{
		...
		if (!ITEM_CHECKINDEX(itemindex))
		{
			...
			for (j = i + 1; j <= 23; j++)
			{
				int itemindex2 = CHAR_getItemIndex(talker, j);
				if (ITEM_CHECKINDEX(itemindex2))
				{
					CHAR_moveEquipItem(talker, j, i);
					break;
				}
			}
		}
...
void NPC_ItemShop_SellMain(int meindex, int talker, int before)
{
    ...
    /*if(CHAR_getInt(talker, CHAR_LOCKED)==1){
        ...
        return;
    }else if(CHAR_getInt(talker, CHAR_LOCKED)==2){
        char message[256];
        char    buf[256];
...
BOOL NPC_SellNewItem(int meindex, int talker, char *data)
{
    ...
    if (select < CHAR_STARTITEMARRAY || select >= CHAR_MAXITEMHAVE)
    {
        return FALSE;
    }
    ...
    sprintf(token, "得到[%d]石币", cost * sellnum);
    CHAR_newMessageToCli(talker, -1, token, CHAR_COLORWHITE);
    CHAR_send_P_StatusString(talker, CHAR_P_STRING_GOLD);
    return TRUE;
}
...
void NPC_ExpressmanCheck(int meindex, int talker)
{
    ...
    sprintf(token, "%s|%s|2|打工|交付行李", CHAR_getChar(meindex, CHAR_NAME), buf);
    lssproto_WN_send(fd,
                     WINDOW_MESSAGETYPE_NEWSELECT,
