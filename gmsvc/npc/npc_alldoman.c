    ...
    switch (seqno)
    {
        ...
        case NPC_ALLDOMAN_SELECT_WND:
            if (atoi(data) == 2)
            {
                ...
                int time = CHAR_getInt(talkerindex, CHAR_YX_TIME2) - CHAR_getInt(talkerindex, CHAR_YX_TIME1);

                if (time <= 0)
                {
                    CHAR_setWorkInt(talkerindex, CHAR_WORKHEROFLOOR, 0);
                    CHAR_setInt(talkerindex, CHAR_YX_TIME1, 0);
                    CHAR_setInt(talkerindex, CHAR_YX_TIME2, 0);
                    CHAR_talkToCli(talkerindex, -1, "这里只能给真正的英雄刻写！", CHAR_COLORRED);
                    break;
                }
...
static void NPC_Alldoman_selectWindow(int meindex, int talker, int num, char *data)
{
    ...
    switch (num)
    {
        ...
        case 6:
            ...
            sprintf(token2, "　　　    姓名　　　　头衔　　使用时间(分)　  等级　　转生\n");
            ...
            for (i = 0 + (showpage - 1) * 15; i < showpage * 15; i++)
            {
                ...
                sprintf(token, "%16s　%13s　%7d　%7d　%7d\n",
                        ...
                        (int)(atoi(Herolist[sort[MAX_HERO_LIST - 1 - i][1]][5]) / 60),
    ...
    while (linenum < 100)
    {
        for (i = 0; i < MAX_HERO_COLUMN; i++)
        {
            if (getStringFromIndexWithDelim(token, "|", i + 1, Herolist[linenum][i], sizeof(Herolist[linenum][i])) == FALSE)
            {
                ...
                {
                    sort[i][0] = atoi(Herolist[i][5]);
                    sort[i][1] = i;

                    if (sort[i][0] != 0 && sort[i][0] != -1 && sort[i][0] != 1479702065)
                    {
                        countnum++;
                    }
                }
                               ...
                               char *nname, char *title, int level, int trns, int floor, int time)
{
    ...
    for (i = 0; i < MAX_HERO_LIST; i++)
    {
        ...
        if (sort[i][0] == 0 || sort[i][0] == -1)
        {
            strcpy(Herolist[i][5], "1479702065");
            sort[i][0] = 1479702065;
        }
    ...
    //for (i = 0; i < MAX_HERO_LIST; i++)
    //  printf("sort0=%d sort1=%d i=%d time=%s\n", sort[i][0], sort[i][1], i, Herolist[sort[i][1]][0]);
    totalpage = countnum / 15;
