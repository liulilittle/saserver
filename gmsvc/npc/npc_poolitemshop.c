...
void NPC_DepotItem_MakeItemString(int meindex, int talkerindex, char *retstring, int retstringlen)
{
    ...
    for (i = CHAR_STARTITEMARRAY; i < CHAR_MAXITEMHAVE; i ++)
    {
        ...
        int itemID = ITEM_getInt(itemindex, ITEM_ID);
        int Flg = FALSE;
        int h;
        int itemid1;
...
BOOL NPC_DepotItem_InsertItem(int meindex, int talkerindex, int num)
{
    ...
    if (arg[0] == '*')
    {
        CHAR_talkToCli(talkerindex, -1, "绑定的物品无法存入大仓库，请妥善保管。", CHAR_COLORYELLOW);
        return  FALSE;
    }
    ...
    if (ITEM_getInt(itemindex, ITEM_ID) == 18543)
    {
        CHAR_talkToCli(talkerindex, -1, "英雄的祝福无法存入大仓库，请妥善保管。", CHAR_COLORYELLOW);
        return  FALSE;
    }
...
BOOL NPC_DepotItem_gettItem(int meindex, int talkerindex, int num)
{
    ...
    if (ITEM_getInt(itemindex, ITEM_USETIME) > 0)
    {
        ...
        {
            ITEM_endExistItemsOne(itemindex);
            CHAR_talkToCli(talkerindex, -1, "您的道具已过期，系统自动清除。", CHAR_COLORRED);
            return TRUE;
        }
    }
...
static BOOL NPC_PoolItemShop_PoolItem(int meindex, int talkerindex, int num)
{
    ...
    if (ITEM_getInt(itemindex, ITEM_ID) >= 19625 && ITEM_getInt(itemindex, ITEM_ID) <= 19628)
    {
        CHAR_talkToCli(talkerindex, -1, "该道具不能强存!", CHAR_COLORYELLOW);
        return FALSE;
    }
