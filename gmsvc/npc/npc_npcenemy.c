                        ...
                        int color)
{
    ...
    if (NPC_Util_GetStrFromStrWithDelim(npcarg, "FULLITEMMSG", buf, sizeof(buf)) != NULL)
    {
        int emptyitemindexinchara = CHAR_findEmptyItemBox(talkerindex);

        if (emptyitemindexinchara < 0)
        {
            CHAR_talkToCli(talkerindex, -1, buf, CHAR_COLORYELLOW);
            return;
        }
    }
...
int NPC_NPCEnemy_Dying(int battleindex, int meindex)
{
    ...
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "herobattlefield", buf, sizeof(buf)) != NULL)
    {
        ...
        if (atoi(buf) == 1)
        {
            CHAR_setInt(charaindex, CHAR_YX_TIME1, time(NULL));
            //printf("1层 %d  %s\n", CHAR_getInt(charaindex, CHAR_YX_TIME1), CHAR_getChar(charaindex, CHAR_NAME));
        }
        else if (atoi(buf) == 132)
        {
            CHAR_setInt(charaindex, CHAR_YX_TIME2, time(NULL));
            //printf("132层 %d  %s\n", CHAR_getInt(charaindex, CHAR_YX_TIME2), CHAR_getChar(charaindex, CHAR_NAME));
        }
    ...
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "EventID", buf, sizeof(buf)) != NULL)
    {
        for (i = 0; i < 5; i++)
        {
            int toindex = BattleArray[battleindex].Side[0].Entry[i].charaindex;
            ...
            if (CHAR_CHECKINDEX(toindex))
            {
                ...
                if (getStringFromIndexWithDelim(buf, "-", 1, buf1, sizeof(buf1)) != FALSE)
                {
                    ...
                    if (NPC_NowEventCheckFlg(toindex, eveno) == TRUE)
                    {
                        NPC_ActionEventID(toindex, buf, 1);
                    }
                }
            }
        }
...
BOOL NPC_NPCEnemy_BattleIn(int meindex, int charaindex)
{
    ...
    if (ret == 0)
    {
        ...
        if (NPC_Util_GetStrFromStrWithDelim(argstr, "randshuxing", buf, sizeof(buf)) != NULL)
        {
            if (atoi(buf) == 1)
            {
                int i;
                ...
                {
                    int tempcharaindex = BattleArray[CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEINDEX)].Side[1].Entry[i].charaindex;

                    if (CHAR_CHECKINDEX(tempcharaindex) == FALSE)
                    {
                        continue;
                    }

                    int TypeOne = RAND(1, 4);
                    int TypeTwo = RAND(1, 4);
                    int Type[4] = { 0, 0, 0, 0 };

                    if (TypeOne + 2 == TypeTwo || TypeOne - 2 == TypeTwo)
                    {
                        if (RAND(1, 100) <= 50)
                        {
                            TypeTwo = TypeOne + 1;
                    ...
                    {
                        Type[TypeOne - 1] = RAND(1, 2) * 10;
                        Type[TypeTwo - 1] = 100 - Type[TypeOne - 1];
                    }

                    CHAR_setInt(tempcharaindex, CHAR_EARTHAT, Type[0]);
                    CHAR_setInt(tempcharaindex, CHAR_WATERAT, Type[1]);
                    CHAR_setInt(tempcharaindex, CHAR_FIREAT, Type[2]);
                    CHAR_setInt(tempcharaindex, CHAR_WINDAT, Type[3]);
                }
            }
        }
    }
