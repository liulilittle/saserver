...
void NPC_ManorSmanTalked(int meindex, int talkerindex, char *msg, int color)
{
    ...
    */
    ...
    if (strlen(CHAR_getWorkChar(meindex, CHAR_NPCWORKCHAR1)) > 0)
        ...
        {
            //sprintf(servernamebuf,"此庄园争夺战只能在[%s]进行，请到该线路申请！",okservername[CHAR_getWorkInt(meindex, NPC_WORK_ID)-1]);
            sprintf(servernamebuf, "此庄园争夺战只能在[%s]进行，请到该线路申请！", CHAR_getWorkChar(meindex, CHAR_NPCWORKCHAR1));
            lssproto_WN_send(fd, WINDOW_MESSAGETYPE_MESSAGE,
                             WINDOW_BUTTONTYPE_YESNO,
                             -1,
                             -1,
...
else if (strlen(CHAR_getWorkChar(meindex, CHAR_NPCWORKCHAR2)) > 0)
{
    ...
    {
        //sprintf(servernamebuf,"此庄园争夺战不能在[%s]进行，请去其他线路申请！",noservername[CHAR_getWorkInt(meindex, NPC_WORK_ID)-1]);
        sprintf(servernamebuf, "此庄园争夺战不能在[%s]进行，请去其他线路申请！", CHAR_getWorkChar(meindex, CHAR_NPCWORKCHAR2));
        lssproto_WN_send(fd, WINDOW_MESSAGETYPE_MESSAGE,
                         WINDOW_BUTTONTYPE_YESNO,
                         -1,
                         -1,
...
//extern Fmpknub_t Fmpknub[100];// 需要外部调用时间在开放最大家族数量好像是60 一会看看那里控制
    ...
    */
    ...
    switch (seqno)
    {
                        ...
                        if (hadfmindex - 1 != tkfmindex)
                        {
                            ...
                            if (check == 0)
                            {
                                ...
                                if (won1 == 1)
                                {
                                    // WON END
#ifdef _NEW_MANOR_LAW
                                    sprintf(buf, "禁止连续恶意保庄下保护战书\n"
                                            "严禁族战场内任何形式影响其他人正常族战\n"
                                            "禁止恶意拖延族战\n"
                                            "下书家族最少有10个人参加族战，不满足10人参战的家族请不要下书\n"
                                            "你确定要挑战这个庄园吗？");
#else
                                ...
                                {
#ifdef _NEW_MANOR_LAW
                                    sprintf(buf, "你的家族气势尚未到达庄园家族气势的一成，请再加油。");
#else
#ifdef _MANOR_PKRULE
#ifdef _FIX_FAMILY_PK_LIMIT    // WON ADD 将可挑战庄园的由前十大改为前三十大
        ...
        case CHAR_WINDOWTYPE_FAMILYMAN_PKNUBER1:
            ...
            //没有按钮选择 是直接选择的列表行数据的 会进入这里执行
            if (select == WINDOW_BUTTONTYPE_NONE)
            {
                switch (atoi(data))
                {
                    //计算选择的时间有选择编号回来重新计算时间
                    case 1:
...
#else
                    memcpy(&tm1, localtime((time_t *)&NowTime.tv_sec), sizeof(tm1));
                    fmpks[fmpks_pos + 1].dueltime = CHAR_getWorkInt(meindex, NPC_WORK_CHALLENGEWAIT) + NowTime.tv_sec - tm1.tm_min * 60 + 1800 ; // Arminius 11.1 改成一律 xx:30 开打+ 3600 * 24
            ...
            if (select == WINDOW_BUTTONTYPE_NONE)
            {
                ...
                */
                ...
                switch (atoi(data))
                {
                    ...
                    case 5:
                        dueltime = NowTime.tv_sec - tm1.tm_min * 60 + ((24 - tm1.tm_hour) * 3600) + 3600 * 22 ;//山猪修改挑战时间
                        break;
        ...
        case FMPKS_FLAG_NONE:
#ifdef _NEW_MANOR_LAW
            {
                ...
                if (hadfmindex != -1)
                {
                    ...
                    fmpks[fmpks_pos + 1].dueltime = NowTime.tv_sec - tm1.tm_min * 60 - tm1.tm_hour * 3600 + 3600 * 22 ; //山猪修改挑战时间今天下 明天休战 后天打 大后天下书
...
#ifndef _NEW_MANOR_LAW
                fmpks[fmpks_pos + 1].dueltime = CHAR_getWorkInt(meindex, NPC_WORK_PEACEWAIT) + NowTime.tv_sec ; //山猪修改挑战时间
#else
...
#else
                fmpks[fmpks_pos + 1].dueltime = NowTime.tv_sec - tm1.tm_min * 60 + iOffsetTime + (18 * 3600); //休战时间1天 后天打 大后天下书+3600 * 24
#endif
                //#else
...
#endif
            fmpks[fmpks_pos].flag = FMPKS_FLAG_NONE;
            fmpks[fmpks_pos + 1].dueltime = CHAR_getWorkInt(meindex, NPC_WORK_PEACEWAIT) + NowTime.tv_sec; //山猪查看
            fmpks[fmpks_pos + 1].flag = FMPKS_FLAG_NONE;
            NPC_CleanPkList(fmpks_pos);
#ifndef _NEW_MANOR_LAW
            NPC_ManorSavePKSchedule(meindex, -1, FMPKS_FLAG_NONE);
...
#endif
{
    ...
    {
#ifndef _NEW_MANOR_LAW
#ifdef _DUELTIME24HOUR//延迟开战时间24小时
    ...
    else
    {
        ...
        if (tm1.tm_hour < 10)
        {
            dueltime = NowTime.tv_sec - tm1.tm_min * 60 - (tm1.tm_hour * 3600) + 3600 * tm2.tm_hour + tm2.tm_min * 60 + 3600 * 24; //写入族长玩家 记录
#ifndef _NO_WAIGUA
        ...
        {
            dueltime = NowTime.tv_sec - tm1.tm_min * 60 + ((24 - tm1.tm_hour) * 3600) + 3600 * tm2.tm_hour + tm2.tm_min * 60 + 3600 * 24; //写入族长玩家 记录
#ifndef _NO_WAIGUA
