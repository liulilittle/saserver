...
//                print("CHAR_FMNAME: %s\n",fmpks[fmpks_pos+i].host_name);
                                            ...
                                            int playnum = FreeGetFamilyBattleNum(CHAR_getWorkInt(meindex, NPC_WORK_ID));

                                            if (playnum > 0)
                                            {
                                                fmpks[fmpks_pos + i].max_player = playnum;    // default
                                            }
                                            else
                                            {
...
#ifdef _FAMILYBATTLENUM_
                                            extern BOOL FreeGetFamilyBattleNum(int id);
                                            a = FreeGetFamilyBattleNum(CHAR_getWorkInt(meindex, NPC_WORK_ID));
...
#ifdef _FAMILYBATTLENUM_
            extern BOOL FreeGetFamilyBattleNum(int id);
            maxnum = FreeGetFamilyBattleNum(CHAR_getWorkInt(meindex, NPC_WORK_ID));
#else
