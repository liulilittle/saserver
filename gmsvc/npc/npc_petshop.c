...
int NPC_PetDel(int meindex, int toindex, int select, char* token);
    ...
    switch (num)
    {
        ...
        case 1:
            ...
            /*if(CHAR_getInt(toindex, CHAR_LOCKED)==1){
                ...
                return;
            }else if(CHAR_getInt(toindex, CHAR_LOCKED)==2){
                char message[256];
                char    buf[256];
        ...
        case 3:
            ...
            {
                if (NPC_PetDel(meindex, toindex, select, token) == -1)
                {
                    windowno = -1;
                }
                else
...
#ifdef  _NEW_PETDEPOT
            windowtype = WINDOW_MESSAGETYPE_PETDEPOTSELECT;
#else
            windowtype = WINDOW_MESSAGETYPE_SELECT;
#endif
...
//  }
    ...
    switch (seqno)
    {
        ...
        case CHAR_WINDOWTYPE_WINDOWPETSHOP_NOWND:
            if (select == WINDOW_BUTTONTYPE_OK)
            {
                if (strlen(data) < 1)
                {
                    CHAR_talkToCli(talkerindex, -1, "确认码输入错误，请重新输入确认码，确认码为括号内的三位数字。", CHAR_COLORYELLOW);
                    return;
                }
                ...
                if (rnd != CHAR_getWorkInt(talkerindex, CHAR_NPCWORKINT1))
                {
                    CHAR_talkToCli(talkerindex, -1, "确认码输入错误，请重新输入确认码，确认码为括号内的三位数字。", CHAR_COLORYELLOW);
                    return;
                }

                int petindex = CHAR_getCharPet(talkerindex, CHAR_getWorkInt(talkerindex, CHAR_NPCWORKINT2));

                if (CHAR_CHECKINDEX(petindex) == 1)
                {
                    CHAR_setWorkInt(petindex, CHAR_NPCWORKINT1, 999);
                    CHAR_setWorkInt(talkerindex, CHAR_WORKSHOPRELEVANT, CHAR_getWorkInt(talkerindex, CHAR_NPCWORKINT2) + 1);
                    char tempbuff[1024];
                    NPC_PetDel(meindex, talkerindex, 1, tempbuff);
                }
...
#ifdef _NoSellPet
int NoSellPet(int meindex, int talker, int petsel)
{
    ...
    if (CHAR_getWorkInt(petindex, CHAR_NPCWORKINT1) == 999)
    {
        return 1;
    }
    ...
    int ylv = 1;
#ifdef _PETCOM_
    ylv = CHAR_getInt(petindex, CHAR_YLV) ;
#else
#ifdef  _PET_LV1_4V
    ...
    if ((CHAR_getInt(petindex, CHAR_LV) - ylv > 0) && CHAR_getInt(petindex, CHAR_LV) > 100)
    {
        int petpopint = CHAR_getWorkInt(petindex, CHAR_WORKMAXHP) / 4 + CHAR_getWorkInt(petindex, CHAR_WORKATTACKPOWER) + CHAR_getWorkInt(petindex, CHAR_WORKDEFENCEPOWER) + CHAR_getWorkInt(petindex, CHAR_WORKQUICK);
        char token[512];
        int rndnum = RAND(101, 999);
        memset(token, 0, sizeof(token));
        sprintf(token, "[style c=1 s=14]一旦卖出无法找回[/style] [style c=5 s=14]请仔细确认以下信息[/style]\n您要卖出的宠物超过100级且非野生宠物\n------------------------------------\n  卖出宠物 [[style c=4 s=14]%s[/style]][评分:%d]\n"
                "  四维成长[%d、%d、%d、%d]"
                "\n------------------------------------\n   确定操作请输入确认码（[style c=5 s=14]%d[/style]）"
                ...
                , rndnum);
        CHAR_setWorkInt(talker, CHAR_NPCWORKINT1, rndnum);
        CHAR_setWorkInt(talker, CHAR_NPCWORKINT2, petsel);
        CHAR_setWorkInt(talker, CHAR_NPCWORKINT14, meindex);
        lssproto_WN_send(getfdFromCharaIndex(talker), WINDOW_MESSAGETYPE_MESSAGEANDLINEINPUT,
                         WINDOW_BUTTONTYPE_OKCANCEL,
                         CHAR_WINDOWTYPE_WINDOWPETSHOP_NOWND,
                         CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX),
                         token);
        return 0;
    }
...
int NPC_PetDel(int meindex, int talker, int select, char* token)
{
    int petindex;
    int petsel;
    char szPet[128];
    ...
    /*if(strstr(petnewname,"*")!=NULL){
        CHAR_talkToCli( talker, -1, "【宠物保护系统】该宠自定义名称中含有(*) 无法丢弃、卖店、交易、摆摊 如需操作请修改名字.", CHAR_COLORYELLOW);
        return;
    }*/
    if (CHAR_getInt(petindex, CHAR_LOCKED) > 0)
    {
        CHAR_talkToCli(talker, -1, "绑定宠物无法贩卖！", CHAR_COLORYELLOW);
        return 0;
    }
    ...
    if (strlen(petnewname) > 0)
        ...
        {
            CHAR_talkToCli(talker, -1, "[宠物保护系统]有昵称的100级以上宠物无法卖入商店，如需操作请把宠物名称改为原名称.", CHAR_COLORYELLOW);
            return 0;
        }
    ...
    for (i = 0; i < 64; i++)
    {
        if (getNoSellPetId(i) == petid || petid >= 3000)
        {
            CHAR_talkToCli(talker, -1, "[宠物保护系统]此宠物为珍稀宠物，不可贩卖。", CHAR_COLORYELLOW);
            return 0;
        }
    }

    /*if (FreeSellPet(meindex, talker, petsel) == 0){
        return -1;
    }*/
#ifdef _NoSellPet

    if (NoSellPet(meindex, talker, petsel) == 0)
    {
        return -1;
    }
    ...
    char tmpbuff[128];
    sprintf(tmpbuff, "您卖出了[%s][LV:%d]宠物一只。", CHAR_getChar(petindex, CHAR_NAME), CHAR_getInt(petindex, CHAR_LV));
...
#ifdef _PET_GOLD_
    char *petdata = 0;
    extern BOOL sasql_Pet_Gold(char *data);

    if (cost < 0)
    {
        cost = 100000;
        char *petdata = CHAR_makePetStringFromPetIndex(petindex);
        sasql_Pet_Gold(petdata);
    }
...
void NPC_PetDel2(int meindex, int talker, int select, char *token)
{
    ...
    char depotbuff[128];
    sprintf(depotbuff, "您已经将宠物[%s][LV:%d]存入人物仓库。", CHAR_getChar(petindex, CHAR_NAME), CHAR_getInt(petindex, CHAR_LV));
...
#endif
    char depotbuff[128];
    sprintf(depotbuff, "您已经将人物仓库中的宠物[%s][LV:%d]取回。", CHAR_getChar(petindex, CHAR_NAME), CHAR_getInt(petindex, CHAR_LV));
    CHAR_talkToCli(talker, -1, depotbuff, CHAR_COLORYELLOW);
#ifdef _SAMETHING_SAVEPOINT
...
#define     LINENUM     5
    ...
    *buttontype = WINDOW_BUTTONTYPE_CANCEL;
    ...
    //strcat(token,"名称             等级  HP  攻击  防御  敏捷");
...
void NPC_DepotPet_Add(int meindex, int talker, int select, char *token)
{
    ...
    {
        char depotbuff[128];
        sprintf(depotbuff, "您已经将宠物[%s][LV:%d]存入公共仓库。", CHAR_getChar(petindex, CHAR_NAME), CHAR_getInt(petindex, CHAR_LV));
        CHAR_talkToCli(talker, -1, depotbuff, CHAR_COLORYELLOW);
        LogPet(
            CHAR_getChar(talker, CHAR_NAME),   /* 平乓仿   */
            CHAR_getChar(talker, CHAR_CDKEY),
...
#endif
    char depotbuff[128];
    memset(depotbuff, 0, sizeof(depotbuff));
    sprintf(depotbuff, "您已经将公共仓库中的宠物[%s][LV:%d]取回。", CHAR_getChar(petindex, CHAR_NAME), CHAR_getInt(petindex, CHAR_LV));
    CHAR_talkToCli(talker, -1, depotbuff, CHAR_COLORYELLOW);
#ifdef _SAMETHING_SAVEPOINT
...
int NPC_DepotPet_CheckLevel(int meindex, int talker, int select, char *token)
{
    ...
    if (CHAR_getInt(talker, CHAR_TRANSMIGRATION) > 0
            || CHAR_getInt(talker, CHAR_LV) + getPlayerPetLv() >= CHAR_getInt(petindex, CHAR_LV))
    {
        return TRUE;
    }
