    ...
    if (CHAR_getWorkInt(meindex, NPC_WORK_MODEFLAG) == NPC_STATEFREE)
    {
        ...
        if ((fmpks[fmpks_pos].prepare_time > 0) && (fmpks[fmpks_pos].flag == FMPKS_FLAG_SCHEDULED))
        {
            ...
            if ((tm1.tm_min >= clock) && (fmpks[fmpks_pos].prepare_time > 0))
            {
                ...
                FmBattlePointFunction(CHAR_getWorkInt(meindex, NPC_WORK_ID), 1);
#endif
#ifdef _FMPOINT_PK_MESSAGE
                char fmpointname[][16] = {"萨姆吉尔庄园", "玛丽娜丝庄园", "加    加庄园", "卡鲁它那庄园", "伊 甸 园庄园", "塔 尔 塔庄园", "尼 克 斯庄园", "弗 烈 顿庄园", "亚 依 欧庄园", "瑞尔亚斯庄园"};
                char token[256];
                sprintf(token, "庄园争夺战开始咯,【%s】『%s ＶＳ %s』，大家速度来族战线渔村医院观战哟。",
                        fmpointname[CHAR_getWorkInt(meindex, NPC_WORK_ID) - 1],
                ...
                if (meid > 0 && meid <= MANORNUM) // CoolFish 2002/2/25 Change MANOR -> MANORNUM
                {
                    ...
                    if (winflag == 1)
                    {
                        ...
                        Logfmpk(
                            ...
                            fmpks[fmpks_pos].guest_index,
                            num2, token, "", "", 2);
                        int fmpknum = sasql_queryFmPointData(meid, 1);

                        if (fmpknum > -1)
                        {
                            sasql_updateFmPointData(meid, 0, fmpknum + 1, 1);
                        }
...
#endif
                        sasql_updateFmPointData(meid, NowTime.tv_sec, 0, 0);
                        saacproto_ACFixFMPoint_send(acfd,
                                                    fmpks[fmpks_pos].guest_name,
                                                    fmpks[fmpks_pos].guest_index + 1,
                                                    fmpks[fmpks_pos].guest_index,
...
static void NPC_FMWarpMan_selectWindow(int meindex, int toindex, int num, int select)
{
    ...
    if (CHAR_getWorkInt(meindex, NPC_WORK_MODEFLAG) == NPC_STATEFREE)
    {
                    ...
                    && (strcmp(CHAR_getChar(toindex, CHAR_FMNAME), fmpks[fmpks_pos].guest_name) == 0)))
        {
            ...
            if (NPC_Util_GetStrFromStrWithDelim(npcarg, "FreeMsg", buf2, sizeof(buf2)) == NULL)
            {
                return;
            }
        ...
        {
            if (NPC_Util_GetStrFromStrWithDelim(npcarg, "NextMsg", buf2, sizeof(buf2)) == NULL)
            {
                return;
            }
        ...
        {
            if (NPC_Util_GetStrFromStrWithDelim(npcarg, "TalkMsg", buf2, sizeof(buf2)) == NULL)
            {
                return;
            }
        ...
        {
            if (NPC_Util_GetStrFromStrWithDelim(npcarg, "AskMsg", buf2, sizeof(buf2)) == NULL)
            {
                return;
            }
    ...
    else if (CHAR_getWorkInt(meindex, NPC_WORK_MODEFLAG) == NPC_STATEWAIT) // 两家族战斗结束
    {
                    ...
                    && (strcmp(CHAR_getChar(toindex, CHAR_FMNAME), fmpks[fmpks_pos].guest_name) == 0)))
        {
            ...
            {
                sprintf(tmpbuf, "%s获胜了！！", CHAR_getWorkChar(meindex, NPC_WORK_WINFMNAME));
            }
    ...
    *num2 = 0;
    ...
    for (i = 0; i < playernum; i++)
    {
        if (CHAR_getCharUse(i))
        {
            if (CHAR_getInt(i, CHAR_FLOOR) == floor)
            {
                if (CHAR_getWorkInt(i, CHAR_WORKFMINDEXI) == index1)
                {
                    *num1 = *num1 + 1;
                }
                else if (CHAR_getWorkInt(i, CHAR_WORKFMINDEXI) == index2)
                {
                    *num2 = *num2 + 1;
                }
            }
...
void NPC_FMBATTLESET(int floor, int index1, int index2, int flag)
{
    ...
    for (i = 0; i < playernum; i++)
    {
        if (CHAR_getCharUse(i))
        {
            if (CHAR_getInt(i, CHAR_FLOOR) == floor)
            {
                if (CHAR_getWorkInt(i, CHAR_WORKFMINDEXI) == index1 || CHAR_getWorkInt(i, CHAR_WORKFMINDEXI) == index2)
                {
                    CHAR_setWorkInt(i, CHAR_WORKBATTLEFLAG, flag);

                    if (flag == 1)
                    {
                        CHAR_setFlg(i, CHAR_ISDUEL, 1);
                    }
                }
            }
        }
    ...
    for (i = 0; i < playernum; i++)
    {
        if (CHAR_getCharUse(i))
        {
            if (CHAR_getInt(i, CHAR_FLOOR) == floor)
            {
                if (CHAR_getWorkInt(i, CHAR_WORKFMINDEXI) == index1 || CHAR_getWorkInt(i, CHAR_WORKFMINDEXI) == index2)
                {
                    if (CHAR_getWorkInt(i, CHAR_WORKFMPKFLAG) < 0)
                    {
                        if (CHAR_getWorkInt(i, CHAR_WORKPARTYMODE) != CHAR_PARTY_NONE)
                        {
                            CHAR_DischargeParty(i, 0);
                        }

                        CHAR_setWorkInt(i, CHAR_WORKWARPCHECK, FALSE);
                        CHAR_setWorkInt(i, CHAR_WORKBATTLEFLAG, FALSE);
                        CHAR_warpToSpecificPoint(i, fl, x, y);
                        CHAR_talkToCli(i, -1, "战败离场", CHAR_COLORRED);
                    }
                }
            }
        }
