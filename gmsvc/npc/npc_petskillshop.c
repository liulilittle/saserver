                                  ...
                                  int seqno, int select, char *data)
{
    ...
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "vigor", msg, sizeof(msg)) != NULL)
    {
        getStringFromIndexWithDelim(msg, ",", skill, buf, sizeof(buf));
        vigor = atoi(buf);
    }
    ...
    if (vigor > 0)
    {
        if (CHAR_getInt(talkerindex, CHAR_ACTIVE) < vigor)
        {
            return;
        }
    }
    ...
    if (vigor > 0)
    {
        CHAR_setInt(talkerindex, CHAR_ACTIVE, (CHAR_getInt(talkerindex, CHAR_ACTIVE) - vigor));
    }
...
BOOL NPC_PetSkillMakeStr(int meindex, int toindex, int select)
{
    ...
    NPC_Util_GetStrFromStrWithDelim(argstr, "vigor", msg2, sizeof(msg2));
    ...
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "pet_skill", msg, sizeof(msg)) != NULL)
    {
        ...
        while (getStringFromIndexWithDelim(msg, ",", i, buf, sizeof(buf)) != FALSE)
        {
            ...
            if (getStringFromIndexWithDelim(msg2, ",", i, buf2, sizeof(buf2)) != FALSE)
            {
                vigor = atoi(buf2);
            }
...
#else
            sprintf(token2, "|%s|%d|%s|%d",   PETSKILL_getChar(skillarray, PETSKILL_NAME),
                    cost, PETSKILL_getChar(skillarray, PETSKILL_COMMENT), vigor);
#endif
            strcat(token, token2);
        }
    }
