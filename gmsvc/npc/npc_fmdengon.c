...
#define FM_FMPOINT             2      // 申请家族据点    (主功能表的按键)
#define FM_FMDPTOP             3      // 家族间强者表    (主功能表的按键)
#ifdef _FM_FMPOINTPK_LIST
#define FM_FMPOINTPK           4      // 庄园族战时间表  (主功能表的按键)
#endif
#else
...
#define DENGONDISTANCE 3
    ...
    else if (seqno == CHAR_WINDOWTYPE_FM_SELECT)
    {
        ...
        switch (buttonevent)
        {
            ...
            case FM_FMPOINTPK:
                {
                    if (FreeShowFmPkWindows(talker) == FALSE)
                    {
                        int fd;
                        fd = getfdFromCharaIndex(talker);
                        memset(&NPC_sendbuf, 0, sizeof(NPC_sendbuf));
                        {
#ifdef _515SA_
                            char fmpointname[5][48] = {"萨姆吉尔", "玛丽娜丝", "加加庄园", "[style c=1 s=14]卡鲁它那[/style]", "[style c=20 s=14]伊甸庄园[/style]"};
#else
                            char fmpointname[5][16] = { "萨姆吉尔", "玛丽娜丝", "加加庄园", "卡鲁它那", "伊甸庄园" };
#endif
#ifdef _515SA_
                            char fmpointtype[5][16] = { "5V5", "5V5", "3V3", "1V1", "5V5" };
#else
...
#ifdef _515SA_
                            strcat(NPC_sendbuf, "　　[style c=5 s=14]注：伊甸庄园为特殊庄园，只允许在周三下书挑战。[/style]						  \n　　　　[style c=5 s=14]下书者需为四大家族，周五开战，有额外奖励。[/style]");
#endif
#ifndef _NO_WAIGUA
    ...
    else if (seqno == CHAR_WINDOWTYPE_FM_MEMBERLIST)
    {
...
#endif
        {
            ...
            if (numberlistindex + buttonevent - 1 < 0 || numberlistindex + buttonevent - 1 >= FAMILY_MAXMEMBER)
            {
                return;
            }
            ...
            if (numberlistindex < 0 || numberlistindex >= FAMILY_MAXMEMBER)
            {
                return;
            }
            ...
            case WINDOW_BUTTONTYPE_PREV:
                {
                    ...
                    if ((pointlistindex + 5) > 5)
                    {
                        ...
                        sprintf(pointbuf, "%s  混乱之庄  %3s   %3s   %s\n", pointbuf, x, y, name);
...
#ifdef _NEW_STONEAGE_TYPE
    strcpy(menubuf, "『家族布告栏』|可以查看家族具体情况的告知牌|4|家族成员列表|申请家族据点|家族之间强者表|庄园族战时间表");
    lssproto_WN_send(fd,
...
void ApplyFamilyPoint(int meindex, int toindex, int select)
{
    ...
    if (memberlist[tkfmindex].fmjoinnum < 20)
#else
...
#ifdef _NO_WAIGUA
            sprintf(buf, "很抱歉，申请庄园据点家族人必数需满20人！");
