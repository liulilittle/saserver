...
//        print("\nTrans meindex=%d,toindex=%d,num=%d",meindex,toindex,num);
    ...
    switch (num)
    {
        ...
        case 5:
            ...
            CHAR_setInt(toindex, CHAR_SAVEPOINT, CHAR_getInt(toindex, CHAR_SAVEPOINT) | (1 << 5));
...
BOOL NPC_TransmigrationMain(int meindex, int toindex, char *buf)
{
    ...
    if (FreePlayerTrans(toindex) == FALSE)
    {
        return FALSE;
    }
    ...
    if (FreePlayerTransFlgCls(toindex) == FALSE)
    {
...
#ifdef _GETTANKPET_
    FreeGetTransPet(talker, petindex2);
#endif
...
#else
    int endevflg[] = {69, 70, 71, 72, 109, 118, 122, 131, 127, 158, 159
...
#endif
                    if (CHAR_getInt(petindex, CHAR_LV) < 120)
                    {
                        sprintf(token, "\n\n接受转生祝福的宠物必需要等级120以上喔！");
                        CHAR_setWorkInt(toindex, CHAR_WORKSHOPRELEVANT, 0);
                    }
...
int NPC_PetTransManCheck(int meindex, int toindex, int select)
{
    ...
    //宠物转生条件
    if (CHAR_getInt(toindex, CHAR_LV) < 120)
    {
        return 0;
    }
    else
...
#ifdef _PET_LV1_4V
    char trnbuff[64];
    sprintf(trnbuff, "%d|%d|%d|%d|%d|%d|%d|%s", CHAR_getInt(petNo, CHAR_TRANSMIGRATION), CHAR_getInt(petNo, CHAR_LV), CHAR_getInt(petNo, CHAR_VITAL), CHAR_getInt(petNo, CHAR_STR), CHAR_getInt(petNo, CHAR_TOUGH), CHAR_getInt(petNo, CHAR_DEX), CHAR_getInt(petNo, CHAR_ALLOCPOINT), CHAR_getChar(petNo, CHAR_PET_4V));
#endif
...
#ifdef _PET_LV1_4V
    CHAR_setChar(ret, CHAR_PETTRN_4V, trnbuff);
#endif
...
int NPC_PetTransManStatusNoMM(int toindex, int petNo, int vital1, int str1, int tgh1, int dex1)
{
    int LevelUpPoint = 0, petrank = 0;
    int i, j, k, count;
    int vital2, str2, tgh2, dex2;
