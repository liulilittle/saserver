...
#endif
    {
        ...
        // Robin 0619 display time
        {
            ...
            snprintf( timeMsg, sizeof( timeMsg ) ,
              "≡≡≡ 本星球[%s]已启动 %d 日 %d 小时 %d 分 %d 秒  ",getGameservername(),dd,hh,mm,ss);
            } else if (hh>0) {
            snprintf( timeMsg, sizeof( timeMsg ) ,
                      "≡≡≡ 本星球[%s]已启动 %d 小时 %d 分 %d 秒  ",getGameservername(),hh,mm,ss);
            } else {
                snprintf( timeMsg, sizeof( timeMsg ) ,
                          "≡≡≡ 本星球[%s]已启动 %d 分 %d 秒  ",getGameservername(),mm,ss);
            }*/
            if (dd > 0)
            {
                snprintf(timeMsg, sizeof(timeMsg),
...
#endif
        {
            ...
            if (getPlayerNum() > 0)
            {
                sprintf(temp, "目前在线玩家 %d 名 ≡≡≡", playeronlinenum + luaplayernum + getPlayerNum());
            }
            else
            {
                sprintf(temp, "目前在线玩家 %d 名 ≡≡≡", playeronlinenum + luaplayernum);
            }
            ...
            if (getPlayerNum() > 0)
            {
                sprintf(temp, "目前在线玩家 %d 名 ≡≡≡", abs(StateTable[LOGIN]) + getPlayerNum() + rand() % 10);
            }
            else
            {
                sprintf(temp, "目前在线玩家 %d 名 ≡≡≡", abs(StateTable[LOGIN]));
            }
        ...
        int tmpcolor = rand() % CHAR_COLORNUM;
        CHAR_talkToCli(talkerindex, -1, buf2, tmpcolor);
        CHAR_talkToCli(talkerindex, -1, buf3, tmpcolor);
    }
}
