...
BOOL NPC_Util_IsVisiblePlayer(int meindex)
{
    ...
    for (i = chr_x - CHARDEFAULTSEESIZ / 2 ;
            i <= chr_x + CHARDEFAULTSEESIZ / 2 && found == FALSE; i++)
    {
        for (j = chr_y - CHARDEFAULTSEESIZ / 2 ;
                j <= chr_y + CHARDEFAULTSEESIZ / 2 && found == FALSE;
                j ++)
        {
            OBJECT  object;
...

{
    ...
    char buff[256];
    sprintf(buff, "%d-0", shiftbit);
    NPC_ActionEventID(talker, buff, 1);
#endif
}
...
void  NPC_NowEventSetFlgCls(int talker, int shiftbit)
{
    ...
    char buff[256];
    sprintf(buff, "%d-0", shiftbit);
    NPC_ActionEventID(talker, buff, 0);
#endif
}
