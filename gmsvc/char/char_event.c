...
BOOL CHAR_makeCAOPT1PartyString(int objindex, int charaindex, char* buf, int buflen, int act, int opt1)
{
    ...
    char    partyobjindexbuf[64];
    int i;
    int partynum = 0;
    char partyindex[64];
    memset(partyindex, 0, sizeof(partyindex));
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        {
            sprintf(partyindex, "%s|%s", partyindex, cnv10to62(CHAR_getWorkInt(partyindextemp, CHAR_WORKOBJINDEX), partyobjindexbuf, sizeof(partyobjindexbuf)));
            partynum++;
        }
    }

    snprintf(buf, buflen, "%s|%d|%d|%d|%d|%d|%d%s",
             cnv10to62(objindex, objindexbuf, sizeof(objindexbuf)),
             OBJECT_getX(objindex), OBJECT_getY(objindex),
             act,
             CHAR_getInt(OBJECT_getIndex(objindex), CHAR_DIR), opt1, partynum, partyindex);
    return TRUE;
}
                          ...
                          )
{
    ...
    if ((channel > -1) && (fmindex > 0))
    {
        if (channel == 0)
        {
            ...
            snprintf(lastbuf, sizeof(lastbuf), "P|[家族]%s",
#else
            snprintf(lastbuf, sizeof(lastbuf), "P|F|[家族]%s",
#endif
    ...
    else
    {
        ...
        if (CHAR_getFlg(talkindex, CHAR_ISPARTYCHAT) && (CHAR_getWorkInt(talkindex, CHAR_WORKPARTYMODE) != CHAR_PARTY_NONE))
            snprintf(lastbuf, sizeof(lastbuf), "P|T|[队伍]%s",
                     makeEscapeString(CHAR_appendNameAndTitle(talkindex, message, mesgbuf, sizeof(mesgbuf)),
                                      escapebuf, sizeof(escapebuf)));
        else
#ifdef _NEW_CHANNEL_
            if (talkindex > -1 && CHAR_getInt(talkindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
            {
#ifdef _FONT_SIZE
...
#endif
#ifdef  _PLAYER_UID
                     , CHAR_getChar(talkindex, CHAR_PLAYERUID)
#endif
#ifdef _YAYA_VOICE
                     , volumeLen
#endif
#ifdef _TK_PLAYERFACE
                     , CHAR_getInt(talkindex, CHAR_FACEIMAGENUMBER)
                     , CHAR_getInt(talkindex, CHAR_LV)
