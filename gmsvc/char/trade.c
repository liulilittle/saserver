...
int TRADE_getMyTarget(int meindex)
{
    ...
    for (i = 0; i < 2; i++)
    {
                ...
                TradeList[ti][i].fd == fd)
        {
            ...
            if (TradeList[ti][side].fd != getfdFromCharaIndex(TradeList[ti][side].charaindex))
            {
                return -1;
            }
...
BOOL TRADE_addSomeTradeList(int meindex, int usTi, int *num, int type)
{
    ...
    switch (type)
    {
        case TRADE_ITEM:
            {
                ...
                if (usTi < CHAR_STARTITEMARRAY || usTi >= CHAR_MAXITEMHAVE)
                {
                    return FALSE;
                }
                ...
                {
                    char buf[256];
                    sprintf(buf, "%s 此道具为绑定道具，无法交易，请妥善保管。", ITEM_getChar(itemindex, ITEM_SECRETNAME));
                    CHAR_talkToCli(meindex, -1, buf, CHAR_COLORYELLOW);
                    return  FALSE;
                }
                ...
                if (ITEM_getInt(itemindex, ITEM_LOCKED) > 0)
                {
                    char buf[256];
                    sprintf(buf, "%s 此道具为绑定道具，无法交易，请妥善保管。", ITEM_getChar(itemindex, ITEM_NAME));
                    CHAR_talkToCli(meindex, -1, buf, CHAR_COLORYELLOW);
                    return  FALSE;
                }
                ...
                if (ITEM_getInt(itemindex, ITEM_CANBEPILE) == 1)   //可堆叠
                {
                    ...
                    for (i = 0; i < 15; i++)
                    {
                        ...
                        else
                        {
                            ...
                            TradeList[ti][side].ItemNum[i] = *num;
                        }
        ...
        case TRADE_PET:
            {
...
#ifdef _LOCK_PET_ITEM
                {
                    ...
                    {
                        CHAR_talkToCli(meindex, -1, "[宠物保护系统]该宠自定义名称中含有(*) 无法丢弃、卖店、交易、摆摊 如需操作请修改名字.", CHAR_COLORYELLOW);
                        return FALSE;
                    }
                ...
                {
                    if (CHAR_getInt(petindex, CHAR_LV) > (CHAR_getInt(toindex, CHAR_LV) + getPlayerPetLv()) &&
        ...
        case TRADE_GOLD:
            if (FreeTradeGold(meindex, toindex, usTi) == 1)
            {
                return FALSE;
            }
...
//   CHAR_setWorkInt(meindex, CHAR_WORKTRADEMODE, CHAR_TRADE_SENDING);
    ...
    if (cnt == 1)
    {
        ...
        /*if(CHAR_getInt(meindex, CHAR_LOCKED)==1){
            ...
            return FALSE;
        }else if(CHAR_getInt(meindex, CHAR_LOCKED)==2){
            char message[256];
            char    buf[256];
...
BOOL TRADE_SearchFromIndex(int fd, int meindex, int toindex)
{
    int     objbuf[16];
    int     front_x, front_y, i;
    int found_count = 0;
    BOOL        found = FALSE, searchflg = FALSE;
    int         cnt = 0, tofd = -1, checkfd = -1;
    char        msgbuf[1024], mycharaname[256], tocharaname[256];
...
#endif
    int objindex = CHAR_getWorkInt(toindex, CHAR_WORKOBJINDEX);
    int index = OBJECT_getIndex(objindex);
...
int TRADE_HandleTrade_DelItem(int charaindex, int MyMaxPile, int MaxPile, int *Item, int *nums, int *indexlist)
{
    ...
    for (i = 0; i < 15; i++)
    {
        ...
        else
        {
            ...
            ITEM_setChar(newindex, ITEM_NAME, ITEM_getChar(itemindex, ITEM_NAME));
            ITEM_setChar(newindex, ITEM_SECRETNAME, ITEM_getChar(itemindex, ITEM_SECRETNAME));
            ITEM_setChar(newindex, ITEM_EFFECTSTRING, ITEM_getChar(itemindex, ITEM_EFFECTSTRING));
            ITEM_setChar(newindex, ITEM_ARGUMENT, ITEM_getChar(itemindex, ITEM_ARGUMENT));
...
BOOL TRADE_HandleTrade(int meindex, STradeList *temp1, int toindex, STradeList *temp2)
{
    ...
    //移除
    if ((ret = TRADE_HandleTrade_DelItem(meindex, MeMaxPile, ToMaxPile, temp1->ItemTi, temp1->ItemNum, ToItem)) == -1)
    {
        return FALSE;
    }

    if ((ret = TRADE_HandleTrade_DelItem(toindex, ToMaxPile, MeMaxPile, temp2->ItemTi, temp2->ItemNum, MeItem)) == -1)
    {
        return FALSE;
    }
    ...
    {
        char item1[64 * 15], item2[64 * 15], pet1[64 * 5], pet2[64 * 5], buf[4096];
        int i;
        memset(item1, 0, sizeof(item1));
        memset(item2, 0, sizeof(item2));
        memset(pet1, 0, sizeof(pet1));
        memset(pet2, 0, sizeof(pet2));

        for (i = 0; i < 15; i++)
        {
            if (ITEM_CHECKINDEX(ToItem[i]))
            {
                sprintf(item1, "%s%s[%s]*%d,", item1,
                        ITEM_getChar(ToItem[i], ITEM_SECRETNAME), ITEM_getChar(ToItem[i], ITEM_UNIQUECODE),
                        ITEM_getInt(ToItem[i], ITEM_USEPILENUMS));
            }

            if (ITEM_CHECKINDEX(MeItem[i]))
            {
                sprintf(item2, "%s%s[%s]*%d,", item2,
                        ITEM_getChar(MeItem[i], ITEM_SECRETNAME), ITEM_getChar(MeItem[i], ITEM_UNIQUECODE),
        ...
        for (i = 0; i < 5; i++)
        {
            ...
            {
                sprintf(pet1, "%s%s[%s]-%d,", pet1,
                        CHAR_getChar(ToPet[i], CHAR_NAME), CHAR_getChar(ToPet[i], CHAR_UNIQUECODE), CHAR_getInt(ToPet[i], CHAR_LV));
            }

            if (CHAR_CHECKINDEX(MePet[i]))
            {
                sprintf(pet2, "%s%s[%s]-%d,", pet2,
                        CHAR_getChar(MePet[i], CHAR_NAME), CHAR_getChar(MePet[i], CHAR_UNIQUECODE), CHAR_getInt(MePet[i], CHAR_LV));
            }
        }

        if (strlen(item1) > 0 || strlen(pet1) > 0 || ToGold > 0)
        {
            sprintf(buf, "INSERT INTO `tradelog` VALUES( '%s','%s','%s','%s',NOW(),'%s','%s',%d)",
                    CHAR_getChar(meindex, CHAR_CDKEY), CHAR_getChar(meindex, CHAR_NAME), CHAR_getChar(toindex, CHAR_CDKEY), CHAR_getChar(toindex, CHAR_NAME)
                    , item1, pet1, ToGold);
            sasql_mysql_query(buf);
        }

        if (strlen(item2) > 0 || strlen(pet2) > 0 || MeGold > 0)
        {
            sprintf(buf, "INSERT INTO `tradelog` VALUES( '%s','%s','%s','%s',NOW(),'%s','%s',%d)",
                    CHAR_getChar(toindex, CHAR_CDKEY), CHAR_getChar(toindex, CHAR_NAME), CHAR_getChar(meindex, CHAR_CDKEY), CHAR_getChar(meindex, CHAR_NAME)
                    , item2, pet2, MeGold);
            sasql_mysql_query(buf);
        }
    }
...
int TRADE_CheckItembuf(int fd, int meindex, int toindex, int tofd, char* mycharaname, char* tocharaname)
{
    ...
    //确定空位
    if (TRADE_CheckTradeList(meindex, &TradeListTemp[0], toindex, &TradeListTemp[1]) == FALSE)
    {
        return -1;
    }
    ...
    if (TRADE_HandleTrade(meindex, &TradeListTemp[0], toindex, &TradeListTemp[1]) == FALSE)
    {
        return -1;
    }
...
int TRADE_CheckItembuf(int fd, int meindex, int toindex, int tofd, char* mycharaname, char* tocharaname)
{
            ...
            && (itemindex5 == toitemindex2) && (itemindex6 == toitemindex3))
    {
        ...
        if ((gold2 + CHAR_getInt(meindex, CHAR_GOLD) - gold1) > CHAR_getMaxHaveGold(meindex))
        {
            return    -8;
        }

        if ((gold1 + CHAR_getInt(toindex, CHAR_GOLD) - gold2) > CHAR_getMaxHaveGold(toindex))
        {
            return    -9;
        }
...
BOOL TRADE_HandleItem(int meindex, int showindex, char *message, char *outmess)
{
...

    {
        ...
        if (getStringFromIndexWithDelim(message, "|", 7, token, sizeof(token)) == TRUE)
        {
            nums = atoi(token);
        }
        ...
        if (arg[0] == '*' || ITEM_getInt(itemindex, ITEM_VANISHATDROP) == 1)
        {
            sprintf(token, "绑定物品");
        }
        else
        {
            sprintf(token, "耐久度：无限");
        }
        ...
        if (arg[0] == '*' || ITEM_getInt(itemindex, ITEM_VANISHATDROP) == 1)
        {
            if (crushe >= 0)
            {
                snprintf(token, sizeof(token), "绑定物品 %d%%", crushe);
            }
            else
            {
                snprintf(token, sizeof(token), "%s", "绑定物品");
            }
        }
        else
        {
            if (crushe >= 0)
            {
                snprintf(token, sizeof(token), "耐久度：%d%%", crushe);
            }
            else
            {
                snprintf(token, sizeof(token), "%s", "耐久度：无限");
            }
        }
        ...
        sprintf(outmess, "%d|%s|%s|%s|%d|%s|%d|%d",
#else
        sprintf(outmess, "%d|%s|%s|%d|%s|%d",
#endif
...
#endif
                FreeShowItemEffect(itemindex), item, token
#ifdef _ITEM_PILEFORTRADE
                , nums
#endif
                , itemtype
        ...
        sprintf(outmess, "%s|%s|%s|%s|%d|%d", outmess, Mname, ITEM_getChar(itemindex, ITEM_TYPECODE), ITEM_getChar(itemindex, ITEM_INLAYCODE)
                , ITEM_getInt(itemindex, ITEM_NEEDTRANS), ITEM_getInt(itemindex, ITEM_LEVEL));
#endif
    }
...
BOOL TRADE_HandlePet(int meindex, int showindex, char *message, char *outmess)
{
...
#ifdef _TRADESYSTEM2
    {
...
#ifdef _POWER_UP
        {
            ...
            {
                char zhufubuff[64];
                memset(zhufubuff, 0, sizeof(zhufubuff));
                /*int       zhufu[4];
                char zhufutmp[4][10] = {"水","火","地","风"};
                int i;
                int j = 0;
                for( i = 0; i < 4; i ++ ) {
                    ...
                    if(zhufu[i]!=0){
                        j++;
                        if(strlen(zhufubuff)<1){
                #ifdef   _TRADE_SHOW_PET_UP
                            sprintf(zhufubuff,"%d",i+1);
                #else
                            sprintf(zhufubuff,"已祝福][%s",zhufutmp[i]);
                #endif
                        }else{
                #ifdef   _TRADE_SHOW_PET_UP
                            sprintf(zhufubuff,"%s,%d",zhufubuff,i+1);
                #else
                            strcat(zhufubuff,zhufutmp[i]);
                #endif
                        }
                    }
                }
                #ifndef  _TRADE_SHOW_PET_UP
                if(j==4){
                    sprintf(zhufubuff,"所有属性已祝福");
                }
                #endif*/
                ...
                if (strlen(CHAR_getChar(petindex, CHAR_NEWNAME)) > 0)
                {
                    char newbuff[16];
                    int newtype1 = 0;
                    int newtype2 = 0;
                    ...
                    if (getStringFromIndexWithDelim(CHAR_getChar(petindex, CHAR_NEWNAME), "|", 1, newbuff, sizeof(newbuff)) == TRUE)
                    {
                        newtype1 = atoi(newbuff);
                    }

                    if (getStringFromIndexWithDelim(CHAR_getChar(petindex, CHAR_NEWNAME), "|", 2, newbuff, sizeof(newbuff)) == TRUE)
                    {
                        newtype2 = atoi(newbuff);
                    }

                    if (getStringFromIndexWithDelim(CHAR_getChar(petindex, CHAR_NEWNAME), "|", 3, newbuff, sizeof(newbuff)) == TRUE)
                    {
                        newtype3 = atoi(newbuff);
                    }

                    if (getStringFromIndexWithDelim(CHAR_getChar(petindex, CHAR_NEWNAME), "|", 4, newbuff, sizeof(newbuff)) == TRUE)
                    {
                        newtype4 = atoi(newbuff);
                    }

                    sprintf(zhufubuff, "祝福:%d/%d/%d/%d", newtype1, newtype2, newtype3, newtype4);
                }

#endif
#ifdef _TRADE_PET_OTHERBUFF
                int oldlv = 0;
                int oldhp = 0;
                int oldstr = 0;
                int oldvgh = 0;
                ...
                if (strlen(CHAR_getChar(petindex, CHAR_PET_4V)) > 0)
                {
                    char newbuff[16];

                    if (getStringFromIndexWithDelim(CHAR_getChar(petindex, CHAR_PET_4V), "|", 1, newbuff, sizeof(newbuff)) == TRUE)
                    {
                        oldhp = atoi(newbuff);
                    }

                    if (getStringFromIndexWithDelim(CHAR_getChar(petindex, CHAR_PET_4V), "|", 2, newbuff, sizeof(newbuff)) == TRUE)
                    {
                        oldstr = atoi(newbuff);
                    }

                    if (getStringFromIndexWithDelim(CHAR_getChar(petindex, CHAR_PET_4V), "|", 3, newbuff, sizeof(newbuff)) == TRUE)
                    {
                        oldvgh = atoi(newbuff);
                    }

                    if (getStringFromIndexWithDelim(CHAR_getChar(petindex, CHAR_PET_4V), "|", 4, newbuff, sizeof(newbuff)) == TRUE)
                    {
                        olddex = atoi(newbuff);
                    }

                    if (getStringFromIndexWithDelim(CHAR_getChar(petindex, CHAR_PET_4V), "|", 5, newbuff, sizeof(newbuff)) == TRUE)
                    {
                        oldlv = atoi(newbuff);
                    }
                }
...
#endif
#ifdef _TRADE_PET_OTHERBUFF
                        , oldlv, oldhp, oldstr, oldvgh, olddex, CHAR_getInt(petindex, CHAR_EARTHAT)
                        , CHAR_getInt(petindex, CHAR_WATERAT), CHAR_getInt(petindex, CHAR_FIREAT), CHAR_getInt(petindex, CHAR_WINDAT)
#endif
#ifdef _PET_NOZHUFU_TYPE
                        , PET_get4v(petindex)
#endif
                        , CHAR_getInt(petindex, CHAR_NOFAME)
                        , CHAR_getInt(petindex, CHAR_LASTTALKELDER)
                        , CHAR_getInt(petindex, CHAR_ATTACK_EFFECT)
