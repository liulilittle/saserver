                            ...
                            )
{
...
#ifdef _NEW_STONEAGE_TYPE
#ifdef _ITEM_PET_TALK_
    char outpetitemdata[1024];
    memset(outpetitemdata, 0, sizeof(outpetitemdata));
    strcpy(outpetitemdata, GetOutPetItemData(cindex, petitemstr));
#endif
#endif
...
#ifdef _NEW_STONEAGE_TYPE
            lssproto_MSG_send(CHAR_getWorkInt(cindex, CHAR_WORKFD), aindex, textbuffer, color
                              , 1
#ifdef _ITEM_PET_TALK_
                              , outpetitemdata
#endif
    ...
    if (sasql_queryonline(ae->cdkey) <= 0 || sasql_queryoffline(ae->cdkey) != 0)
    {
        CHAR_newMessageToCli(cindex, -1, "玩家不在线", CHAR_COLORWHITE);
        return FALSE;
    }
...
BOOL ADDRESSBOOK_addEntry(int meindex)
{
    ...
    {
        CHAR_newMessageToCli(meindex, -1, ADDRESSBOOK_MYTABLEFULL, CHAR_COLORWHITE);
        return FALSE;
    }
    ...
    if (found_count == 0)
    {
        CHAR_newMessageToCli(meindex, -1, ADDRESSBOOK_CANTADD, CHAR_COLORWHITE);
        return FALSE;
    }
    ...
    if (cnt == 0)
    {
        if (found)
        {
            CHAR_newMessageToCli(meindex, -1, ADDRESSBOOK_CANTADD2, CHAR_COLORWHITE);
        }
        else
        {
            CHAR_newMessageToCli(meindex, -1, ADDRESSBOOK_CANTADD, CHAR_COLORWHITE);
...
BOOL ADDRESSBOOK_addEntryFromToindex(int meindex, int toindex)
{
    ...
    //if(CHAR_getInt(meindex,CHAR_FLOOR) != CHAR_getInt(toindex,CHAR_FLOOR))return FALSE;
    fd = getfdFromCharaIndex(meindex);
    ...
    found_count = 1;
    objbuf[0] = CHAR_getWorkInt(toindex, CHAR_WORKOBJINDEX);
...
#ifdef  _PLAYER_UID
    strcpysafe(ae->uid, sizeof(ae->uid),
               CHAR_getChar(charaindex, CHAR_PLAYERUID));
#endif
#ifdef _PLAYERCARD_FAMILYNAME
    strcpysafe(ae->fmname, sizeof(ae->fmname),
...
void ADDRESSBOOK_notifyLoginLogout(int cindex, int flg)
{
...
#ifdef _PLAYERCARD_FAMILYNAME
                    strcpysafe(ae->fmname, sizeof(ae->fmname),
                               CHAR_getChar(cindex, CHAR_FMNAME));
#endif // _PLAYERCARD_FAMILYNAME
    ...
    else if (flg == 1)
    {
        ...
        saacproto_Broadcast_send(acfd, cd, nm, "online", getServernumber());
...
BOOL ADDRESSBOOK_sendAddressbookTable(int cindex)
{
    ...
    for (i = 0 ; i < ADDRESSBOOK_MAX ; i++)
    {
        ...
        if (ae && ae->use)
        {
            char tmp[CHARNAMELEN * 2];
            char charname_escaped[CHARNAMELEN * 2];
            makeEscapeString(ae->charname, charname_escaped,
                             sizeof(charname_escaped));
#ifdef  _PLAYER_UID
            char uid[16];
            makeEscapeString(ae->uid, uid,
                             sizeof(uid));
#endif
#ifdef _PLAYERCARD_FAMILYNAME
            char fmname[CHARNAMELEN * 2];
            makeEscapeString(ae->fmname, fmname,
                             sizeof(fmname));
#endif // _PLAYERCARD_FAMILYNAME
            snprintf(tmp, sizeof(tmp),
                     ...
                     ,
                     ae->use,
                     charname_escaped, ae->level, ae->duelpoint,
#ifdef _ADDRESS_ONLINE
                     FreeServerId(ae->online)
#else
                     ae->online
#endif
                     , ae->graphicsno,
...
#ifdef  _PLAYER_UID
    char uid[16];
    makeEscapeString(a->uid, uid, sizeof(uid));
#endif
#ifdef _PLAYERCARD_FAMILYNAME
    char fmname[CHARNAMELEN * 2];
    makeEscapeString(a->fmname, fmname,
                     sizeof(fmname));
#endif // _PLAYERCARD_FAMILYNAME
...
#ifdef  _PLAYER_UID
    char work7[16];
    getStringFromIndexWithDelim(in, "|", 7, work7, sizeof(work7));
    makeStringFromEscaped(work7);
    strcpysafe(a->uid, sizeof(a->uid), work7);
#endif
#ifdef _PLAYERCARD_FAMILYNAME
    char work8[CHARNAMELEN * 2];
    getStringFromIndexWithDelim(in, "|", 8, work8, sizeof(work8));
    makeStringFromEscaped(work8);
    strcpysafe(a->fmname, sizeof(a->fmname), work8);
#endif // _PLAYERCARD_FAMILYNAME
    a->use = 1;
...
void ADDRESSBOOK_addAddressBook(int meindex, int toindex)
{
    ...
    if (hisblank < 0)
    {
        CHAR_newMessageToCli(meindex, -1, ADDRESSBOOK_HISTABLEFULL, CHAR_COLORWHITE);
        return ;
    }
    ...
    if (myaddindex < 0)
    {
        ...
        snprintf(tmpstring, sizeof(tmpstring),
                 ...
                 hisentry.charname);
        CHAR_newMessageToCli(meindex, -1, tmpstring, CHAR_COLORWHITE);
    ...
    if (toaddindex < 0)
    {
        ...
        snprintf(tmpstring, sizeof(tmpstring),
                 ...
                 CHAR_getChar(meindex, CHAR_NAME));
        CHAR_newMessageToCli(toindex, -1, tmpstring, CHAR_COLORWHITE);
...
#ifdef _PLAYERCARD_FAMILYNAME
    char fmname[CHARNAMELEN * 2];
    getStringFromIndexWithDelim(value, "|", 6, fmname, sizeof(fmname));
#endif // _PLAYERCARD_FAMILYNAME
        ...
        if (CHAR_CHECKINDEX(i))
        {
            ...
            for (j = 0 ; j < ADDRESSBOOK_MAX ; j++)
            {
                        ...
                        strcmp(ae->charname, nm) == 0)
                {
...
#ifdef _PLAYERCARD_FAMILYNAME
                        strcpysafe(ae->fmname, sizeof(ae->fmname), fmname);
#endif // _PLAYERCARD_FAMILYNAME
#ifdef _NEW_STONEAGE_TYPE
                        saacproto_NewCardUpdate_send(acfd, CHAR_getChar(i, CHAR_CDKEY), CHAR_getChar(i, CHAR_NAME), getServernumber(), CHAR_getInt(i, CHAR_LV), CHAR_getInt(i, CHAR_FACEIMAGENUMBER), CHAR_getInt(i, CHAR_TRANSMIGRATION)
#ifdef _PLAYERCARD_FAMILYNAME
                    ...
                    //saacproto_Broadcast_send(acfd, CHAR_getChar(i, CHAR_CDKEY), CHAR_getChar(i, CHAR_NAME), "online", 1);
                    ADDRESSBOOK_sendAddressbookTableOne(i, j);
                    break;
                }
            }
