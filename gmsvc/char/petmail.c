                         ...
                         int havepetindex, int haveitemindex, char* text, int color)
{
    ...
    if (CHAR_getInt(petindex, CHAR_LV) > 30)
    {
        CHAR_talkToCli(cindex, -1, "大于30级的宠物无法传递邮件喔！(建议渔村门口捕捉乌力邮寄)", CHAR_COLORYELLOW);
        return  FALSE;
    }
    ...
    if (arg[0] == '*')
    {
        CHAR_talkToCli(cindex, -1, "绑定的物品无法转移给他人，请妥善保管。", CHAR_COLORYELLOW);
        return  FALSE;
    }
    ...
    {
        char token[256];
        snprintf(token, sizeof(token), "限时物品不可邮寄。",
...
#endif
#ifdef _MAILITEMFULL_
    extern int CHAR_findEmptyItemBox(int index);

    if (CHAR_findEmptyItemBox(i) == -1)
    {
        CHAR_talkToCli(cindex, -1, "对方道具栏已经满了！", CHAR_COLORYELLOW);
        return FALSE;
    }

    if (CHAR_getWorkInt(i, CHAR_WORKBATTLEMODE) != BATTLE_CHARMODE_NONE)
    {
        CHAR_talkToCli(cindex, -1, "对象战斗状态中无法传递宠物邮件。", CHAR_COLORYELLOW);
        return  FALSE;
    }

    if (CHAR_getInt(i, CHAR_FLOOR) == 1042 || CHAR_getInt(i, CHAR_FLOOR) == 2032 || CHAR_getInt(i, CHAR_FLOOR) == 3032 || CHAR_getInt(i, CHAR_FLOOR) == 4032 /*|| CHAR_getInt(i,CHAR_FLOOR) == 2000*/)
    {
        if (17 == ITEM_getInt(itemindex, ITEM_TYPE))
        {
            CHAR_talkToCli(cindex, -1, "族战地图不可寄送回旋镖。", CHAR_COLORYELLOW);
            return  FALSE;
        }
    }
...
#endif
    {
        if (FreePetMail(cindex, petindex) == FALSE)
        {
            return FALSE;
        }
...
static void PETMAIL_sendPetmail(int index, int tocharaindex)
{
    ...
    else
    {
        ...
        if (ret < 0 || ret >= CHAR_MAXITEMHAVE)
        {
            ...
            ITEM_setChar(itemindex, ITEM_CDKEY, CHAR_getChar(tocharaindex, CHAR_CDKEY));
            LogItem(
...
static void PETMAIL_IdleProc1(int index)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (distance > CHARDEFAULTSEESIZ / 2)
            {
                warp = TRUE;
            }
            else if (distance > 1)
...
static void PETMAIL_IdleProc5(int index)
{
    ...
    else
    {
        ...
        if (tocharaindex != -1)
        {
            if (NPC_Util_CharDistance(tocharaindex, index) > CHARDEFAULTSEESIZ / 2)
            {
                int     fl, x, y, ch_x, ch_y;
                int     cnt;
                fl = CHAR_getInt(tocharaindex, CHAR_FLOOR);
