...
int PET_DEBUG_initPetOne(int charaindex)
{
    ...
    ch.data[CHAR_HP] = 100;
    /*    蟆 */
    strcpysafe(ch.string[CHAR_NAME].string, sizeof(ch.string[CHAR_NAME].string), "宠物１");
    /* CHAR卞喃曰癫化月 */
...
static int _PET_dropPet(int charaindex, int havepetindex, int tofl, int tox, int toy)
{
...
#ifdef _LOCK_PET_ITEM
    {
        ...
        if (strlen(owntile) > 0)
            ...
            {
                CHAR_talkToCli(charaindex, -1, "[宠物保护系统]有昵称的100级以上宠物无法丢出，如需操作请把宠物名称改为原名称.", CHAR_COLORYELLOW);
                return FALSE;
            }
    }
#endif
    ...
    snprintf(szPet, sizeof(szPet), "您成功丢出宠物[%s][%d]（消失时间：%d秒）", CHAR_getChar(petindex, CHAR_NAME), CHAR_getInt(petindex, CHAR_LV), getPetdeletetime());
