                               ...
                               int fx, int fy)
{
     ...
     */
    int     vx, vy;
    const int     seesiz = MAPCHARDEFALUTSEESIZ;
...
void CHAR_sendCharaAtWalk(int charaindex, int of, int ox, int  oy, int xflg, int yflg)
{
    ...
    int     seesiz = CHARDEFAULTSEESIZ / 2;
    char    cabuf[128];
            ...
            for (object = MAP_getTopObj(fl, i, j) ; object ; object = NEXT_OBJECT(object))
            {
                    ...
                    if (OBJECT_getType(objindex) == OBJTYPE_CHARA)
                    {
                        if (CHAR_getInt(c_index, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
                        {
                            ...
                            if (tofd != -1)
                            {
                                ...
                                if (CHAR_getShowType(charaindex, c_index) == 1 || (whichtype != CHAR_TYPEPLAYER && whichtype != CHAR_TYPEPLAYERNPC))
#endif
                                {
                                    CDsend(tofd);
                ...
                if (whichtype == CHAR_TYPEPLAYER)
                {
                    ...
                    if (CHAR_makeObjectCString(objindex, introduction,   sizeof(introduction)))
                    {
                        ...
                        if (CHAR_getShowType(c_index, charaindex) == 1 || (CHAR_getInt(c_index, CHAR_WHICHTYPE) != CHAR_TYPEPLAYER &&  CHAR_getInt(c_index, CHAR_WHICHTYPE) != CHAR_TYPEPLAYERNPC))
#endif
                        {
