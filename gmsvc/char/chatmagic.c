...
void CHAR_CHAT_DEBUG_announce(int charindex, char *message)
{
    ...
    int     playernum = CHAR_getPlayerMaxNum();
    char token[256];
    sprintf(token, "P|P|≮%s服务器公告≯ %s", getGameservername(), message);
...
#define     CHAT_CHAT_DEBUG_SUPERMAN_PARAMDEF       100000
    ...
    if (strlen(cdkey) > 0)
    {
        ...
        CHAR_setInt(charaindex, CHAR_LV, getMaxLevel(charaindex));
#else
...
void CHAR_CHAT_DEBUG_petmake(int charaindex, char* message)
{
    ...
    if (strlen(cdkey) > 0)
    {
        ...
        if (level != -1)
        {
            ...
            if (level > getMaxLevel(ret))
            {
                level = getMaxLevel(ret);
            }
...
void CHAR_CHAT_DEBUG_petmakenum(int charaindex, char* message)
{
    int ret;
    int enemynum;
    int enemyid;
    ...
    char token[128];
    int petpoint = 0;
    int tpvital, tpstr, tptgh, tpdex, tpallpoint = 0;
    easyGetTokenFromString(message, 1, buf, sizeof(buf));
    enemyid = atoi(buf);
    easyGetTokenFromString(message, 2, buf, sizeof(buf));
    level = atoi(buf);
    ...
    petnum = atoi(buf);
    int havepetelement = CHAR_getCharPetElement(charaindex);
    ...
    for (kk = 1; kk <= petnum; kk++)
    {
        ...
        ret = ENEMY_createPetFromEnemyIndexTest(charaindex, i);
        ...
        if (level != -1)
        {
            ...
            templevel = level - CHAR_getInt(ret, CHAR_LV);

            if (templevel > 0)
            {
                int k;

                for (k = 1; k <= templevel; k ++)    //升级
                {
                    CHAR_PetLevelUp(ret);
                    CHAR_PetAddVariableAi(ret, AI_FIX_PETLEVELUP);
                    CHAR_setInt(ret, CHAR_LV, CHAR_getInt(ret, CHAR_LV) + 1);
        ...
        CHAR_setInt(ret, CHAR_HP, CHAR_getWorkInt(ret, CHAR_WORKMAXHP));
        CHAR_complianceParameter(ret);
        int temppetpoint = CHAR_getWorkInt(ret, CHAR_WORKMAXHP) / 4 + CHAR_getWorkInt(ret, CHAR_WORKFIXSTR) + CHAR_getWorkInt(ret, CHAR_WORKFIXTOUGH) + CHAR_getWorkInt(ret, CHAR_WORKFIXDEX);

        if (temppetpoint > petpoint)
        {
            petpoint = temppetpoint;
            tpvital = CHAR_getInt(ret, CHAR_VITAL);
            tpstr = CHAR_getInt(ret, CHAR_STR);
            tptgh = CHAR_getInt(ret, CHAR_TOUGH);
            tpdex = CHAR_getInt(ret, CHAR_DEX);
            tpallpoint = CHAR_getInt(ret, CHAR_ALLOCPOINT);
        }

        if (kk < petnum)
        {
            CHAR_endCharOneArray(ret);
        }
    }

    CHAR_setInt(ret, CHAR_VITAL, tpvital);
    CHAR_setInt(ret, CHAR_STR, tpstr);
    CHAR_setInt(ret, CHAR_TOUGH, tptgh);
    CHAR_setInt(ret, CHAR_DEX, tpdex);
    CHAR_setInt(ret, CHAR_ALLOCPOINT, tpallpoint);
    CHAR_complianceParameter(ret);
    CHAR_setInt(ret, CHAR_HP, CHAR_getWorkInt(ret, CHAR_WORKMAXHP));
    CHAR_setCharPet(charaindex, havepetelement, ret);
...
void CHAR_CHAT_DEBUG_eventclean(int charaindex, char* message)
{
    ...
    {
        if (shiftbit == -1 || strlen(token_cdkey) == 0)
        {
            sprintf(token, "%s", "参数不正确。正确格式：[eventsetend 任务旗标 帐号]");
            CHAR_talkToCli(charaindex, -1, token, CHAR_COLORWHITE);
            return;
        }
    ...
    if (strcmp(token_flag, "all") == 0 && find)
    {
        ...
        CHAR_setChar(charaindex_tmp, CHAR_EVENTID, "");
        lssproto_EventID_send(CHAR_getWorkInt(charaindex_tmp, CHAR_WORKFD));
#endif
        return ;
    }
    ...
    if (find)
    {
        ...
        NPC_NowEndEventSetFlgCls(charaindex_tmp, shiftbit);
...
void CHAR_CHAT_DEBUG_eventsetend(int charaindex, char* message)
{
    ...
    if (find)
    {
        ...
        NPC_EventSetFlg(charaindex_tmp, shiftbit);
...
void CHAR_CHAT_DEBUG_fixfmdata(int charaindex, char *message)
{
    ...
    if (index < 1 || index > FAMILY_MAXNUM)
    {
...
void CHAR_CHAT_DEBUG_petlevelup(int charaindex, char* message)
{
    ...
    easyGetTokenFromString(message, 4, cdkey, sizeof(cdkey));
#ifdef _TRANS_LEVEL_CF
    ...
    if (strlen(cdkey) > 0)
    {
        ...
        {
            CHAR_GMPetLevelUp(petindex, zhang);
            CHAR_PetAddVariableAi(petindex, AI_FIX_PETLEVELUP);
            CHAR_setInt(petindex, CHAR_LV, CHAR_getInt(petindex, CHAR_LV) + 1);
        }
...
void CHAR_CHAT_DEBUG_set_momentum(int charaindex, char* message)
{
    ...
    for (i = 0; i < iPlayerNum; i++)
    {
        if (CHAR_getCharUse(i) != FALSE)
        {
            if (!strcmp(CHAR_getChar(i, CHAR_CDKEY), szName))
            {
                break;
            }
        }
        ...
        for (j = x - area; j <= x + area; j++)
        {
            for (object = MAP_getTopObj(floor, j, i) ; object ; object = NEXT_OBJECT(object))
            {
                ...
                if (OBJECT_getType(o) == OBJTYPE_CHARA)
                {
                    ...
                    getnum++;
                    snprintf(line, sizeof(line), "第%d个找到[%s]的帐号是[%s]声望[%d]", getnum, CHAR_getChar(chara_index, CHAR_NAME), CHAR_getChar(chara_index, CHAR_CDKEY), CHAR_getInt(chara_index, CHAR_FAME) / 100);
    ...
    for (k = 0; k < MAX_USER; k++)
    {
        ...
        if (i_use)
        {
            ...
            if (strcmp(cdkey, cdkeytoken) == 0)
            {
                ...
                if (sendnum <= 1)
                {
                    CHAR_talkToCli(charaindex, -1, "  围内没有别的玩家", CHAR_COLORYELLOW);
                }
...
void CHAR_CHAT_DEBUG_petmakecf(int charaindex, char* message)
{
    ...
    else
    {
        ...
        CHAR_setInt(petindex, CHAR_HP, CHAR_getWorkInt(petindex, CHAR_WORKMAXHP));
        CHAR_setInt(petindex, CHAR_EXP, RAND(2000, 15000));
        CHAR_complianceParameter(petindex);
        snprintf(token, sizeof(token), "K%d", i);
        CHAR_sendStatusString(charaindex, token);
        snprintf(token, sizeof(token), "W%d", i);
        ...
        for (j = x - area; j <= x + area; j++)
        {
            for (object = MAP_getTopObj(floor, j, i) ; object ; object = NEXT_OBJECT(object))
            {
                ...
                if (OBJECT_getType(o) == OBJTYPE_CHARA)
                {
                    ...
                    if (CHAR_getInt(chara_index, CHAR_WHICHTYPE) != CHAR_TYPEPLAYERNPC || CHAR_getWorkInt(chara_index, CHAR_WORKPARTYMODE) != CHAR_PARTY_LEADER)
                    {
                        continue;
                    }
                    ...
                    for (k = 0; k < 5; k++)
                    {
                        jiarenindex[k] = CHAR_getWorkInt(chara_index, CHAR_WORKPARTYINDEX1 + k);

                        if (CHAR_CHECKINDEX(jiarenindex[k]) == FALSE || CHAR_getInt(jiarenindex[k], CHAR_WHICHTYPE) != CHAR_TYPEPLAYERNPC)
                        {
                            continue;
                        }
                    }
...
void CHAR_CHAT_DEBUG_remotecommand(int charaindex, char*message)
{
    char magicname[128];
    char temp[256];
    ...
    {
        CHAR_talkToCli(charaindex, -1, "指令格式错误1！", CHAR_COLORRED);
        return;
    }

    if (strlen(message) == strlen(magicname))
    {
        saacproto_remoteCommand_send(magicname, "", charaindex, CHAR_getChar(charaindex, CHAR_CDKEY));
    }
    else
    {
        saacproto_remoteCommand_send(magicname, message + strlen(magicname) + 1, charaindex, CHAR_getChar(charaindex, CHAR_CDKEY));
    }
}

void remoteCommandUse(char *magicname, char *data, int charaindex, char *cdkey)
{
    CHATMAGICFUNC   func;
    ...
    if (func)
    {
        ...
        MAGIC_getLUAListFunction(magicname, 3, index, data);
