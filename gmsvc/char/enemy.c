...
int *ENEMY_getEnemy(int charaindex, int x, int y)
{
    ...
    if (found >= 2 && CHAR_getInt(charaindex, CHAR_ENEMY_MIN_TIME) > 0 && r_max >= 20)
    {
        /*int wrtemp = -1;
        int tempfond = -1;
        for(i=0;i<found;i++){
        ...
        for (i = 0; i < found; i++)
        {
            if (wr[i] <= wrtemp)
            {
                ...
                if (CHAR_getInt(charaindex, CHAR_ENEMY_MIN_BASE) < 3)
                {
                    basetemp = 2;
                }
                else if (CHAR_getInt(charaindex, CHAR_ENEMY_MIN_BASE) < 5)
                {
                    basetemp = 3;
                }
                else if (CHAR_getInt(charaindex, CHAR_ENEMY_MIN_BASE) < 10)
                {
                    basetemp = 7;
                }
                else if (CHAR_getInt(charaindex, CHAR_ENEMY_MIN_BASE) == 99)
                {
                    basetemp = 19;
                }

                r_max = r_max + wr[i] * basetemp;
                wr[i] = wr[i] + wr[i] * basetemp;
            }
        }
    }
    ...
    for (i = ENEMY_ID1; i < CREATEPROB1; i ++)
    {
        ...
        if (e_array != -1)
        {
            ...
            if (ENEMY_getInt(e_array, ENEMY_LV_MIN) == 1 && CHAR_getInt(charaindex, CHAR_ENEMY_MIN_TIME) > 0)
            {
                int basetemp = 0;
                ...
                wr[found] = wr[found] + wr[found] * basetemp;
            }
...
int *ENEMY_getEnemyShow(int charaindex, int x, int y)
{
    int     i;
    int     array;
    int     g_array;
    ...
    for (i = 0; i < ENCOUNT_GROUPMAXNUM; i ++)
    {
        if (ENCOUNT_getGroupIdFromArray(array, i) != - 1)
        {
            groupid = ENCOUNT_getGroupIdFromArray(array, i);
            g_array = GROUP_getGroupArray(groupid);
            int itemid = GROUP_getInt(g_array, GROUP_APPEARBYITEMID);
            int j;

            for (j = ENEMY_ID1; j < CREATEPROB1; j ++)
            {
                e_array = ENEMY_getEnemyArrayFromIndex(g_array, j - ENEMY_ID1);
                ...
                if (e_array != -1)
                {
                    ...
                    if (zhao == 0)
                    {
                        work[found * 2] = e_array;
                        work[found * 2 + 1] = itemid + 1;
                        found ++;
                    }
                }
            }
    ...
    work[found * 2] = -1;
    return found > 0 ? work : NULL;
}

int *ENEMY_getEnemyShowId(int charaindex, int x, int y)
{
    int     i;
    int     array;
    int     g_array;
    ...
    for (i = 0; i < ENCOUNT_GROUPMAXNUM; i++)
    {
        ...
        {
            groupid = ENCOUNT_getGroupIdFromArray(array, i);
            encountid = ENCOUNT_getEncountIndexFromArray(g_array);
            g_array = GROUP_getGroupArray(groupid);
            int itemid = GROUP_getInt(g_array, GROUP_APPEARBYITEMID);
            int j;
            ...
            for (j = ENEMY_ID1; j < CREATEPROB1; j++)
            {
                ...
                if (e_array != -1)
                {
                    ...
                    if (zhao == 0)
                    {
                        work[found * 4] = e_array;
                        work[found * 4 + 1] = itemid + 1;
                        work[found * 4 + 2] = groupid;
                        work[found * 4 + 3] = encountid;
                        found++;
                    }
                }
            }
...
#undef  PARAM_CAL
    ...
    CHAR_complianceParameter(newindex);
    CHAR_setInt(newindex, CHAR_LASTTALKELDER, 0);
    CHAR_setInt(newindex, CHAR_HP, CHAR_getWorkInt(newindex, CHAR_WORKMAXHP));
...
int ENEMY_createPetFromEnemyIndexTest(int charaindex, int array)
{
    Char    CharNew;
    int newindex;
    int     *p;
...
int ENEMY_createPetFromTempNo(int tempno)
{
    Char    CharNew;
    int newindex;
    int     tp[E_T_DATAINTNUM];
    ...
    int     level;
    int     enemyrank;
    tarray = ENEMYTEMP_getEnemyTempArrayFromTempNo(tempno);
...
#define     PARAM_CAL( l)   ( (E_PAR(ENEMY_LV) -1)*ET_PAR( E_T_LVUPPOINT)+ ET_PAR(E_T_INITNUM)) * ET_PAR( (l))
    ...
    enemyrank = ENEMY_getRank(-1, tarray);
    CharNew.data[CHAR_PETRANK] = enemyrank;
#undef  E_PAR
#undef  ET_PAR
#undef  PARAM_CAL
...
#undef  PARAM_CAL
    ...
    char pet4vbuff[128];
    memset(pet4vbuff, 0, sizeof(pet4vbuff));
    sprintf(pet4vbuff, "%d|%d|%d|%d|1", CHAR_getWorkInt(petindex, CHAR_WORKMAXHP), CHAR_getWorkInt(petindex, CHAR_WORKFIXSTR), CHAR_getWorkInt(petindex, CHAR_WORKFIXTOUGH), CHAR_getWorkInt(petindex, CHAR_WORKFIXDEX));
    CHAR_setChar(petindex, CHAR_PET_4V, pet4vbuff);
#endif
    CHAR_setInt(petindex, CHAR_LASTTALKELDER, 0);
...
#undef  PARAM_CAL
    ...
    char pet4vbuff[128];
    memset(pet4vbuff, 0, sizeof(pet4vbuff));
    sprintf(pet4vbuff, "%d|%d|%d|%d|%d", CHAR_getWorkInt(petindex, CHAR_WORKMAXHP), CHAR_getWorkInt(petindex, CHAR_WORKFIXSTR), CHAR_getWorkInt(petindex, CHAR_WORKFIXTOUGH), CHAR_getWorkInt(petindex, CHAR_WORKFIXDEX), CHAR_getInt(petindex, CHAR_LV));
    CHAR_setChar(petindex, CHAR_PET_4V, pet4vbuff);
#endif
    CHAR_setInt(petindex, CHAR_HP, CHAR_getWorkInt(petindex, CHAR_WORKMAXHP));
