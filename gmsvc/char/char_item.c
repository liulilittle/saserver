...
BOOL CHAR_sendItemData(int charaindex, int *itemgroup, int num)
{
    ...
    for (i = 0; i < num; i ++)
    {
        ...
        if (itemindex >= 0)
        {
            snprintf(token, sizeof(token), "%d|%s|", itemgroup[i], ITEM_makeItemStatusString(charaindex, - 1, itemindex));
        }
        else
        {
            snprintf(token, sizeof(token), "%s|", ITEM_makeItemFalseStringWithNum(charaindex, itemgroup[i]));
        }
...
void CHAR_DropItem(int charaindex,  int itemcharaindex)
{
    ...
    {
        char dropmsg[256];
        sprintf(dropmsg, "%s 此道具为绑定道具，无法丢弃，请妥善保管。", ITEM_getChar(itemindex, ITEM_NAME));
        CHAR_talkToCli(charaindex, -1, dropmsg, CHAR_COLORYELLOW);
        return;
    }

#endif
#ifdef _LIMITTIMEITEM_

    if (ITEM_getInt(itemindex, ITEM_USETIME) > 0)
    {
        char token[256];
        snprintf(token, sizeof(token), "限时物品不可丢弃。",
                 ITEM_getChar(itemindex, ITEM_NAME));
    ...
    if (FreeDropItem(charaindex, itemindex, 1) == 0)
    {
        return;
    }
    ...
    {
        CHAR_newMessageToCli(charaindex, -1, "周围的地面已经满了。", CHAR_COLORYELLOW);
        return;
    }
    ...
    else
    {
        ...
        {
            char tmpbuf[256];
            sprintf(tmpbuf, "您已成功丢弃道具[%s]（消失时间：%d秒）", ITEM_getChar(itemindex, ITEM_NAME), getItemdeletetime());
            CHAR_talkToCli(charaindex, -1, tmpbuf, CHAR_COLORYELLOW);
            LogItem(
                CHAR_getChar(charaindex, CHAR_NAME),   /* 平乓仿   */
                CHAR_getChar(charaindex, CHAR_CDKEY),
...
void CHAR_DropItem_Num(int charaindex,  int itemcharaindex, int num)
{
    int itemindex, fl, x, y, ret, objindex, beDropOne = 1;
    ...
    if (FreeDropItem(charaindex, itemindex, num) == 0)
    {
        return;
    }
            ...
            ITEM_getInt(itemindex, ITEM_USEPILENUMS) > 1)
    {
        ...
        if (num < nums)
        {
            ...
            ITEM_setInt(itemindex, ITEM_USEPILENUMS, nums - num);
            ITEM_setChar(ret, ITEM_NAME, ITEM_getChar(itemindex, ITEM_NAME));
            ITEM_setChar(ret, ITEM_SECRETNAME, ITEM_getChar(itemindex, ITEM_SECRETNAME));
            ITEM_setChar(ret, ITEM_ARGUMENT, ITEM_getChar(itemindex, ITEM_ARGUMENT));
            itemindex = ret;
            ITEM_setInt(itemindex, ITEM_USEPILENUMS, num);
            beDropOne = 0;
        }
    }
...
static int CHAR_PickUpItemObjIndex(int charaindex, int index,
                                   int* contents)
{
    OBJECT  object;
    ...
    else if (objtype == OBJTYPE_ITEM)
    {
        ...
        {
            if (strlen(ITEM_getChar(itemindex, ITEM_CDKEY)) != 0 && strcmp(ITEM_getChar(itemindex, ITEM_CDKEY),
                    CHAR_getChar(charaindex, CHAR_CDKEY)) != 0)
            {
                ...
                {
                    CHAR_talkToCli(charaindex, -1, "别人丢的东西不能捡哦！", CHAR_COLORYELLOW);
                    return -6;
                }
            }
        }
        ...
        if (ITEM_getInt(itemindex, ITEM_ID) == CASINOTOKENID)
        {
            ...
            for (i = 0; i < arraysizeof(casinomap); i++)
            {
                if (OBJECT_getFloor(index) == casinomap[i].casinofl)
                {
                    if (OBJECT_getX(index) == casinomap[i].casinox && OBJECT_getY(index) == casinomap[i].casinoy)
                    {
                        int j = 0, stakeflag = 0;
                        int pickupflag = 0;
                        casinoflag = 1;
    ...
    else if (objtype == OBJTYPE_CHARA)
    {
        ...
        if (CHAR_getInt(pindex, CHAR_WHICHTYPE) == CHAR_TYPEPET)
        {
            ...
            {
                if ((CHAR_getInt(pindex, CHAR_LV) - CHAR_getInt(charaindex, CHAR_LV) > getPlayerPetLv())
...
#endif
            {
                ...
                else
                {
                            ...
                            strcmp(CHAR_getChar(pindex, CHAR_OWNERCHARANAME), CHAR_getChar(charaindex, CHAR_NAME)))
                    {
                        ...
                        if (CHAR_getInt(pindex, CHAR_LV) > getOtherPetUpLv())
                        {
                            char petupbuff[128];
                            memset(petupbuff, 0, sizeof(petupbuff));
                            sprintf(petupbuff, "%d级以上宠物无法捡取，需要交易请通过交易功能。", getOtherPetUpLv());
                            CHAR_talkToCli(charaindex, -1, petupbuff, CHAR_COLORYELLOW);
                            return -5;
                        }

                        int petid = CHAR_getInt(pindex, CHAR_PETID);
                        int i;
                        ...
                        for (i = 0; i < 64; i++)
                        {
                            ...
                            {
                                CHAR_talkToCli(charaindex, -1, "珍惜宠物无法捡取，需要交易请通过交易功能。", CHAR_COLORYELLOW);
                                return -5;
                            }
                        }
            ...
            if (CHAR_getInt(pindex, CHAR_PETID) != 718)
            {
                ...
                if (strcmp(CHAR_getChar(pindex, CHAR_OWNERCDKEY), CHAR_getChar(charaindex, CHAR_CDKEY)) ||
                        strcmp(CHAR_getChar(pindex, CHAR_OWNERCHARANAME), CHAR_getChar(charaindex, CHAR_NAME)))
                {
                    CHAR_setInt(pindex, CHAR_SUPER, 0);
                }
...
void CHAR_PickUpItemObj(int charaindex, int objindex)
{
    int i;
    int dirx[2], diry[2];
    ...
    int contents;
    int ret = CHAR_PickUpItemObjIndex(charaindex, objindex, &contents);
    int x = OBJECT_getX(objindex);
    int y = OBJECT_getY(objindex);
            ...
            object = NEXT_OBJECT(object))
    {
        ...
        else if (objtype == OBJTYPE_GOLD)
        {
            ...
            *objindex = index;
            ...
            sprintf(buff, "丢出%d石币", amount);
            CHAR_newMessageToCli(charaindex, -1, buff, CHAR_COLORWHITE);
            return 0;
        }
        else if (objtype == OBJTYPE_ITEM)
        {
...
void CHAR_DropMoney(int charaindex,  int amount)
{
    ...
    {
        CHAR_newMessageToCli(charaindex, -1, "交易状态中无法丢出石币。", CHAR_COLORYELLOW);
        return;
    }
...
#ifdef _DROPSTAKENEW
        {
            ...
            for (j = 0; j < arraysizeof(casinomap); j++)
            {
                if (CHAR_getInt(charaindex, CHAR_FLOOR) == casinomap[j].casinofl)
                {
                    if (x == casinomap[j].casinox && y == casinomap[j].casinoy)
                    {
                        CHAR_newMessageToCli(charaindex, -1, "这里不能放置石币！", CHAR_COLORYELLOW);
                        return;
                    }
                }
            }
...
void CHAR_sendPetItemData(int charaindex, int petid)
{
    ...
    for (i = 0 ; i < CHAR_MAXPETITEMHAVE ; i ++)
    {
        ...
        if (itemindex >= 0)
        {
            snprintf(token, sizeof(token), "%s|", ITEM_makeItemStatusString(charaindex, -1, itemindex));
        }
        else
        {
