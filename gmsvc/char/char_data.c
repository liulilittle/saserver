...
{
#ifdef _NEW_FACEIMAGE
    int a = (faceimagenumber & 0xff0000) >> 16;
    int b = (faceimagenumber & 0xff00) >> 8;
    int c = faceimagenumber & 0xff;
#ifdef _IMAGE_EXTENSION

    if (a >= 0 && a <= 17 && b >= 0 && b <= 4 && c >= 0 && c <= 4)
#else
    if (a >= 0 && a <= 11 && b >= 0 && b <= 4 && c >= 0 && c <= 4)
#endif // _IMAGE_EXTENSION
    {
        return TRUE;
    }
    else
...
int CHAR_eqimagetbl[][5] =
{
    ...
    { 100730, 100731, 100732, 100733, 100734 },
    { 100735, 100736, 100737, 100738, 100739 },
    { 100740, 100741, 100742, 100743, 100744 },
    ...
    { 100815, 100816, 100817, 100818, 100819 },
#endif
};
...
#ifdef  _NEW_UNIFIDE_MALINASI
    ch->data[CHAR_FLOOR] = elders[1].floor;
    ch->data[CHAR_X] = elders[1].x;
    ch->data[CHAR_Y] = elders[1].y;
#else
...
int CHAR_GetLevelExp(int charaindex, int level)
{
    ...
    if (level > getMaxLevel(charaindex))
    {
    ...
    else if (level > getYBLevel())
    {
           ...
           )
        {
            if (getPettrans() != -1)
            {
                ...
                {
                    return getNeedLevelUpTbls(charaindex, level);
                }
            }
            else
            {
...
int CHAR_PetLevelUp(int petindex)
{
    ...
    //    gettimeofday(&tv, &tz);
    //    srand(tv.tv_usec);
    //    int randcnt = ((RAND(1, 1000) % 98) * 2 + 5) % 11 + 1;
    //    int j;
    //    for (j = 0; j < randcnt; j++)
    //    {
    ...
    fRand = (float)RAND(RankRandTbl[petrank].min, RankRandTbl[petrank].max) * 0.01;
    //}
    int luafRand = FreePetLevelUpRand(petindex);

    if (luafRand > 0)
    {
        fRand = (float)luafRand * 0.01;
    }
...
#endif
                    feedpoint = FreeGetFame(ownerindex, feedpoint, level);

                    if (CHAR_getWorkInt(petindex, CHAR_FAMELEVEL) == CHAR_getInt(petindex, CHAR_LV))
                    {
                        return 0;
                    }

                    CHAR_setWorkInt(petindex, CHAR_FAMELEVEL, CHAR_getInt(petindex, CHAR_LV));
...
int CHAR_GMPetLevelUp(int petindex, int zhang)
{
    struct _RankRandTbl
    {
        int min;
    ...
    float Param[4] = { 0.0, 0.0, 0.0, 0.0 };
    int LevelUpPoint, petrank, i;
    float str, vital, dex, tgh, fRand;
#ifdef _TEACHER_SYSTEM
    int iGetFame = 0;
#endif
...
#endif
                    feedpoint = FreeGetFame(ownerindex, feedpoint, level + 1);
...
char* PET_get4v(int petindex)
{
    ...
    int zhuhp, zhustr, zhutgh, zhudex;
    zhuhp = 0;
    zhustr = 0;
    zhutgh = 0;
    zhudex = 0;
    ...
    if (strlen(CHAR_getChar(petindex, CHAR_NEWNAME)) > 0)
    {
        ...
        if (getStringFromIndexWithDelim(CHAR_getChar(petindex, CHAR_NEWNAME), "|", 1, zhutmp, sizeof(zhutmp)) != FALSE)
        {
            zhuhp = atoi(zhutmp);
        }

        if (getStringFromIndexWithDelim(CHAR_getChar(petindex, CHAR_NEWNAME), "|", 2, zhutmp, sizeof(zhutmp)) != FALSE)
        {
            zhustr = atoi(zhutmp);
        }

        if (getStringFromIndexWithDelim(CHAR_getChar(petindex, CHAR_NEWNAME), "|", 3, zhutmp, sizeof(zhutmp)) != FALSE)
        {
            zhutgh = atoi(zhutmp);
        }

        if (getStringFromIndexWithDelim(CHAR_getChar(petindex, CHAR_NEWNAME), "|", 4, zhutmp, sizeof(zhutmp)) != FALSE)
        {
            zhudex = atoi(zhutmp);
        }
    }

    int hp = ((CHAR_getInt(petindex, CHAR_VITAL) - zhuhp * 100) * 4
              + (CHAR_getInt(petindex, CHAR_STR) - zhustr * 100) * 1
              + (CHAR_getInt(petindex, CHAR_TOUGH) - zhutgh * 100) * 1
              + (CHAR_getInt(petindex, CHAR_DEX) - zhudex * 100) * 1) * 0.01;
    int strint = (CHAR_getInt(petindex, CHAR_STR) - zhustr * 100) * 0.01 * 1.0
                 + (CHAR_getInt(petindex, CHAR_TOUGH) - zhutgh * 100) * 0.01 * 0.1
                 + (CHAR_getInt(petindex, CHAR_VITAL) - zhuhp * 100) * 0.01 * 0.1
                 + (CHAR_getInt(petindex, CHAR_DEX) - zhudex * 100) * 0.01 * 0.05;
    int tghint = (CHAR_getInt(petindex, CHAR_TOUGH) - zhutgh * 100) * 0.01 * 1.0
                 + (CHAR_getInt(petindex, CHAR_STR) - zhustr * 100) * 0.01 * 0.1
                 + (CHAR_getInt(petindex, CHAR_VITAL) - zhuhp * 100) * 0.01 * 0.1
                 + (CHAR_getInt(petindex, CHAR_DEX) - zhudex * 100) * 0.01 * 0.05;
    int dexint = (CHAR_getInt(petindex, CHAR_DEX) - zhudex * 100) * 0.01;
    static char pet4vbuff[64];
    memset(pet4vbuff, 0, sizeof(pet4vbuff));
    sprintf(pet4vbuff, "%d|%d|%d|%d", hp, strint, tghint, dexint);
    return pet4vbuff;
}
#endif // _PET_NOZHUFU_TYPE
