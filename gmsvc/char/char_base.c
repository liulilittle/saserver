...
char* CHAR_setintdata[CHAR_DATAINTNUM] =
{
    ...
    "ini",              /*  CHAR_INITDATA   */
    "silent",              /*  CHAR_SILENT   */
    "fmi",        // CHAR_FMINDEX CoolFish: Family 2001/5/24 家族 index
    "fmlf",       // CHAR_FMLEADERFLAG CoolFish: Family 2001/5/24 是否为家族族长
...
INLINE int _CHAR_setInt(char *file, int line, int index, CHAR_DATAINT element, int data)
{
    ...
    buf = CHAR_chara[index].data[element];
    CHAR_chara[index].data[element] = data;
    FreeSetInt(index, element, buf, data, file, line);
    return buf;
}
...
#endif
{
    ...
    //FreeSetWorkInt(index, element, buf, data, file, line);
#ifdef _NEW_ADDEXPTIME_SHOW

    if (element == CHAR_WORKITEM_ADDEXPTIME)
    {
        CHAR_send_P_StatusString(index, CHAR_P_STRING_ADDEXPTIME);
    }

#endif // _NEW_ADDEXPTIME_SHOW
#ifdef _NEWPARTY_OFFLINE

    if (element == CHAR_WORK_OFFLINE && buf != data)
    {
        if (CHAR_getWorkInt(index, CHAR_WORKPARTYMODE) != CHAR_PARTY_NONE)
        {
            ...
            {
                CHAR_sendStatusString(index, "N0");
                toindex = index;
            }
            else
            {
                toindex = CHAR_getWorkInt(index, CHAR_WORKPARTYINDEX1);
            }
            ...
            {
                int partyindex = CHAR_getWorkInt(toindex, jj + CHAR_WORKPARTYINDEX1);
            ...
            for (jj = 0; jj < getPartyNum(toindex); jj++)
            {
                ...
                {
                    snprintf(c, sizeof(c), "N%d", ii);
                    CHAR_sendStatusString(partyindex, c);
                }
            }
        }
    }
...
INLINE BOOL _CHAR_setFlg(char *file, int line, int index, CHAR_DATACHAR element, int newdata)
{
    ...
    if (element == CHAR_ISDIE && newdata == 1)
    {
        if (CHAR_getInt(index, CHAR_WHICHTYPE) == CHAR_TYPEPET && CHAR_getWorkInt(CHAR_getWorkInt(index, CHAR_WORKPLAYERINDEX), CHAR_WORKBATTLEINDEX) > -1)
        {
            CHAR_setWorkInt(index, CHAR_WORKBATTLEDIETYPE, 1);
        }
    }
...
int CHAR_getCharSkillLevel(int index, int sindex)
{
    ...
    if (hskill != NULL)
    {
        return SKILL_getRealInt(&hskill->skill, SKILL_LEVEL);
    }
    else
    {
        return -1;
...
void CHAR_setCharSkillLevel(int index, int sindex, int new)
{
    CHAR_HaveSkill* hskill;
    hskill = CHAR_getCharHaveSkill(index, sindex);
    SKILL_setInt(&hskill->skill, SKILL_LEVEL, new);
}
#endif
...
INLINE lua_State *CHAR_getLUAFunction(int charaindex, int functype)
{
    ...
    if (!lua_isfunction(CHAR_chara[charaindex].lua[functype], -1))
    {
        lua_pop(CHAR_chara[charaindex].lua[functype], 1);
        print("Charaindex: [%d], has no func [%s]!, type:[%d]", charaindex, CHAR_chara[charaindex].luafunctable[functype], functype);
        return NULL;
    }
...
#else
            feedpoint = needexp / 20000;
#endif
            feedpoint = FreeGetFame(charaindex, feedpoint, level);
...
BOOL CHAR_makeDepotItemStringToChar(int charaindex, char* data)
{
    ...
    while (1)
    {
        ...
        if ((ret = getStringFromIndexWithDelim(linebuf, "=",  1,
                                               firstToken, sizeof(firstToken))) == FALSE)
        {
            return FALSE;
        }
...
char* CHAR_makeDepotPetFromCharIndex(int charaindex)
{
    ...
    for (i = 0 ; i < CHAR_MAXDEPOTPETHAVE ; i ++)
    {
        ...
        snprintf(linedata, sizeof(linedata),
                 DEPOTPETRESERVESTRING "%d=%s" CHAR_DELIMITER, petcnt, petstring);
        petcnt++;
        strcpysafe(&CHAR_dataString[strlength], sizeof(CHAR_dataString) - strlength,
                   linedata);
        strlength += strlen(linedata);
