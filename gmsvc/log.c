            ...
            if (strcmp(LogConf[i].entry, firstToken) == 0)
            {
                ...
                if (opendir(dirname) == NULL)
                {
                    if (mkdir(dirname, 0777) == 0)
                    {
                        snprintf(dirname,
                                 sizeof(dirname),
                                 "%s/%s/%d", basedir, datedir, getServernumber());
                ...
                snprintf(LogConf[i].filename,
                         ...
                         "%s/%s/%d/%s", basedir, datedir, getServernumber(), secondToken);
                snprintf(LogConf[i].name,
                         sizeof(LogConf[i].name),
                         "%s", secondToken);
            }
        }
    }
...
{
#ifdef _MYSQLLOG_
    extern void streetvendorUpdate(
        char *SellName,
        char *SellID,
        char *BuyName,
        char *BuyID,
    ...
    streetvendorUpdate(SellName, SellID, BuyName, BuyID, ItemPetName, PetLv, iPrice, Key, Sfloor, Sx, Sy, Bfloor, Bx, By, uniquecode);
#else
    struct  tm tm1;
    memcpy(&tm1, localtime((time_t *)&NowTime.tv_sec), sizeof(tm1));
    char buf[256];
...
void backupAllLogFile(struct tm *ptm)
{
    ...
    char datedir[32];
    memset(datedir, 0, sizeof(datedir));
    sprintf(datedir, "%4d%02d%02d", ptm->tm_year + 1900, ptm->tm_mon + 1, ptm->tm_mday);
    char dirname[256];
    snprintf(dirname,
             sizeof(dirname),
             "%s/%s", basedir, datedir);
    ...
    for (i = 0 ; i < arraysizeof(LogConf) ; i++)
    {
        ...
        if (LogConf[i].f != NULL)
        {
            ...
            snprintf(LogConf[i].filename,
                     sizeof(LogConf[i].filename),
                     "%s/%s/%d/%s", basedir, datedir, getServernumber(), LogConf[i].name);
            LogConf[i].f = fopen(LogConf[i].filename, "a");
        }
        else
        {
