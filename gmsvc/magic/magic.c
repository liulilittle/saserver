...
int MAGIC_Use(int charaindex, int haveitemindex, int toindex)
{
    ...
    char magicname[64];
    memset(magicname, 0, sizeof(magicname));
    strcpy(magicname, FreeMagicName(charaindex, magicid));

    if (strcmp(magicname, "NULL") == 0)
    {
        func = MAGIC_getMagicFuncPointer(MAGIC_getChar(marray, MAGIC_FUNCNAME));    //MAGIC_NAME
    }
    else
    {
        func = MAGIC_getMagicFuncPointer(magicname);    //MAGIC_NAME
    }

#else
    func = MAGIC_getMagicFuncPointer(MAGIC_getChar(marray, MAGIC_FUNCNAME));  //MAGIC_NAME
#endif // _NEWUI_MAGICUSE
...
// shan add begin
    ...
    {
        int myfmsprite = CHAR_getInt(charaindex, CHAR_FMSPRITE);
#ifdef  _MAGIC_FAMILY_FMSPRITE_FIVE

        if (CHAR_getWorkInt(charaindex, CHAR_WORKFMFLOOR) == 6031 || CHAR_getWorkInt(charaindex, CHAR_WORKFMFLOOR) == 5031)
        {
            int ridepet = CHAR_getInt(charaindex, CHAR_RIDEPET);

            if (ridepet > -1)
            {
                int ridepetindex = CHAR_getCharPet(charaindex, ridepet);
...
// shan end
    if (func)
    {
        ...
        if (magic_use_type == 1)
        {
            ...
            {
                sprintf(magicbuff, "您的[%s]无法在平时使用哦！", MAGIC_getChar(marray, MAGIC_NAME));
        ...
        else if (magic_use_type == 2)
        {
            if (BATTLE_CHECKINDEX(battleindex))
            {
                sprintf(magicbuff, "您的[%s]只能在非战斗状态下使用哦！", MAGIC_getChar(marray, MAGIC_NAME));
                CHAR_talkToCli(charaindex, -1, magicbuff, CHAR_COLORYELLOW);
                return FALSE;
            }
        }
        else if (magic_use_type == 3)
        {
            if (BATTLE_CHECKINDEX(battleindex))
            {
                ...
                {
                    sprintf(magicbuff, "您的[%s]无法在PK中使用哦！", MAGIC_getChar(marray, MAGIC_NAME));
        ...
        else if (magic_use_type == 4)
        {
            ...
            else
            {
                if (BattleArray[battleindex].type != BATTLE_TYPE_P_vs_P)
                {
                    sprintf(magicbuff, "您的[%s]只能在PK中使用哦！", MAGIC_getChar(marray, MAGIC_NAME));
                    CHAR_talkToCli(charaindex, -1, magicbuff, CHAR_COLORRED);
                    return FALSE;
                }
            }
...
#ifdef  _MAGICID_NOUSE_NOMP

        if ((magicid == 12 || magicid == 22 || magicid == 26) && mp == 0)
        {
            if (CHAR_getInt(charaindex, CHAR_FLOOR) == 1042 || CHAR_getInt(charaindex, CHAR_FLOOR) == 2032 || CHAR_getInt(charaindex, CHAR_FLOOR) == 3032 || CHAR_getInt(charaindex, CHAR_FLOOR) == 4032
                    || CHAR_getInt(charaindex, CHAR_FLOOR) == 5032 || CHAR_getInt(charaindex, CHAR_FLOOR) == 6032 || CHAR_getInt(charaindex, CHAR_FLOOR) == 12345)
            {
                sprintf(magicbuff, "您的[%s]无法在该地图中使用哦！", MAGIC_getChar(marray, MAGIC_NAME));
                CHAR_talkToCli(charaindex, -1, magicbuff, CHAR_COLORRED);
                return FALSE;
            }
        }
        ...
        if (!FreeMagicUse(charaindex, toindex, magicid, mp))
        {
            return FALSE;
        }
...
int MAGIC_Recovery(int charaindex, int toindex, int marray, int mp)
{
    ...
    if (mp == 0)
    {
        ...
        {
            CHAR_talkToCli(charaindex, -1, "族战地图不可使用免气精灵！", CHAR_COLORYELLOW);
            return FALSE;
        }
    }
...
int MAGIC_Recovery_2(int charaindex, int toindex, int marray, int mp)
{
    int battlemode;
...
int MAGIC_OtherRecovery_2(int charaindex, int toindex, int marray, int mp)
{
    int battlemode;
...
int MAGIC_MagicDef(int charaindex, int toindex, int marray, int mp)
{
    ...
    if (CHAR_getInt(charaindex, CHAR_FLOOR) == 12222)
    {
        char *magicarg = MAGIC_getChar(marray, MAGIC_OPTION);

        if (strncmp(magicarg, "无", 2) == 0)
        {
            CHAR_talkToCli(charaindex, -1, "该地图无法使用此技能。", CHAR_COLORYELLOW);
            return FALSE;
        }
    }
