...
BOOL MAP_readMapOne(char*   filename)
{
    ...
    char    showstring[CHARNAMELEN];         /*  穴永皿犯□正及域凛伞  桦赭  */
                              ...
                              int *tilesum, int *objsum, int *eventsum)
{
    ...
    RECT    scr;
    char    escapebuffer[128];
    unsigned short  eventbuf[MAPCHARDEFALUTSEESIZ * MAPCHARDEFALUTSEESIZ];
    unsigned short  tilebuf[ MAPCHARDEFALUTSEESIZ * MAPCHARDEFALUTSEESIZ];
    unsigned short  objbuf[  MAPCHARDEFALUTSEESIZ * MAPCHARDEFALUTSEESIZ];
    floorindex = MAP_getfloorIndex(floor);
...
void MAP_sendAroundMapdata(int fl, int fromx, int fromy)
{
    ...
    if (mapdata != NULL)
    {
        ...
        for (i = x - MAPCHARDEFALUTSEESIZ / 2 ; i <= x + MAPCHARDEFALUTSEESIZ / 2 ; i++)
        {
            for (j = y - MAPCHARDEFALUTSEESIZ / 2 ; j <= y + MAPCHARDEFALUTSEESIZ / 2 ;
                    j ++)
            {
                OBJECT  object;
