...
int ITEM_eventDrop(int itemindex, int charaindex, int itemcharaindex)
{
    ...
    snprintf(szBuffer, sizeof(szBuffer), "%s 消失了。",
    ...
    if (OBJECT_getType(moveobjindex) == OBJTYPE_CHARA)
    {
        ...
        if (CHAR_getInt(moveindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
        {
            snprintf(szBuffer, sizeof(szBuffer), "%s",
                     ITEM_getAppropriateName(itemindex));
            CHAR_talkToCli(moveindex, -1, "%s 消失了。", CHAR_COLORWHITE);
        }
    }
...
)
{
    ...
    if ((p = strstr(arg, "PROPERTY")) != NULL)
    {
        ...
        for (i = 0; i < 9; i++)
        {
            myitemindex = CHAR_getItemIndex(toindex, i);

            if (ITEM_CHECKINDEX(myitemindex))
            {
                if (ITEM_getInt(myitemindex, ITEM_MODIFYATTRIB) > 0)
                {
                    CHAR_talkToCli(charaindex, -1, "请脱下属性装备后使用", CHAR_COLORWHITE);
                    return;
                }
            }
        }
    ...
    for (j = 0; j < BD_KIND_END; j ++)
    {
        ...
        if (charaindex != toindex)
        {
            ...
            CHAR_newMessageToCli(charaindex, -1, msgbuf, CHAR_COLORWHITE);
            ...
            if (CHAR_getInt(toindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
            {
                ...
                CHAR_newMessageToCli(toindex, -1, msgbuf, CHAR_COLORWHITE);
            }
        }
        else
        {
...
void ITEM_useStatusChange(int charaindex, int toindex, int haveitemindex)
{
    ...
    else if (battlemode)
    {
        ...
        if (BATTLE_CHECKINDEX(battleindex))
        {
            ...
            {
                CHAR_talkToCli(charaindex, -1, "很遗憾，串串类道具精灵无法在PVP中使用哦。", CHAR_COLORYELLOW);
                return;
            }
        }
...
void ITEM_usePetSkillCanned_WindowResult(int charaindex, int seqno, int select, char * data)
{
    ...
    if (*skillarg == 'A')
    {
        SkillID = FreeusePetSkill(charaindex, petindex, atoi(skillarg + 1));
...
void ITEM_randEnemyEquipOne(int charaindex, int toindex, int haveitemindex)
{
    ...
    if ((RandNum = atoi(buf)) > 0)
    {
        ...
        CHAR_setWorkInt(charaindex, CHAR_WORK_MOONACT_NUM, RandNum);
...
#ifdef _NEWPETSKILL_
        char * skillarg = ITEM_getChar(itemindex, ITEM_ARGUMENT);

        if (*skillarg == 'A')
        {
            if (FreeusePetSkill(charaindex, toindex, atoi(skillarg + 1)) == 0)
            {
                return;
            }
        }
...
void ITEM_Addexp(int charaindex, int toindex, int haveitemindex)
{
    ...
    if (CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEMODE) != BATTLE_CHARMODE_NONE)
    {
        ITEM_useAddexp_Effect(charaindex, charaindex, haveitemindex);
    }
    else
    {
        if (CHAR_CHECKINDEX(toindex) == FALSE)
...
void ITEM_Gold(int charaindex, int toindex, int haveitemindex)
{
    ...
    {
        char goldbuf[128];
        memset(goldbuf, 0, sizeof(goldbuf));
        sprintf(goldbuf, "使用失败，您身上的石币上限为%d，放不下更多石币了哦。", CHAR_getMaxHaveGold(charaindex));
        CHAR_talkToCli(charaindex, -1, goldbuf, CHAR_COLORYELLOW);
        return;
    }
...
void ITEM_ItemPetLocked(int charaindex, int toindex, int haveitemindex)
{
    ...
    else
    {
        char message[256];
        char    buf[256];
        sprintf(message, "您正在进行 「永久解除安全锁」 操作。\n请输入您的安全密码进行确认。\n解锁后彻底打开安全锁，无需输入安全码！\n\n此为高危操作，敬请慎重！");
        lssproto_WN_send(getfdFromCharaIndex(charaindex), WINDOW_MESSAGETYPE_MESSAGEANDLINEINPUT,
                         WINDOW_BUTTONTYPE_OKCANCEL,
                         CHAR_WINDOWTYPE_ITEM_PET_LOCKED_2,
                         -1,
...
void ITEM_SuperManItem(int charaindex, int toindex, int haveitemindex)
{
    ...
    //宠物转生任务
    NPC_NowEventSetFlg(charaindex, 69);
    NPC_NowEventSetFlg(charaindex, 70);
    NPC_EventSetFlg(charaindex, 69);
    NPC_EventSetFlg(charaindex, 70);
    //CHAR_Skillupsend( charaindex);
    ...
    CHAR_talkToCli(charaindex, -1, "您已经成为圆满人！", CHAR_COLORYELLOW);
    CHAR_talkToCli(charaindex, -1, "您已完成【成人任务】、【四大洞穴任务】、【转宠任务】！", CHAR_COLORYELLOW);
    CHAR_DelItem(charaindex, haveitemindex);
}
#endif
...
void ITEM_ExpLvBase(int charaindex, int exp)
{
    if (CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER && CHAR_getInt(charaindex, CHAR_LV) >= 140)
    {
        ...
        for (i = 9; i <= 23; i++)
        {
            toitemindex = CHAR_getItemIndex(charaindex, i);
            ...
            if (ITEM_CHECKINDEX(toitemindex))
            {
                if (strlen(ITEM_getChar(toitemindex, ITEM_USEFUNC)) > 0 && strcmp(ITEM_getChar(toitemindex, ITEM_USEFUNC), "ITEM_useExpLv") == 0)
                {
                    ...
                    while (1)
                    {
                        ...
                        nextexp = getNeedLevelUpTbls(-1, itemlv + 1);
...
void ITEM_useExpLv(int charaindex, int toindex, int haveitemindex)
{
...
#ifdef _NOTEAM_ITEMEXPLV

    if (CHAR_getWorkInt(charaindex, CHAR_WORKPARTYMODE) != CHAR_PARTY_NONE)
    {
        CHAR_talkToCli(charaindex, -1, "经验丹在组队模式下无法使用。", CHAR_COLORYELLOW);
        return;
    }
    ...
    if (CHAR_getInt(toindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
    {
        ...
        if (itemlevel < 120)
        {
            CHAR_talkToCli(charaindex, -1, "您的经验丹都还没搜集够经验呢，怎么使用呀？最低要求搜集到120级经验哦！", CHAR_COLORYELLOW);
            return;
        }
    ...
    {
        if (CHAR_getWorkInt(toindex, CHAR_WORKPLAYERINDEX) != charaindex)
        {
            return;
        }
        ...
        if (CHAR_getInt(toindex, CHAR_LIMITLEVEL) > 0)
        {
            if (itemlevel > CHAR_getInt(toindex, CHAR_LIMITLEVEL))
            {
                sprintf(token, "目前你的宠物不能修炼到%d等级，该经验丹无法使用。", itemlevel);
                CHAR_talkToCli(charaindex, -1, token, CHAR_COLORYELLOW);
                return;
...
#ifdef _ITEM_JIGSAW
char *GetJigsawString(int itemindex)
{
    int image = ITEM_getInt(itemindex, ITEM_BASEIMAGENUMBER);

    if (image >= 25150 && image <= 25159)
    {
        return ITEM_getChar(itemindex, ITEM_ARGUMENT);
    }
    else
    {
        return "";
...
int JigsawOK[][10] = //笵ㄣ┏狾,瓜絪腹
{
    {25151, 25160, 25161, 25162, 25163, 25164, 25165, 25166, 25167, 25168},
    {25152, 25169, 25170, 25171, 25172, 25173, 25174, 25175, 25176, 25177},
    {25153, 25178, 25179, 25180, 25181, 25182, 25183, 25184, 25185, 25186},
    {25154, 25187, 25188, 25189, 25190, 25191, 25192, 25193, 25194, 25195},
    {25155, 25196, 25197, 25198, 25199, 25200, 25201, 25202, 25203, 25204},
    {25156, 25205, 25206, 25207, 25208, 25209, 25210, 25211, 25212, 25213},
    {25157, 25214, 25215, 25216, 25217, 25218, 25219, 25220, 25221, 25222},
    {25158, 25223, 25224, 25225, 25226, 25227, 25228, 25229, 25230, 25231},
    {25159, 25232, 25233, 25234, 25235, 25236, 25237, 25238, 25239, 25240},
    {25150, 25151, 25152, 25153, 25154, 25155, 25156, 25157, 25158, 25159}
};
void ITEM_Jigsaw(int charaindex, int toindex, int haveitemindex)
{
    int itemindex, toitemindex, img, toimg, i, j, k;
    char arg[32], toarg[32];
    itemindex = CHAR_getItemIndex(charaindex, haveitemindex);
    ...
    toitemindex = CHAR_getItemIndex(charaindex, toindex);
    ...
    img = ITEM_getInt(itemindex, ITEM_BASEIMAGENUMBER);
    toimg = ITEM_getInt(toitemindex, ITEM_BASEIMAGENUMBER);
    strcpy(toarg, ITEM_getChar(toitemindex, ITEM_ARGUMENT));

    for (i = 0; i < 10; i++)
    {
        if (toimg == JigsawOK[i][0])
        {
            for (j = 1; j < 10; j++)
            {
                if (img == JigsawOK[i][j] && toarg[j - 1] == '0')
                {
                    char buf1[255] = "", buf2[255] = "", *bufp = NULL;
                    int total = 0;

                    for (k = 0; k < 10; k++)
                    {
                        if (img == JigsawOK[k][0]) //琌┏狾
                        {
                            if (strcmp(ITEM_getChar(itemindex, ITEM_ARGUMENT), "111111111"))//浪诡琌ЧΘ(ㄓ)
                            {
                                return;
                            }
                        }
                    ...
                    toarg[j - 1] = '1';
                    //э笵ㄣ弧
                    strcpy(buf1, ITEM_getChar(toitemindex, ITEM_SECRETNAME));

                    for (k = 0; k < strlen(buf1); k++)
                    {
                        if (buf1[k] == '(')
                        {
                            buf1[k] = '\0';
                    ...
                    sprintf(buf2, "%s(%d)", buf1, total);
                    ITEM_setChar(toitemindex, ITEM_SECRETNAME, buf2);
                    CHAR_setItemIndex(charaindex, haveitemindex, -1);
                    ITEM_endExistItemsOne(itemindex);//Change fix а,穦琵笵ㄣ脄奔 2005/09/26 干
                    CHAR_sendItemDataOne(charaindex, haveitemindex);
                    ITEM_setChar(toitemindex, ITEM_ARGUMENT, toarg);
