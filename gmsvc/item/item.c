...
#endif
#ifdef _ITEM_LOCKED
    { "locked", 0, ITEM_LOCKED },                /*  ITEM_LOCKED  */
#endif // _ITEM_LOCKED
#ifdef __NEW_CONSIGNMENT
	{"price",	0,		ITEM_PRICE },                /*  ITEM_USETIME  */
#endif
};
...
INLINE lua_State *ITEM_getLUAFunction(int itemindex, int functype)
{
    ...
    if (!lua_isfunction(ITEM_item[itemindex].itm.lua[functype - ITEM_FIRSTFUNCTION], -1))
    {
        lua_pop(ITEM_item[itemindex].itm.lua[functype - ITEM_FIRSTFUNCTION], 1);
        print("itemindex [%d], has no func [%s]!, type:[%d]", itemindex, ITEM_item[itemindex].itm.luafunctable[functype - ITEM_FIRSTFUNCTION], functype);
        return NULL;
    }
...
char*   ITEM_makeStringFromItemData(ITEM_Item* one, int mode)
{
    ...
    for (i = 0; i < arraysizeof(ITEM_setintdata) ; i ++)
    {
        ...
        if (!CHECK_HaveBeSave(itemID, one->data[ ITEM_setintdata[i].table], ITEM_setintdata[i].table))
        {
            continue;
        }
...
BOOL ITEM_makeExistItemsFromStringToArg(char* src, ITEM_Item* item, int mode)
{
...
#ifdef _SIMPLIFY_ITEMSTRING
    {
        ...
        while (1)
        {
            ...
            if (getStringFromIndexWithDelim(src, delim2, readindex, linebuf, sizeof(linebuf)) == FALSE)
            {
                break;
            }
            ...
            if (getStringFromIndexWithDelim(linebuf, delim1, 1, firstToken, sizeof(firstToken)) == FALSE)
            {
                return FALSE;
            }
...
char*  ITEM_makeItemStatusString(int charaindex, int haveitemindex, int itemindex)
{
    char    escapename[256];
...
#endif
#ifdef _ITEM_SHOW_VANISHATDROP
    int tradetype = 0;
#endif // _ITEM_SHOW_VANISHATDROP
    ...
    if (haveitemindex == -1)
    {
        ...
        if (arg[0] == '*' || ITEM_getInt(itemindex, ITEM_VANISHATDROP) == 1)
        {
            ...
            else
            {
                ...
                sprintf(buff1, "绑定物品 %d%%", (int)((crushe * 100) / maxcrushe));
            }
        }
        else
        {
            ...
            else
            {
                ...
                sprintf(buff1, "耐久度：%d%%", (int)((crushe * 100) / maxcrushe));
            }
        }
...
#endif
#ifdef _SEND_ITEM_TYPE

        if (CONNECT_getloginmark(CHAR_getWorkInt(charaindex, CHAR_WORKFD)) == 2 || CONNECT_getloginmark(CHAR_getWorkInt(charaindex, CHAR_WORKFD)) == 4)
        {
            sprintf(ITEM_itemStatusStringBuffer, "%s|%d", ITEM_itemStatusStringBuffer, itemtype);
        }

#endif // _SEND_ITEM_TYPE
    }
    else
    {
#ifdef _ADD_SHOW_ITEMDAMAGE  // WON ADD 显示物品耐久度
...
#ifdef _ITEM_LOCKED
    sprintf(ITEM_itemStatusStringBuffer, "%s|%d", ITEM_itemStatusStringBuffer, ITEM_getInt(itemindex, ITEM_LOCKED));
#endif
#ifdef _ITEM_SHOWCUSTEM
    char Mname[64];
    ...
    sprintf(ITEM_itemStatusStringBuffer, "%s|%s", ITEM_itemStatusStringBuffer, Mname);
    sprintf(ITEM_itemStatusStringBuffer, "%s|%s", ITEM_itemStatusStringBuffer, ITEM_getChar(itemindex, ITEM_TYPECODE));
    sprintf(ITEM_itemStatusStringBuffer, "%s|%s", ITEM_itemStatusStringBuffer, ITEM_getChar(itemindex, ITEM_INLAYCODE));
    sprintf(ITEM_itemStatusStringBuffer, "%s|%d", ITEM_itemStatusStringBuffer, ITEM_getInt(itemindex, ITEM_NEEDTRANS));
#endif // _ITEM_SHOWCUSTEM
    sprintf(ITEM_itemStatusStringBuffer, "%s|", ITEM_itemStatusStringBuffer);
    return ITEM_itemStatusStringBuffer;
}

char*   ITEM_makeItemFalseString(int charaindex)
{
#ifdef _ADD_SHOW_ITEMDAMAGE  // WON ADD 显示物品耐久度
...
char*   ITEM_makeItemFalseStringWithNum(int charaindex, int haveitemindex)
{
#ifdef _ADD_SHOW_ITEMDAMAGE  // WON ADD 显示物品耐久度
    snprintf(ITEM_itemStatusStringBuffer, sizeof(ITEM_itemStatusStringBuffer),
#ifdef _ITEM_PILENUMS
...
void ITEM_equipEffect(int index)
{
    ...
    if (CHAR_getInt(index, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
    {
        if (CHAR_getInt(index, CHAR_TITLE_NEW) > 0
#ifdef  _NOSHOW_TITLE_FAMILYBADGE
                && CHAR_getInt(index, CHAR_FLOOR) != 500 && CHAR_getInt(index, CHAR_FLOOR) != 34567
#endif
           )
        {
            char msg[256];
            sprintf(msg, "6|%d", CHAR_getInt(index, CHAR_TITLE_NEW));
            lssproto_CHAREFFECT_send(getfdFromCharaIndex(index), msg);
        }
    }
    ...
    for (i = 5 ; i < arraysizeof(itemEffect) ; i ++)
    {
        ...
        if (itemEffect[i].charmodifyparamindex == CHAR_WORKATTACHPILE)
        {
            CHAR_setWorkInt(index, itemEffect[i].charmodifyparamindex, CHAR_getWorkInt(index, itemEffect[i].charmodifyparamindex) + work);
        }
        else
        {
...
void Other_DefcharWorkInt(int index)
{
    ...
    if (CHAR_getInt(index, CHAR_BASEIMAGENUMBER) == 101428
            ...
            || CHAR_getInt(index, CHAR_BASEIMAGENUMBER) == 102011
            || CHAR_getInt(index, CHAR_BASEIMAGENUMBER) == 102012
#endif
       )
    {
        int Rands = 0, power;
...
char*  ITEM_petmakeItemStatusString(int petindex, int itemid)
{
    ...
    if (arg[0] == '*' || ITEM_getInt(itemindex, ITEM_VANISHATDROP) == 1)
    {
        ...
        else
        {
            ...
            snprintf(buff1, sizeof(buff1), "绑定物品 %d%%", (int)((crushe * 100) / maxcrushe));
        }
    }
    else
    {
        ...
        else
        {
            ...
            snprintf(buff1, sizeof(buff1), "耐久度：%d%%", (int)((crushe * 100) / maxcrushe));
        }
    }
