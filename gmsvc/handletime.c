...
static clock_t EndClock2 = 0;
static double SysTime = 0.0;
static int Cnum = 0;

#ifdef _ASSESS_SYSEFFICACY_SUB
static clock_t Net_TotalClock = 0;
static clock_t NetCli_TotalClock = 0;
static clock_t NetSaac_TotalClock = 0;
...
static double NetCli_SysTime = 0.0;
static double NetSaac_SysTime = 0.0;
...
static clock_t SubStartClock2 = 0;
#endif
...
void Assess_SysEfficacy(int flg)
{
    ...
    else if (flg == 1)
    {
        ...
        if (Cnum % 500 == 0)
        {
            ...
            Net_TotalClock = 0;
            NetCli_SysTime = (float)(NetCli_TotalClock / Cnum) / CLOCKS_PER_SEC;
            NetCli_TotalClock = 0;
            NetSaac_SysTime = (float)(NetSaac_TotalClock / Cnum) / CLOCKS_PER_SEC;
            NetSaac_TotalClock = 0;
...
void Assess_SysEfficacy_sub(int flg, int loop)
{
    ...
    else if (flg == 1)
    {
        ...
        switch (loop)
        {
            ...
            case 9: // NetCli_TotalClock
                NetCli_TotalClock += (int)(EndClock - SubStartClock);
                break;

            case 10: // NetSaac_TotalClock
                NetSaac_TotalClock += (int)(EndClock - SubStartClock);
                break;
        }
    }
}

void Assess_SysEfficacy_sub2(int flg, int loop)
{
    if (flg == 0)
    {
        SubStartClock2 = clock();
    }
    else if (flg == 1)
    {
        EndClock2 = clock();

        if (EndClock2 < SubStartClock2)
        {
            return;
        }
        ...
        {
            case 9: // NetCli_TotalClock
                NetCli_TotalClock += (int)(EndClock2 - SubStartClock2);
                break;

            case 10: // NetSaac_TotalClock
                NetSaac_TotalClock += (int)(EndClock2 - SubStartClock2);
                break;
        }
    }
}
...
void ASSESS_getSysEfficacy_sub(float *TVsec, int loop_index)
{
    switch (loop_index)
    {
        ...
        case 9:
            *TVsec = NetCli_SysTime;
            break;

        case 10:
            *TVsec = NetSaac_SysTime;
            break;
    }
}
