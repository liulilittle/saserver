...
static int isFaceToChara(lua_State *L)
{
    const int talkerindex = luaL_checkint(L, 1);
    const int meindex = luaL_checkint(L, 2);
    const int distance = luaL_checkint(L, 3);

    if (NPC_Util_isFaceToChara(talkerindex, meindex, distance) == FALSE)
    {
        lua_pushinteger(L, 0);
        return 1;
    }
...
{
    const int index = luaL_checkint(L, 1);
    int evNo = luaL_checkint(L, 2);
    lua_pushinteger(L, NPC_EventCheckFlg(index, evNo));
    return 1;
}

static int CheckNowEvent(lua_State *L)
{
    const int index = luaL_checkint(L, 1);
    int evNo = luaL_checkint(L, 2);
    lua_pushinteger(L, NPC_NowEventCheckFlg(index, evNo));
    return 1;
}
...
static int ClsEvent(lua_State *L)
{
    const int index = luaL_checkint(L, 1);
    int evNo = luaL_checkint(L, 2);
    NPC_NowEndEventSetFlgCls(index, evNo);
    return 1;
}
#ifdef _ONLINE_COST
...
static int SellPet(lua_State *L)
{
    size_t l;
    const int meindex = luaL_checkint(L, 1);
    const int charaindex = luaL_checkint(L, 2);
    const int select = luaL_checkint(L, 3);
    char *token = luaL_checklstring(L, 4, &l);
    NPC_PetDel(meindex, charaindex, select, token);
    return 1;
}

static int PoolPet(lua_State *L)
{
    const int charaindex = luaL_checkint(L, 1);
...
static int PoolItem(lua_State *L)
{
    const int charaindex = luaL_checkint(L, 1);
...
static int EventID(lua_State *L)
{
    size_t l;
    const int charaindex = luaL_checkint(L, 1);
    char *token = luaL_checklstring(L, 2, &l);
    NPC_ActionEventID(charaindex, token, 1);
    return 1;
}

static int CheckFreePetSkill(lua_State *L)
{
    const int charaindex = luaL_checkint(L, 1);
    const int petindex = luaL_checkint(L, 2);
    const int skillID = luaL_checkint(L, 3);
    lua_pushinteger(L, NPC_CHECKFREEPETSKILL(charaindex, petindex, skillID));
    return 1;
}
...
#endif
    { "ClsEvent", ClsEvent },
    { "CheckNowEvent", CheckNowEvent },
    { "CheckEvent", CheckEvent },
    { "SellPet", SellPet },
    { "PoolPet", PoolPet },
    { "PoolItem", PoolItem },
    { "EventID", EventID },
    { "CheckFreePetSkill", CheckFreePetSkill },
    {NULL,                          NULL}
};
