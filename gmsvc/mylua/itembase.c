...
static int getAtkFromITEMtabl(lua_State *L)
{
    const int id = luaL_checkint(L, 1);
    lua_pushinteger(L, ITEMTBL_getInt(id, ITEM_MODIFYATTACK));
    return 1;
}

static int getDefFromITEMtabl(lua_State *L)
{
    const int id = luaL_checkint(L, 1);
    lua_pushinteger(L, ITEMTBL_getInt(id, ITEM_MODIFYDEFENCE));
    return 1;
}

static int getQuickFromITEMtabl(lua_State *L)
{
    const int id = luaL_checkint(L, 1);
    lua_pushinteger(L, ITEMTBL_getInt(id, ITEM_MODIFYQUICK));
    return 1;
}
...
static int getCharItemtabl(lua_State *L)
{
    const int itemid = luaL_checkint(L, 1);
    const int element = getCharBaseValue(L, 2, ItemBaseChar, arraysizeof(ItemBaseChar));
    lua_pushstring(L, ITEMTBL_getChar(itemid, element));
    return 1;
}

static int getIntItemtabl(lua_State *L)
{
    const int itemid = luaL_checkint(L, 1);
    const int element = getCharBaseValue(L, 2, ItemBaseInt, arraysizeof(ItemBaseInt));
    lua_pushinteger(L, ITEMTBL_getInt(itemid, element));
    return 1;
}

static int getIntNum(lua_State *L)
{
    lua_pushstring(L, ITEM_DATAINTNUM);
    return 1;
}

static int getCharNum(lua_State *L)
{
    lua_pushstring(L, ITEM_DATACHARNUM);
    return 1;
}
...
static const luaL_Reg itemlib[] =
{
    ...
    {"getAtkFromITEMtabl",      getAtkFromITEMtabl},
    {"getDefFromITEMtabl",      getDefFromITEMtabl},
    {"getQuickFromITEMtabl",        getQuickFromITEMtabl},
    {"UpdataItemOne",                   UpdataItemOne},
    {"UpdataHaveItemOne",           UpdataHaveItemOne},

    { "getCharItemtabl", getCharItemtabl },
    { "getIntItemtabl", getIntItemtabl },
    { "getIntNum", getIntNum },
